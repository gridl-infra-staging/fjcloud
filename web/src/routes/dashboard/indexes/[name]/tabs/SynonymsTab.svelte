<script lang="ts">
	...
	$effect(() => {
		if (newSynonymJson.trim().length === 0) {
			newSynonymJson = JSON.stringify(defaultSynonymForType(newSynonymType, newSynonymObjectID), null, 2);
		}
	});
	...
	function synonymSummary(synonym: Synonym): string {
		...
		<div class="mb-6 rounded-lg bg-white p-6 shadow" data-testid="synonyms-section" data-index={index.name}>
			...
			{#if synonyms === null}
				<p class="mb-6 text-sm text-amber-700">Synonyms could not be loaded. Try refreshing the page.</p>
						...
						<tbody class="divide-y">
							{#each synonyms.hits as synonym (synonym.objectID)}
								<tr>
									...
									<td class="px-4 py-2">
										<span class="inline-flex rounded-full bg-blue-100 px-2 py-0.5 text-xs font-medium text-blue-800">{synonym.type}</span>
									</td>
			...
			<div class="rounded-md border border-gray-200 p-4">
				...
				<form method="POST" action="?/saveSynonym" use:enhance>
					<label for="synonym-object-id" class="mb-2 block text-sm font-medium text-gray-700">Object ID</label>
					<input
					...
					<label for="synonym-json" class="mb-2 block text-sm font-medium text-gray-700">Synonym JSON</label>
					<textarea
						id="synonym-json"
						name="synonym"
