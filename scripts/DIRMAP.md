...
| Directory | Summary |
| --- | --- |
| canary | The canary directory contains synthetic health probes and validation scripts for continuous production monitoring, including external AWS health checks, email deliverability verification, and system configuration contracts (EC2 firewall, OAuth, Lambda, API URLs). |
...
| reliability | The reliability directory contains shell scripts and utilities for capacity profiling across three document tiers, security automation checks (cargo audit, secret scanning, unsafe code patterns), and backend reliability gates that aggregate test results into machine-readable JSON summaries. |
