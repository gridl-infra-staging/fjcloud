...
impl RegionFailoverMonitor {
    ...
    pub fn new(
        ...
        Self::new_with_optional_lifecycle_guard(
            vm_repo,
            tenant_repo,
            replica_repo,
            alert_service,
    ...
    /// held, just before a tenant's guarded promotion writes. Test-only seam used
    /// to prove the open failover window excludes competing catalog admission.
    ...
    async fn try_failover_tenant(
        ...
        let promoted = match &self.lifecycle_lease {
            Some(lease) => {
                lease
                    .guarded_mutation(
                        tenant.customer_id,
