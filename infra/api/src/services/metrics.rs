use prometheus::{
    Encoder, GaugeVec, HistogramVec, IntCounter, IntCounterVec, Registry, TextEncoder,
};
...
impl MetricsCollector {
    /// Creates an isolated Prometheus registry and registers six metric families:
    /// an HTTP request counter, a request duration histogram, a panic counter,
    /// an active tenants gauge, a VM count gauge vec (labeled by status), and
    /// an active indexes gauge.
    pub fn new() -> Self {
        ...
        let panics_total = IntCounter::new(
            "panics_total",
            "Total number of recovered application panics",
        )
        .expect("metric can be created");
        ...
        registry
            ...
            .expect("metric can be registered");
        registry
            .register(Box::new(panics_total.clone()))
            .expect("metric can be registered");
        registry
    ...
    /// Record a recovered application panic.
    pub fn record_panic(&self) {
        self.panics_total.inc();
    }

    /// Return the total number of recovered application panics recorded by this collector.
    pub fn panic_total(&self) -> u64 {
        self.panics_total.get()
    }
...
mod tests {
    ...
    /// Verifies that a freshly created [`MetricsCollector`] exposes all six
    ...
    fn new_creates_registry_with_all_metrics() {
        ...
        assert!(text.contains("panics_total"), "missing panics_total");
        assert!(
    ...
    #[test]
    fn record_panic_increments_counter_and_renders_metric_family() {
        let collector = MetricsCollector::new();
        ...
        assert!(text.contains("# HELP panics_total"));
        assert!(text.contains("# TYPE panics_total counter"));
        assert!(text.contains("panics_total 1"));
    }

    #[test]
    fn panic_total_reports_recorded_panics() {
        let collector = MetricsCollector::new();

        assert_eq!(collector.panic_total(), 0);

        collector.record_panic();
        collector.record_panic();

        assert_eq!(collector.panic_total(), 2);
    }
