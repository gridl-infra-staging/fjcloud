...
assert_resource_block_contains_multiline "$iam_file" "aws_iam_role_policy" "fjcloud_ssm_read" '\{[[:space:]]*Effect[[:space:]]*=[[:space:]]*"Allow"[[:space:]]*Action[[:space:]]*=[[:space:]]*\["ssm:DescribeParameters"\][[:space:]]*Resource[[:space:]]*=[[:space:]]*"\*"[[:space:]]*\}' "IAM policy grants unscoped ssm:DescribeParameters for Parameter Store listing"
