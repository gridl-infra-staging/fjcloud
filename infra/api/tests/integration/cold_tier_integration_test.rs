...
async fn tenant_map_includes_tier_field() {
    ...
    let app = crate::common::test_app_with_tenant_repo(tenant_repo.clone());
    let response = app
        .oneshot(
            axum::http::Request::builder()
                .uri("/internal/tenant-map")
                .header("x-internal-key", crate::common::TEST_INTERNAL_AUTH_TOKEN)
...
async fn cold_storage_usage_endpoint_returns_snapshot_sizes() {
    let cold_snapshot_repo = crate::common::mock_cold_snapshot_repo();
    let customer_id = uuid::Uuid::new_v4();
    ...
    let app = crate::common::test_app_with_cold_snapshot_repo(cold_snapshot_repo);
    let response = app
        .oneshot(
            axum::http::Request::builder()
