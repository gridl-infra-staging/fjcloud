...
const RECOMMENDATION_FIELD_LABELS = {
	'recommendations-object-id': 'Object ID'
} as const;
...
function recommendationField(section: Locator, fieldId: keyof typeof RECOMMENDATION_FIELD_LABELS) {
...
async function expectTooltipVisible(
	page: Page,
	section: Locator,
	triggerName: string,
	expectedCopy: string
) {
	const trigger = section.getByRole('button', { name: triggerName });
	await expect(trigger).toHaveAttribute('aria-describedby', /.+/);
	await trigger.hover();
	await expect(page.getByRole('tooltip').filter({ hasText: expectedCopy })).toBeVisible();
}

test.describe('Recommendations tab rendering', () => {
	test.describe.configure({ timeout: 120_000 });

	test('advanced-control tooltips explain the current recommendation controls', async ({
		page,
		createUser,
		loginAs,
		testRegion
		...
		const objectIdTrigger = section.getByRole('button', {
			name: 'What the recommendation object ID is for'
		});
		await expect(objectIdTrigger).toHaveCount(0);
		await expectTooltipVisible(
			page,
			section,
			'What the recommendation facet name is for',
			'Choose the facet attribute whose values you want to trend, such as category or brand.'
		);
		await expectTooltipVisible(
			page,
			section,
			'What the recommendation facet value is for',
	...
	test('happy-path submit renders recommendation hits', async ({
		...
		await recommendationField(section, 'recommendations-object-id').fill(seeded.primaryObjectID);
