...
| Directory | Summary |
| --- | --- |
| canary | Canary contains synthetic monitoring scripts that verify customer-facing flows and external system health, plus bash contract test suites that validate critical security gates and infrastructure boundaries. |
| chaos | The chaos directory contains bash scripts for testing infrastructure resilience and failover mechanisms, including high-availability region failover proof-of-concept, health monitoring alerts during region failures, and metering-agent circuit breaker behavior under failure conditions. |
| dev | This script migrates integration test files from the top-level tests directory into a consolidated `integration/` subdirectory, rewriting module declarations and references to use crate-scoped paths. |
| launch | The `launch` directory contains shell scripts for post-deployment verification and evidence capture, including browser test automation against staging, tenant isolation probes, billing cross-check input capture, and SSM-based environment hydration. |
| lib | This directory contains a collection of shared shell script helpers that provide reusable functionality for backend validation, API operations, Stripe integration, database operations, and infrastructure verification across the fjcloud automation scripts. |
| load | The load directory provides regression checking functions for the load testing harness, comparing test results against baseline measurements across five key API endpoints (health, search_query, index_create, admin_tenant_list, document_ingestion) by evaluating latency percentiles, throughput, and error rates with configurable thresholds. |
| reliability | The reliability directory contains shell scripts for backend validation, including capacity profiling across document tiers, security automation gates (cargo audit, secret scanning, unsafe code detection), test data seeding, and VM inventory reconciliation against live AWS EC2 state. |
| stripe | This directory contains Stripe integration scripts for managing the Flapjack billing system, specifically for configuring the Stripe Customer Portal and creating the product catalog against a designated Stripe account with multi-account support. |
| tests | Smoke test coverage for customer broadcast and SES bounce/complaint workflows, with shared bash utilities for integration testing and validation scenarios across the fjcloud project. |
| vlm | The vlm directory contains verdict aggregation logic and environment helper utilities for the VLM judge system. |
| w3_triage | The w3_triage directory contains tooling to intake audit recommendations, probe live system state, and author staged work lanes for triage execution. |
<!-- [scrai:end] -->
