...
if [[ "$payload" == *'url = "https://api.cloudflare.com/client/v4/zones/cf_zone_foo_test'* ]]; then
  if [[ "$payload" == *'dns_records'* ]]; then
    printf '{"success":true,"result":[{"name":"flapjack.foo","type":"CNAME","content":"fjcloud-staging-alb-1511805790.us-east-1.elb.amazonaws.com","proxied":false},{"name":"api.flapjack.foo","type":"CNAME","content":"fjcloud-staging-alb-1511805790.us-east-1.elb.amazonaws.com","proxied":false},{"name":"www.flapjack.foo","type":"CNAME","content":"fjcloud-staging-alb-1511805790.us-east-1.elb.amazonaws.com","proxied":false},{"name":"cloud.flapjack.foo","type":"CNAME","content":"staging.flapjack-cloud.pages.dev","proxied":true}]}'
    exit 0
  fi
...
# ---------- Cloudflare parent zone covers staging subdomain ----------
echo ""
echo "--- Cloudflare parent zone covers staging subdomain ---"
...
output=""
exit_code=0
output=$(run_script --domain staging.flapjack.foo) || exit_code=$?

if [[ "$exit_code" -eq 20 ]]; then
  pass "Cloudflare parent zone lets staging subdomain reach runtime checks"
else
  fail "Cloudflare parent zone lets staging subdomain reach runtime checks (got $exit_code)"
fi
if echo "$output" | rg -q 'Cloudflare zone .* covers staging\.flapjack\.foo'; then
  pass "Cloudflare parent-zone output confirms subdomain coverage"
else
  fail "Cloudflare parent-zone output confirms subdomain coverage"
fi
if ! echo "$output" | rg -q 'PREFLIGHT FAIL \[cloudflare_dns\]'; then
  pass "Cloudflare parent-zone check does not emit a DNS preflight failure"
else
  fail "Cloudflare parent-zone check does not emit a DNS preflight failure"
fi
teardown
