...
```bash
# Set degraded with a public message:
scripts/set_status.sh --status degraded --message "Investigating elevated error rates."

# Set outage with a public message and deploy:
scripts/set_status.sh --status outage --message "API is unreachable." --publish
```

- Use `scripts/set_status.sh` to write `SERVICE_STATUS`, `SERVICE_STATUS_UPDATED`, and `SERVICE_STATUS_MESSAGE` into `web/wrangler.toml`. Pass `--publish` to deploy the update immediately.
- Pass `--message` to set a public incident message on `/status`. Omitting `--message` or passing an empty string clears the message.
- The script auto-stamps `SERVICE_STATUS_UPDATED` to current UTC time unless `--updated` is given explicitly.
- See `scripts/set_status.sh --help` for the full option reference.
...
- Clear customer-facing incident status as soon as mitigation is confirmed:
  ```bash
  scripts/set_status.sh --status operational --publish
  ```
  Omitting `--message` clears the public incident message. The script auto-stamps
  `SERVICE_STATUS_UPDATED` to the current UTC time.
