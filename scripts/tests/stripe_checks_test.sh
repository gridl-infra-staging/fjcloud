...
# check_stripe_account_status tests
# Pure body-parser: takes a Stripe GET /v1/account JSON body and emits only
# booleans/counts (never account id, email, or raw disabled_reason text).
test_check_stripe_account_status_ready_emits_expected_lines() {
    local ready_body='{"id":"acct_ready_example","object":"account","charges_enabled":true,"payouts_enabled":true,"details_submitted":true,"requirements":{"currently_due":[],"past_due":[],"disabled_reason":null}}'
    local output exit_code=0
    output="$(bash -c "
        source '$REPO_ROOT/scripts/lib/stripe_checks.sh'
        check_stripe_account_status '$ready_body'
    " 2>&1)" || exit_code=$?

    assert_eq "$exit_code" "0" "check_stripe_account_status should return 0 for a fully-ready account"
    assert_eq "$output" "charges_enabled=true
payouts_enabled=true
details_submitted=true
currently_due_count=0
past_due_count=0
disabled_reason_present=false" "ready account should emit exact booleans/counts and nothing else"
}

test_check_stripe_account_status_not_ready_fails_with_reason() {
    # Hand-counted: currently_due has 2 entries, past_due has 1.
    local not_ready_body='{"id":"acct_1MPIIexample","object":"account","email":"pii@example.com","charges_enabled":false,"payouts_enabled":false,"details_submitted":false,"requirements":{"currently_due":["external_account","tos_acceptance.date"],"past_due":["individual.verification.document"],"disabled_reason":"requirements.past_due"}}'
    local output exit_code=0
    output="$(BACKEND_LIVE_GATE=1 bash -c "
        source '$REPO_ROOT/scripts/lib/stripe_checks.sh'
        check_stripe_account_status '$not_ready_body'
    " 2>&1)" || exit_code=$?

    assert_eq "$exit_code" "1" "check_stripe_account_status should fail (exit 1) for a not-ready account under live gate"
    assert_contains "$output" "payouts_enabled=false" "not-ready output should report payouts_enabled=false"
    assert_contains "$output" "currently_due_count=2" "not-ready output should hand-count currently_due entries"
    assert_contains "$output" "past_due_count=1" "not-ready output should hand-count past_due entries"
    assert_contains "$output" "disabled_reason_present=true" "not-ready output should flag disabled_reason presence"
    assert_contains "$output" "REASON: stripe_account_not_ready" "not-ready failure should emit stripe_account_not_ready reason code"
}

test_check_stripe_account_status_emits_no_pii() {
    local pii_body='{"id":"acct_1MPIIexample","object":"account","email":"pii@example.com","charges_enabled":false,"payouts_enabled":true,"details_submitted":true,"requirements":{"currently_due":["external_account"],"past_due":[],"disabled_reason":"rejected.fraud"}}'
    local output exit_code=0
    output="$(BACKEND_LIVE_GATE=1 bash -c "
        source '$REPO_ROOT/scripts/lib/stripe_checks.sh'
        check_stripe_account_status '$pii_body'
    " 2>&1)" || exit_code=$?

    assert_not_contains "$output" "acct_1MPIIexample" "output must not leak the account id"
    assert_not_contains "$output" "pii@example.com" "output must not leak the account email"
    assert_not_contains "$output" "rejected.fraud" "output must not leak the raw disabled_reason string"
    assert_not_contains "$output" "external_account" "output must not leak raw requirement descriptions"
}
...
test_check_stripe_webhook_forwarding_skips_when_gate_off
echo ""
echo "--- check_stripe_account_status ---"
test_check_stripe_account_status_ready_emits_expected_lines
test_check_stripe_account_status_not_ready_fails_with_reason
test_check_stripe_account_status_emits_no_pii
echo ""
