...
assert_contains 'Dashboard route error boundary .*No known gap in the centralized browser-runtime seam; customer-visible references remain `web-` prefixed by design, backend `x-request-id` stays maintainer-only in route-error logs, and sanitized browser-runtime reports flow through `/browser-errors`\.' "$COVERAGE_FILE" "dashboard error boundary row documents repo-owned browser-runtime ingestion without reopening customer/backend correlation"
