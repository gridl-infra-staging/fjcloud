<script lang="ts">
	...
	let lastSecuritySourceAppendedToastState = $state(false);
	let lastSecuritySourceDeletedToastState = $state(false);
	...
	$effect(() => {
		if (securitySourceAppended && !lastSecuritySourceAppendedToastState) {
			toast.success('Security source added.', { duration: TOAST_DURATION_MS });
		}
		lastSecuritySourceAppendedToastState = securitySourceAppended;

		if (securitySourceDeleted && !lastSecuritySourceDeletedToastState) {
			toast.success('Security source deleted.', { duration: TOAST_DURATION_MS });
		}
		lastSecuritySourceDeletedToastState = securitySourceDeleted;
	});
...
<div
	class="space-y-6"
	data-testid={INDEX_DETAIL_TAB_PANEL_TEST_IDS['security-sources']}
