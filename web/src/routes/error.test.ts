...
describe('Public error boundary (+error.svelte)', () => {
	...
	it('renders public auth navigation without changing recovery actions', async () => {
		mockPage = {
			status: 404,
			error: { message: 'Not found' },
			url: new URL('http://localhost/missing')
		...
		expect(within(navigation).queryByRole('link', { name: 'Sign Up' })).not.toBeInTheDocument();
		expect(screen.getByRole('link', { name: /go.*home|back.*home|home/i })).toHaveAttribute(
			'href',
			'/'
		);
	});
