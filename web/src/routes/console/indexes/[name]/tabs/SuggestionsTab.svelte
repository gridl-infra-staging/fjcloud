<script lang="ts">
	...
	let lastQsConfigSavedToastState = $state(false);
	let lastQsConfigDeletedToastState = $state(false);
	let lastQsBuildQueuedToastState = $state(false);

	$effect(() => {
		if (qsConfigSaved && !lastQsConfigSavedToastState) {
			toast.success('Suggestions config saved.', { duration: TOAST_DURATION_MS });
		}
		lastQsConfigSavedToastState = qsConfigSaved;

		if (qsConfigDeleted && !lastQsConfigDeletedToastState) {
			toast.success('Suggestions config deleted.', { duration: TOAST_DURATION_MS });
		}
		lastQsConfigDeletedToastState = qsConfigDeleted;

		if (qsBuildQueued && !lastQsBuildQueuedToastState) {
			toast.info('Suggestions rebuild queued.', { duration: TOAST_DURATION_MS });
		}
		lastQsBuildQueuedToastState = qsBuildQueued;
	});
</script>

<div
	class="mb-6 rounded-lg bg-white p-6 shadow"
	data-testid={INDEX_DETAIL_TAB_PANEL_TEST_IDS.suggestions}
