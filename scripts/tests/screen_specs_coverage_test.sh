...
assert_contains '^- \[x\] Public and console boundaries each render exactly one `Support reference` label\.$' "$ERROR_BOUNDARIES_SPEC" "error boundaries spec marks support-reference label criterion satisfied"
...
assert_contains '^- \[x\] Backend `x-request-id` values are preserved by `ApiRequestError` metadata \(`web/src/lib/api/client.ts`\) and paired with the web support reference in route-error logs \(`web/src/hooks.server.ts`\)\.$' "$ERROR_BOUNDARIES_SPEC" "error boundaries spec marks backend request-id correlation satisfied"
assert_contains '^- \[x\] Browser-only uncaught errors and unhandled promise rejections are normalized by `web/src/lib/error-boundary/client-runtime.ts` into one sanitized support reference, keep browser-console maintainer metadata with `backend_correlation: '\''absent'\''`, and submit the same sanitized payload to `/browser-errors` via `infra/api/src/routes/browser_error_reporting.rs`\.$' "$ERROR_BOUNDARIES_SPEC" "error boundaries spec marks centralized browser-runtime reporting satisfied"
assert_contains '^- Component tests: `web/src/lib/error-boundary/recovery-copy.test.ts`; `web/src/lib/error-boundary/SupportReferenceBlock.test.ts`; `web/src/lib/error-boundary/client-runtime.test.ts`; `web/src/routes/layout.test.ts`; `web/src/routes/error.test.ts`; `web/src/routes/console/error.test.ts`$' "$ERROR_BOUNDARIES_SPEC" "error boundaries spec component coverage lists shared helper/component and route owners"
...
assert_contains 'Public route error boundary .*web/src/lib/error-boundary/recovery-copy.test.ts.*web/src/lib/error-boundary/SupportReferenceBlock.test.ts.*web/src/lib/error-boundary/client-runtime.test.ts.*web/src/routes/layout.test.ts.*web/src/routes/error.test.ts.*web/src/routes/console/error.test.ts' "$COVERAGE_FILE" "public error boundary coverage row includes shared helper/component and route tests"
assert_contains 'Console route error boundary .*web/src/lib/error-boundary/recovery-copy.test.ts.*web/src/lib/error-boundary/SupportReferenceBlock.test.ts.*web/src/lib/error-boundary/client-runtime.test.ts.*web/src/routes/layout.test.ts.*web/src/routes/error.test.ts.*web/src/routes/console/error.test.ts' "$COVERAGE_FILE" "console error boundary coverage row includes shared helper/component and route tests"
assert_contains 'Public route error boundary .*No known gap in the centralized browser-runtime seam; customer-visible references remain `web-` prefixed by design, backend `x-request-id` stays maintainer-only in route-error logs, and sanitized browser-runtime reports flow through `/browser-errors`\.' "$COVERAGE_FILE" "public error boundary row documents repo-owned browser-runtime ingestion without reopening customer/backend correlation"
assert_contains 'Console route error boundary .*No known gap in the centralized browser-runtime seam; customer-visible references remain `web-` prefixed by design, backend `x-request-id` stays maintainer-only in route-error logs, and sanitized browser-runtime reports flow through `/browser-errors`\.' "$COVERAGE_FILE" "console error boundary row documents repo-owned browser-runtime ingestion without reopening customer/backend correlation"
