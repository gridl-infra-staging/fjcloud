...
async fn pay_invoice_against_live_sandbox() {
    ...
    let payment_method_id =
        live_stripe_helpers::attach_test_payment_method(&stripe.client, &stripe_customer_id).await;
    ...
    live_stripe_helpers::delete_stripe_customer(&stripe.client, &stripe_customer_id).await;
}
