...
fn seed_api_key(customer_id: Uuid) -> Arc<crate::common::MockApiKeyRepo> {
    seed_api_key_with_scopes(customer_id, vec!["read".to_string(), "search".to_string()])
}

fn seed_api_key_with_scopes(
    customer_id: Uuid,
    scopes: Vec<String>,
...
#[tokio::test]
async fn discover_provisioning_intent_index_returns_not_found_even_with_vm_assignment() {
    let (tenant_repo, vm_repo, customer_id, deployment_id) = setup();
    let vm = create_test_vm(&vm_repo, "vm-provisioning.flapjack.foo").await;

    tenant_repo
        .create(customer_id, "provisioning-idx", deployment_id)
        .await
        .unwrap();
    tenant_repo
        .set_vm_id(customer_id, "provisioning-idx", vm.id)
        .await
        .unwrap();
    tenant_repo
        .set_tier(customer_id, "provisioning-idx", "provisioning")
        .await
        .unwrap();
    ...
    let result = svc.discover(customer_id, "provisioning-idx").await;
    assert!(
        matches!(
            result,
            Err(api::services::discovery::DiscoveryError::NotFound)
        ),
        "provisioning intent rows must not be discovery-visible"
    );
}

#[tokio::test]
async fn discover_deleting_intent_index_returns_not_found_through_legacy_fallback() {
    let (tenant_repo, vm_repo, customer_id, deployment_id) = setup();

    tenant_repo
        .create(customer_id, "deleting-idx", deployment_id)
        .await
        .unwrap();
    tenant_repo
        .set_tier(customer_id, "deleting-idx", "deleting")
        .await
        .unwrap();
    ...
    let result = svc.discover(customer_id, "deleting-idx").await;
    assert!(
        matches!(
            result,
            Err(api::services::discovery::DiscoveryError::NotFound)
        ),
        "deleting intent rows must not fall back to deployment discovery"
    );
}
