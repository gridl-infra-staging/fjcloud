...
LOCAL_RUNBOOK_DOC="$REPO_ROOT/docs/runbooks/local-dev.md"
...
IMPLEMENTED_ROADMAP_DOC="$REPO_ROOT/implemented/2026-06-05_roadmap_v2_reshape_archive.md"
...
owner_resolves_to_stage1_class() {
    ...
    local owner_paths
    owner_paths="$(printf '%s\n' "$normalized_owner" | grep -oE '(PROJECT_OVERVIEW\.md|ROADMAP\.md|docs/[A-Za-z0-9_./+-]+|scripts/[A-Za-z0-9_./+-]+|ops/[A-Za-z0-9_./+-]+|chats/icg/[A-Za-z0-9_./+-]+|web/[A-Za-z0-9_./+-]+|infra/[A-Za-z0-9_./+-]+)' || true)"
...
test_local_dev_runbook_links_signoff_checklist() {
    local local_runbook_content
    local_runbook_content="$(cat "$LOCAL_RUNBOOK_DOC")"

    assert_contains "$local_runbook_content" "LOCAL_SIGNOFF_CHECKLIST.md" \
        "local-dev runbook should link the local signoff checklist"
}
...
test_evidence_bundle_references_canonical_docs() {
    ...
    assert_contains "$evidence_content" "PROJECT_OVERVIEW.md" \
        "evidence bundle should link stable priority rationale"
    assert_contains "$evidence_content" "docs/runbooks/local-dev.md" \
        "evidence bundle should link local-dev signoff owner"
}
...
test_evidence_bundle_has_no_secret_like_values() {
    local current_alert_bundle current_alert_bundle_dir alert_bundle_scan_payload
...
test_status_docs_track_bounce_complaint_follow_on_owner_breadcrumb() {
    ...
    assert_contains "$status_docs_content" "docs/runbooks/email-production.md" \
        "status docs should point operators to the email-production runbook owner"
    assert_contains "$status_docs_content" "docs/research/ses_bounce_complaint_gap.md" \
        "status docs should point operators to the SES bounce/complaint gap rationale owner"
    assert_contains "$status_docs_content" "scripts/probe_ses_bounce_complaint_e2e.sh" \
        "status docs should track the bounce/complaint E2E probe owner seam"
    if [[ "$status_docs_content" == *"ops/terraform/dns/main.tf"* ]] || [[ "$status_docs_content" == *"ops/terraform/monitoring/main.tf"* ]]; then
        pass "status docs should track Terraform SES event wiring owner"
    else
        fail "status docs should track Terraform SES event wiring owner"
    fi
    assert_contains "$status_docs_content" "ops/terraform/monitoring/" \
        "status docs should track monitoring Terraform SES event-destination wiring owner"
    assert_contains "$status_docs_content" "docs/runbooks/evidence/ses-deliverability/20260501T193004Z_bounce_e2e_proof_GREEN/SUMMARY.md" \
        "status docs should track the committed bounce/complaint proof bundle pointer"
...
test_beta_checklist_top_of_file_authority_and_deferral_language() {
    ...
    assert_contains "$beta_preamble" "PROJECT_OVERVIEW.md" \
        "beta checklist preamble should defer durable priority rationale to PROJECT_OVERVIEW.md"
...
test_beta_checklist_citations_and_antipattern_guards() {
    ...
    assert_not_contains "$beta_content" "launch readiness is currently blocked by the preserved Stage 3 paid-beta RC result" \
        "beta checklist should not copy stale blocker prose from retired priorities"
    assert_not_contains "$beta_content" "Current staging status" \
        "beta checklist should not copy stale blocker section prose from retired priorities"
...
test_status_docs_align_to_current_alert_bundle_and_preserved_stage3_rc_verdict() {
    local roadmap_content overview_content staging_evidence_content implemented_content status_docs_content
    ...
    status_docs_content="$(printf '%s\n%s\n%s\n%s\n' "$roadmap_content" "$overview_content" "$staging_evidence_content" "$implemented_content")"
    current_alert_bundle="$(cat "$ALERT_DELIVERY_CURRENT_BUNDLE_FILE")"
    current_alert_bundle_dir="$REPO_ROOT/$current_alert_bundle"
...
test_paid_lifecycle_cross_check_pointers_stay_aligned() {
    ...
    assert_contains "$status_docs_content" "$expected_verdict" \
        "status docs Apr 28 cross-check references should mirror the bundle verdict token"
...
test_canary_customer_loop_status_docs_resolve_through_current_bundle() {
    ...
    roadmap_content="$(cat "$ROADMAP_DOC")"
    status_docs_content="$(printf '%s\n%s\n' "$(cat "$PROJECT_OVERVIEW_DOC")" "$roadmap_content")"
