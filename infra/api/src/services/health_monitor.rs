...
/// Bounded timing policy for waiting until a newly provisioned engine is healthy.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct EngineHealthWaitPolicy {
...
impl EngineHealthWaitPolicy {
    pub fn new(
        ...
        assert!(!retry_interval.is_zero(), "retry interval must be non-zero");
...
impl Default for EngineHealthWaitPolicy {
    fn default() -> Self {
        Self::new(
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum EngineHealthWaitStatus {
    Healthy,
    DeadlineExhausted,
}
...
/// Wait until the engine responds healthy or the bounded policy is exhausted.
///
/// The 2026-07 postmortem found 28 production VMs recorded healthy while
/// `flapjack.service` crash-looped. Instance-running plus DNS-written is not
/// provisioned, and an unbounded health-check hang is not an acceptable failure mode.
pub async fn await_engine_health<C>(
    health_client: Arc<C>,
    flapjack_url: Option<String>,
    policy: EngineHealthWaitPolicy,
) -> EngineHealthWaitStatus
where
    C: HealthCheckClient + ?Sized,
{
    let deadline = tokio::time::Instant::now() + policy.overall_deadline;

    loop {
        let remaining = deadline.saturating_duration_since(tokio::time::Instant::now());
        if remaining.is_zero() {
            return EngineHealthWaitStatus::DeadlineExhausted;
        }

        let attempt_timeout = policy.attempt_timeout.min(remaining);
        let result =
            tokio::time::timeout(attempt_timeout, health_client.check(flapjack_url.clone())).await;
        if matches!(result, Ok(HealthCheckResult::Healthy)) {
            return EngineHealthWaitStatus::Healthy;
        }
