...
# Map the canonical database-url SSM parameter owner to the target environment.
# Callers that must accept only the shared staging/prod DB-url owners should use
# this helper instead of re-implementing DATABASE_URL_SSM_PARAM parsing.
staging_db_database_url_ssm_environment() {
    case "${DATABASE_URL_SSM_PARAM:-}" in
...
staging_db_env_tag() {
    local canonical_env
    if canonical_env="$(staging_db_database_url_ssm_environment)"; then
        echo "$canonical_env"
    elif [[ "${DATABASE_URL_SSM_PARAM:-}" == */prod/* ]]; then
        echo "prod"
