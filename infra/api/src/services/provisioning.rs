...
const ENGINE_HEALTH_FAILURE_REASON: &str = "engine_health_check_failed";
...
pub struct ProvisioningService {
    ...
    engine_health_wait_policy: EngineHealthWaitPolicy,
}
...
impl ProvisioningService {
    ...
    pub fn new(
        ...
        Self {
            ...
            customer_repo,
            dns_domain,
            engine_health_client: Arc::new(ReqwestHealthCheckClient::new(reqwest::Client::new())),
            engine_health_wait_policy: EngineHealthWaitPolicy::default(),
        }
    }
    ...
    pub fn with_engine_health_client_for_test(
        ...
        Arc::new(Self {
            vm_provisioner: Arc::clone(&self.vm_provisioner),
            dns_manager: Arc::clone(&self.dns_manager),
            node_secret_manager: Arc::clone(&self.node_secret_manager),
            deployment_repo: Arc::clone(&self.deployment_repo),
            customer_repo: Arc::clone(&self.customer_repo),
            dns_domain: self.dns_domain.clone(),
            engine_health_client,
            engine_health_wait_policy,
        })
    }
    ...
    async fn mark_provisioning_failed(&self, deployment_id: Uuid, failure_reason: Option<&str>) {
        match self
            .deployment_repo
            .mark_failed_provisioning(deployment_id, failure_reason)
            .await
        {
            Ok(true) => {}
            Ok(false) => {
    ...
    pub async fn provision_deployment(
        ...
        auto_provision::ensure_supported_vm_provider(vm_provider)?;
    ...
    pub async fn complete_provisioning(
        ...
        let (ip, vm_instance) = match self.create_node_secret_and_vm(&deployment, &hostname).await {
            Ok(result) => result,
            Err(e) => {
                self.mark_provisioning_failed(deployment_id, None).await;
                return Err(e);
            }
        };
    ...
    async fn create_node_secret_and_vm(
        ...
        auto_provision::ensure_supported_vm_provider(&deployment.vm_provider)?;
    ...
    async fn persist_dns_and_deployment(
        ...
        if let Err(e) = self.dns_manager.create_record(hostname, ip).await {
            self.rollback_provisioned_resources(deployment, Some(provider_vm_id), None)
                .await;
            self.mark_provisioning_failed(deployment.id, None).await;
            return Err(ProvisioningError::DnsFailed(e.to_string()));
        }

        if matches!(
            await_engine_health(
                Arc::clone(&self.engine_health_client),
                Some(flapjack_url.to_string()),
                self.engine_health_wait_policy,
            )
            .await,
            EngineHealthWaitStatus::DeadlineExhausted
        ) {
            self.rollback_provisioned_resources(deployment, Some(provider_vm_id), Some(hostname))
                .await;
            self.mark_provisioning_failed(deployment.id, Some(ENGINE_HEALTH_FAILURE_REASON))
                .await;
            return Err(ProvisioningError::ProvisionerFailed(
