...
const TENANT_ISOLATION_REPLAY_COMMAND_MARKER: &str =
    "cd infra && cargo test -p api --test platform --features proptest-tests \
...
impl RouteCase {
    ...
    fn settings_update() -> Self {
        ...
        Self {
            ...
            // This expectation follows the runtime proxy contract: update_settings()
            // accepts dashboard-shaped settings, then update_index_settings() mirrors
            // filterableAttributes to attributesForFaceting before sending to Flapjack.
            expected_proxy_body: Some(json!({
                "searchableAttributes": ["title", "body"],
                "filterableAttributes": ["category"],
    ...
    fn search() -> Self {
        ...
        Self {
            ...
            expected_proxy_body: Some(json_body),
        }
    }
