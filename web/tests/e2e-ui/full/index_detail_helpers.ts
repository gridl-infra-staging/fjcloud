...
export async function openIndexDetailTab(
	...
	if (tabName !== 'Settings' || sectionTestId !== 'settings-section') {
		return openFixtureIndexDetailTab(page, tabName, sectionTestId, expectNotMountedBeforeOpen);
	}
	...
	const accessibleSettingsSection = page
		.getByRole('heading', { name: 'Settings', exact: true })
		.locator('xpath=ancestor::div[.//textarea[@aria-label="Settings JSON"]][1]');
	await expect(accessibleSettingsSection).toBeVisible({ timeout: 10_000 });
	return accessibleSettingsSection;
}

export async function assertSynonymsDataPathHealthy(section: Locator): Promise<void> {
...
export async function openSynonymCreateDialog(page: Page): Promise<Locator> {
	...
	await expectEditorDialogTokens(page, page.getByTestId('editor-dialog'));
	return section;
}
...
export async function editSynonymThroughDialog(
	page: Page,
	objectId: string,
