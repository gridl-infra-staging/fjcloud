...
Validated source HEAD: 3305ea8f7fb8fff1629a7388d8f3f5a5290a4d83
UTC evidence timestamp: 20260603T233430Z

- Validation-cache hit: bash scripts/check-sizes.sh (clean-tree current-HEAD pass; no size violations reported)
- Validation-cache hit: bash scripts/tests/validate_customer_quickstart_test.sh (clean-tree current-HEAD pass; 90 passed, 0 failed)
...
- Hydrated staging setup: set -a; source .secret/.env.secret; set +a; eval "$(bash scripts/launch/hydrate_seeder_env_from_ssm.sh staging)"; export SES_REGION=us-east-1 INBOUND_ROUNDTRIP_S3_URI=s3://flapjack-cloud-releases/e2e-emails/ INBOUND_ROUNDTRIP_RECIPIENT_DOMAIN=test.flapjack.foo; bash scripts/validate_customer_quickstart.sh staging
