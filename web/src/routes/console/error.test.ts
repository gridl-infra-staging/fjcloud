 ...
 * into the console error boundary component under test.
 */
...
describe('Console error boundary (+error.svelte)', () => {
	...
	it('shows a link to /console for 404 recovery', async () => {
		mockPage = {
			status: 404,
			error: { message: 'Not found' },
			url: new URL('http://localhost/console/missing')
		...
		const consoleLink = screen.getByRole('link', { name: /console|go back/i });
	...
	it('shows /console link for generic 4xx recovery', async () => {
		mockPage = {
			status: 403,
			error: { message: 'Forbidden' },
			url: new URL('http://localhost/console/forbidden')
	...
	it.each([
		...
		'renders %s shape with console-safe copy, one support reference, and no backend request-id text',
	...
	it('auth__dashboard__error__desktop P.brand_palette_consistency uses diner fallback canvas and CTA styling', async () => {
		mockPage = {
			status: 500,
			error: { message: 'Internal server error' },
			url: new URL('http://localhost/console/broken')
	...
	it('keeps recovery copy and secondary status CTA on diner ink and cream accents', async () => {
		mockPage = {
			status: 500,
			error: { message: 'Internal server error' },
			url: new URL('http://localhost/console/broken')
