...
shell_quote_for_script() { printf '%q\n' "$1"; }

assert_nonzero_exit() {
    if [ "$1" -ne 0 ]; then pass "$2"; else fail "$2 (actual exit code was 0)"; fi
}

run_dir_count_under_root() {
    local count=0 d
    for d in "$1"/*; do [ -d "$d" ] && count=$((count + 1)); done
    printf '%s\n' "$count"
}
...
test_rc_invocation_helper_bridges_restricted_stripe_alias_without_overwriting_canonical_key() {
    setup_workspace
    local helper marker_file output
    helper="$TEST_WORKSPACE/scripts/lib/rc_invocation.sh"
    marker_file="$TEST_WORKSPACE/tmp/credential_exec_marker"

    if [ ! -f "$helper" ]; then
        fail "rc_invocation helper should exist for shared restricted Stripe alias bridging"
        return
    fi
    ...
    output="$(
        ...
        export STRIPE_SECRET_KEY_RESTRICTED='sk_test_$(touch '"$marker_file"')'
        unset STRIPE_SECRET_KEY
        rc_bridge_restricted_stripe_secret_key
        printf '%s\n' "$STRIPE_SECRET_KEY"

        export STRIPE_SECRET_KEY='sk_test_canonical_wins'
        rc_bridge_restricted_stripe_secret_key
        printf '%s\n' "$STRIPE_SECRET_KEY"
    )"

    if [ -e "$marker_file" ]; then
        fail "restricted Stripe alias bridge should treat alias value as inert data"
    else
        pass "restricted Stripe alias bridge should treat alias value as inert data"
    fi
    # shellcheck disable=SC2016
    assert_contains "$output" 'sk_test_$(touch '"$marker_file"')' "restricted Stripe alias bridge should export the literal restricted key value"
    assert_contains "$output" "sk_test_canonical_wins" "restricted Stripe alias bridge should not overwrite an existing canonical key"
}

test_rc_invocation_helper_builds_paid_beta_argv_in_exact_order() {
    setup_workspace
    local helper expected actual
    helper="$TEST_WORKSPACE/scripts/lib/rc_invocation.sh"

    if [ ! -f "$helper" ]; then
        fail "rc_invocation helper should exist for paid-beta RC argv construction"
        return
    fi
    ...
    actual="$(
        ...
        rc_build_paid_beta_argv \
            ...
            "$TEST_WORKSPACE/artifacts/03_paid_beta_rc" \
            "$TEST_WORKSPACE/inputs/credentials.env" \
            "2026-04" \
...
--artifact-dir=$TEST_WORKSPACE/artifacts/03_paid_beta_rc
...
EOF
    ...
    assert_eq "$actual" "$expected" "paid-beta RC argv helper should preserve exact delegated flag order"
}

test_rc_invocation_helper_credential_file_overrides_ambient_exports() {
    setup_workspace
    local helper env_helper credential_file expected actual
    helper="$TEST_WORKSPACE/scripts/lib/rc_invocation.sh"
    env_helper="$TEST_WORKSPACE/scripts/lib/env.sh"
    credential_file="$TEST_WORKSPACE/inputs/credentials.env"

    cat > "$credential_file" <<'ENVFILE'
AWS_ACCESS_KEY_ID=AKIA_FILE_WINS
AWS_SECRET_ACCESS_KEY=file-secret-wins
AWS_DEFAULT_REGION=us-west-2
STRIPE_SECRET_KEY_RESTRICTED=sk_test_file_restricted_wins
STRIPE_WEBHOOK_SECRET=whsec_file_wins
ENVFILE
    ...
    actual="$(
        ...
        # shellcheck disable=SC1090,SC1091
        source "$helper"
        export AWS_ACCESS_KEY_ID=AKIA_AMBIENT_SHOULD_LOSE
        export AWS_SECRET_ACCESS_KEY=ambient-secret-should-lose
        export AWS_DEFAULT_REGION=us-east-1
        # shellcheck disable=SC2031
        export STRIPE_SECRET_KEY_RESTRICTED=sk_test_ambient_should_lose
        export STRIPE_WEBHOOK_SECRET=whsec_ambient_should_lose
        rc_load_credential_env_file "$credential_file"
        printf '%s\n' "$AWS_ACCESS_KEY_ID"
        printf '%s\n' "$AWS_SECRET_ACCESS_KEY"
        printf '%s\n' "$AWS_DEFAULT_REGION"
        printf '%s\n' "$STRIPE_SECRET_KEY_RESTRICTED"
...
file-secret-wins
us-west-2
sk_test_file_restricted_wins
whsec_file_wins
EOF
)"

    assert_eq "$actual" "$expected" "credential env file should override matching ambient exported credentials"
}

test_rc_invocation_helper_rejects_directory_credential_path() {
    setup_workspace
    local helper env_helper credential_dir stderr_file exit_code
    helper="$TEST_WORKSPACE/scripts/lib/rc_invocation.sh"
    env_helper="$TEST_WORKSPACE/scripts/lib/env.sh"
    credential_dir="$TEST_WORKSPACE/inputs/credential_dir"
    stderr_file="$TEST_WORKSPACE/tmp/credential_dir_stderr.txt"
    ...
    (
        ...
        # shellcheck disable=SC1090,SC1091
        source "$helper"
        rc_load_credential_env_file "$credential_dir"
    ) 2>"$stderr_file" || exit_code=$?

    assert_nonzero_exit "$exit_code" "credential env directory path should fail before env loading"
    assert_contains "$(cat "$stderr_file")" "ERROR: credential env file is not readable: $credential_dir" "credential env directory rejection should preserve wrapper error text"
}
...
common_required_args() {
    ...
    printf '%s' "--sha=aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa --artifact-dir=$artifact_dir --credential-env-file=$credentials_file --billing-month=2026-04 --staging-smoke-api-ami-id=ami-0123456789abcdef0 --staging-smoke-flapjack-ami-id=ami-0fedcba9876543210"
}
...
test_cli_missing_required_flags_exit_2() {
    ...
    _run_post_deploy_capture --args "$args"
    assert_eq "$RUN_EXIT_CODE" "2" "missing --staging-smoke-api-ami-id should exit 2"
    assert_contains "$RUN_STDERR" "missing required argument: --staging-smoke-api-ami-id" "missing --staging-smoke-api-ami-id should emit canonical error text"
}

test_cli_rejects_invalid_rc_identifier_formats() {
    setup_workspace
    local artifact_root args
    artifact_root="$TEST_WORKSPACE/artifacts"

    args="--sha=not-a-sha --artifact-dir=$artifact_root/sha --credential-env-file=$TEST_WORKSPACE/inputs/credentials.env --billing-month=2026-04 --staging-smoke-api-ami-id=ami-0123456789abcdef0 --staging-smoke-flapjack-ami-id=ami-0fedcba9876543210 --dry-run"
    _run_post_deploy_capture --args "$args"
    assert_eq "$RUN_EXIT_CODE" "2" "invalid sha should exit 2"
    assert_contains "$RUN_STDERR" "ERROR: --sha must be a 40-character lowercase hexadecimal commit SHA" "invalid sha should reuse the canonical RC SHA validation text"

    args="--sha=aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa --artifact-dir=$artifact_root/month --credential-env-file=$TEST_WORKSPACE/inputs/credentials.env --billing-month=2026-13 --staging-smoke-api-ami-id=ami-0123456789abcdef0 --staging-smoke-flapjack-ami-id=ami-0fedcba9876543210 --dry-run"
    _run_post_deploy_capture --args "$args"
    assert_eq "$RUN_EXIT_CODE" "2" "invalid billing month should exit 2"
    assert_contains "$RUN_STDERR" "ERROR: --billing-month must use YYYY-MM format with month 01-12" "invalid billing month should reuse the canonical RC billing-month validation text"

    args="--sha=aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa --artifact-dir=$artifact_root/ami --credential-env-file=$TEST_WORKSPACE/inputs/credentials.env --billing-month=2026-04 --staging-smoke-api-ami-id=bad-ami --staging-smoke-flapjack-ami-id=ami-0fedcba9876543210 --dry-run"
    _run_post_deploy_capture --args "$args"
    assert_eq "$RUN_EXIT_CODE" "2" "invalid API AMI ID should exit 2"
    assert_contains "$RUN_STDERR" "ERROR: --staging-smoke-api-ami-id must use AMI ID format (ami-xxxxxxxx or ami-xxxxxxxxxxxxxxxxx)" "invalid API AMI ID should reuse the canonical RC AMI validation text"
}
...
_assert_stripe_live_key_rejected() {
    local key_value="$1" prefix_label="$2"
    setup_workspace
    cat > "$TEST_WORKSPACE/inputs/credentials.env" <<ENVFILE
AWS_ACCESS_KEY_ID=AKIAPOSTDEPLOYFIXTURE
AWS_SECRET_ACCESS_KEY=fixture-secret
AWS_DEFAULT_REGION=us-east-1
STRIPE_SECRET_KEY_RESTRICTED=${key_value}
STRIPE_WEBHOOK_SECRET=whsec_fixture_contract
ENVFILE
    ...
    assert_nonzero_exit "$RUN_EXIT_CODE" "$prefix_label key should be rejected"
    assert_contains "$RUN_STDERR" "STRIPE_SECRET_KEY must start with sk_test_ or rk_test_ (sk_live_ and rk_live_ keys are not allowed)" "$prefix_label live-key rejection should reuse canonical stripe_checks text"
    assert_contains "$RUN_STDERR" "REASON: stripe_key_bad_prefix" "$prefix_label live-key rejection should preserve stripe_key_bad_prefix reason semantics"
}

test_stripe_live_key_rejection_sk_live_reuses_canonical_text() {
    _assert_stripe_live_key_rejected "sk_live_FAKE_FOR_TEST" "sk_live_"
}

test_stripe_live_key_rejection_rk_live_reuses_canonical_text() {
    _assert_stripe_live_key_rejected "rk_live_FAKE_FOR_TEST" "rk_live_"
}
...
MOCK
    ...
    assert_contains "$summary_json" "\"staging_smoke_api_ami_id\":\"ami-0123456789abcdef0\"" "failed live mode summary should include staging_smoke_api_ami_id metadata"
    assert_contains "$summary_json" "\"staging_smoke_flapjack_ami_id\":\"ami-0fedcba9876543210\"" "failed live mode summary should include staging_smoke_flapjack_ami_id metadata"
}
