...
| `FJCLOUD_FLAPJACK_REQUIRED_SHA256`  | No       | —                            | Expected Flapjack binary SHA-256 used by API Algolia import admission and local-stack compatibility checks.                                                                                                                                                                                                                                    |
| `FJCLOUD_FLAPJACK_REQUIRED_CAPABILITY` | No    | `vectorSearchLocal`          | Required Flapjack capability used by API Algolia import admission and local-stack compatibility checks. `/health` may expose capabilities as a list or a map with the required capability set to `true`.                                                                                                                                       |
