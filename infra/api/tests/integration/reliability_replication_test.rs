...
) -> crate::common::ReplicationHarness {
    crate::common::setup_replication_harness(config, seed_replica_key).await
}

async fn setup_replication() -> crate::common::ReplicationHarness {
    setup_replication_inner(ReplicationConfig::default(), true).await
}

async fn setup_replication_with_config(
    config: ReplicationConfig,
) -> crate::common::ReplicationHarness {
    setup_replication_inner(config, true).await
}

async fn setup_replication_without_replica_key() -> crate::common::ReplicationHarness {
