...
async fn list_deployments_returns_200_excludes_terminated() {
    ...
    assert!(
        arr[0]["failure_reason"].is_null(),
        "ordinary deployment response must include failure_reason as JSON null"
    );
}

#[tokio::test]
async fn list_deployments_includes_engine_health_failure_reason() {
    let customer_repo = crate::common::mock_repo();
    let deployment_repo = crate::common::mock_deployment_repo();
    let tenant = customer_repo.seed("Acme", "acme@example.com");
    deployment_repo.seed_with_failure_reason(
        tenant.id,
        "node-engine-health-failed",
        "us-east-1",
        "t4g.small",
        "aws",
        "failed",
    ...
    let arr = json.as_array().expect("response should be an array");
    assert_eq!(arr.len(), 1);
    assert_eq!(arr[0]["node_id"], "node-engine-health-failed");
    assert_eq!(
        arr[0]["failure_reason"], "engine_health_check_failed",
        "admin deployment response must expose engine health failure_reason"
    );
}
...
#[tokio::test]
async fn update_deployment_clears_failure_reason_when_status_leaves_failed() {
    let customer_repo = crate::common::mock_repo();
    let deployment_repo = crate::common::mock_deployment_repo();
    let tenant = customer_repo.seed("Acme", "acme@example.com");
    let dep = deployment_repo.seed_with_failure_reason(
        tenant.id,
        "node-failed",
        "us-east-1",
        "t4g.small",
    ...
    assert_eq!(json["status"], "running");
    assert!(
        json["failure_reason"].is_null(),
        "recovering a failed deployment must clear the old failure_reason"
    );
}
...
async fn terminate_deployment_already_terminated_returns_404() {
    ...
    let app = crate::common::test_app_with_repos(customer_repo.clone(), deployment_repo.clone());
    ...
    let app2 = crate::common::test_app_with_repos(customer_repo, deployment_repo);
