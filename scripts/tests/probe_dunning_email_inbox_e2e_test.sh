...
assert_probe_json_result() {
    ...
    assert_valid_json "$final_line" "$msg_prefix should emit exactly one parseable final JSON result line"
    assert_eq "$(validation_json_get_field "$final_line" "result")" "$expected_result" "$msg_prefix should emit expected result"
    assert_eq "$(validation_json_get_field "$final_line" "classification")" "$expected_classification" "$msg_prefix should emit expected classification"
}
...
test_validator_failure_bubbles_owner_classification() {
    ...
    assert_probe_json_result "failed" "billing_run_no_created_invoices" "validator failure"
}
...
test_missing_inbound_scope_artifact_fails_closed() {
    ...
    assert_probe_json_result "failed" "inbound_scope_artifact_missing" "missing inbound scope artifact"
}
...
test_missing_region_in_scope_file_is_precondition_failure() {
    ...
    assert_probe_json_result "blocked" "ses_region_missing" "missing region"
}
...
test_success_emits_terminus_with_hosted_invoice_url() {
    ...
    assert_probe_json_result "passed" "dunning_delivery_verified" "successful probe"
}
...
test_runtime_failure_when_no_hosted_invoice_url_found() {
    ...
    assert_probe_json_result "failed" "hosted_invoice_url_missing" "missing hosted invoice URL"
}
