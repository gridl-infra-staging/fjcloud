...
emit_skip() {
    ...
    CANARY_SKIP_REASON="$reason"
    CANARY_SKIP_DETAIL="$detail"

    if [ -n "$detail" ]; then
        echo "SKIPPED: ${reason}: ${detail}"
    else
        echo "SKIPPED: ${reason}"
    fi

    python3 - "$reason" "$detail" "$TEST_INBOX_PREREQ_SKIP_EXIT_CODE" <<'PY'
import json
import sys

reason, detail, exit_code = sys.argv[1:]
print(json.dumps({
    "status": "skip",
...
precheck_canary_probe_env() {
    ...
    case "$prereq_rc" in
        0)
            return 0
            ;;
        "$TEST_INBOX_PREREQ_SKIP_EXIT_CODE")
            reason="${prereq_output%%:*}"
            detail="${prereq_output#*: }"
            emit_skip "$reason" "$detail"
...
main() {
    ...
    if [ "$CANARY_TEARDOWN_ONLY" -eq 0 ]; then
        precheck_canary_probe_env || prereq_rc=$?
    ...
    if [ "$CANARY_PROBE_ONLY" -eq 1 ] && [[ "$API_URL" != *"staging."* ]]; then
        echo "API_URL must target staging when --probe-only is set (got ${API_URL})" >&2; return 1
    fi
