...
test_preflight_rejects_path_fallback_after_selected_source_failure() {
    ...
    assert_contains "$output" "FAIL: Flapjack binary provenance check failed" \
        "preflight should reject a failed selected source even when a pinned PATH release exists"
    assert_not_contains "$output" "OK: Flapjack binary provenance legacy-release" \
        "preflight must not replace the selected source with a PATH release"
}
...
'
    ...
    assert_contains "$output" "OK: Flapjack binary provenance source-build:" \
        "preflight should surface helper source-build provenance for selected source"
...
test_admin_key_is_optional_when_playwright_owns_web_server() {
    ...
    case "$arg" in
        http://localhost:*/version)
            printf '\''{"capabilities":["preview_events_v1"]}'\''
            exit 0
            ;;
...
test_api_missing_preview_events_capability_fails_before_browser_auth_probe() {
    local tmp_dir
    tmp_dir=$(mktemp -d)
    backup_preflight_env_files "$tmp_dir"
    trap 'restore_preflight_env_files "'"$tmp_dir"'"; rm -rf "'"$tmp_dir"'"; trap - RETURN' RETURN
...
EOF
    rm -f "$REPO_ROOT/web/.env.local"
    ensure_repo_vite_runtime_stub
    local stale_api_curl_body='for arg in "$@"; do case "$arg" in
        http://localhost:*/health) exit 0 ;;
        http://localhost:*/version) printf '\''{"dev_sha":"local-dev"}'\''; exit 0 ;;
        http://localhost:*/auth/login) echo "AUTH_PROBE_SHOULD_NOT_RUN" >&2; exit 0 ;;
    esac; done; exit 1'
    local output
    output=$(run_preflight_isolated_with_curl_body "$tmp_dir" "$stale_api_curl_body")
    assert_contains "$output" "preview_events_v1" "preflight names the missing API capability"
    assert_contains "$output" "restart scripts/api-dev.sh" "preflight provides stale-API remediation"
    assert_not_contains "$output" "AUTH_PROBE_SHOULD_NOT_RUN" "preflight stops before browser auth for stale API"
    assert_not_contains "$output" "PREFLIGHT PASSED" "preflight rejects a live incompatible API"
}
