...
test_gate_runs_cargo_rust_tests_as_composite_check() {
    ...
    assert_contains "$cargo_invocation" "args=test -p api --test platform integration_metering_pipeline_test:: -- --test-threads=1" \
        "rust validation should run the platform grouped binary scoped to the integration_metering_pipeline_test module"
