...
# Regression: a pk_live_ publishable key paired with a test-mode secret key must
# fail closed BEFORE launch, not silently ship a broken browser Payment Element
# (the pk is served to Stripe.js, which rejects a test-mode SetupIntent secret
# under a live key). The Stripe balance call is mocked healthy (200) so the
# secret-key live check passes and the publishable-key mode guard is what trips.
test_api_dev_live_stripe_opt_in_rejects_publishable_key_mode_mismatch() {
    local tmp_dir
    tmp_dir=$(mktemp -d)
    trap 'restore_repo_path "'"$REPO_ROOT/.env.local"'" "${API_DEV_ENV_BACKUP:-}"; rm -rf "'"$tmp_dir"'"' RETURN
...
STRIPE_SECRET_KEY=sk_test_mode_match_secret
STRIPE_PUBLISHABLE_KEY=pk_live_mode_mismatch_publishable
EOF

    mkdir -p "$tmp_dir/bin"
    local cargo_log="$tmp_dir/cargo.log"
    write_mock_script "$tmp_dir/bin/cargo" '
echo "cargo should not run when publishable-key mode mismatches" >> "'"$cargo_log"'"
exit 0'
    write_mock_lsof_reports_free "$tmp_dir/bin/lsof"
    write_mock_script "$tmp_dir/bin/curl" '
printf "%s\n" "{\"object\":\"balance\",\"available\":[]}"
...
exit 0'
    ...
    assert_eq "$exit_code" "1" \
        "api-dev should fail fast when the publishable key is live-mode but the secret key is test-mode"
    assert_contains "$output" "STRIPE_PUBLISHABLE_KEY mode mismatch" \
        "api-dev should name the publishable/secret key mode mismatch"

    local cargo_calls
    cargo_calls=$(cat "$cargo_log" 2>/dev/null || true)
    assert_eq "$cargo_calls" "" \
        "api-dev should not invoke cargo when the publishable-key mode guard trips"
}
