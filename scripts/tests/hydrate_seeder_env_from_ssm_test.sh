...
case "$name" in
    ...
    /fjcloud/staging/ses_region)             echo "us-east-1" ;;
...
test_exports_full_staging_set() {
    ...
    assert_contains "$out" "export SES_REGION=us-east-1" "SES_REGION export missing — required by SES clickthrough and bounce/complaint probes"
