...
ADMIN_KEY=${admin_key}
AWS_ACCESS_KEY_ID=secret-file-access-key
AWS_SECRET_ACCESS_KEY=secret-file-secret-key
AWS_DEFAULT_REGION=us-west-2
EOF_SECRET
}

make_prod_secret_file() {
    local path="$1"
    local admin_key="$2"
...
mock_staging_hydrator_aws_body() {
    cat <<'MOCK'
set -euo pipefail
: "${CUSTOMER_BROADCAST_TEST_AWS_LOG:?missing CUSTOMER_BROADCAST_TEST_AWS_LOG}"
{
    printf '%s\n' "$*"
    printf 'aws_access_key_id=%s\n' "${AWS_ACCESS_KEY_ID:-}"
    printf 'aws_default_region=%s\n' "${AWS_DEFAULT_REGION:-}"
} >> "$CUSTOMER_BROADCAST_TEST_AWS_LOG"
...
case "$parameter_name" in
    /fjcloud/staging/admin_key) echo "resolved-staging-admin-key" ;;
    /fjcloud/staging/database_url) echo "postgres://staging.example/fjcloud" ;;
    /fjcloud/staging/dns_domain) echo "flapjack.foo" ;;
    /fjcloud/staging/stripe_secret_key) echo "sk_test_staging" ;;
    /fjcloud/staging/ses_from_address) echo "system@flapjack.foo" ;;
    /fjcloud/staging/stripe_webhook_secret) echo "whsec_staging" ;;
    *)
...
mock_capture_curl_body() {
    cat <<'MOCK'
set -euo pipefail
: "${CUSTOMER_BROADCAST_TEST_CURL_LOG:?missing CUSTOMER_BROADCAST_TEST_CURL_LOG}"
...
{
    printf 'url=%s\n' "$request_url"
    printf 'x_admin_key=%s\n' "$admin_header"
    printf 'payload=%s\n' "$payload"
} > "$CUSTOMER_BROADCAST_TEST_CURL_LOG"

printf '{"accepted":true}\n200'
MOCK
}

setup_staging_hydration_mocks() {
    local tmp_dir="$1"

    mkdir -p "$tmp_dir/bin"
    write_mock_script "$tmp_dir/bin/aws" "$(mock_staging_hydrator_aws_body)"
...
test_staging_file_admin_key_uses_hydrated_deployed_value() {
    ...
    tmp_dir="$(mktemp -d)"
    setup_staging_hydration_mocks "$tmp_dir"
    secret_file="$tmp_dir/.env.secret"
    make_staging_secret_file "$secret_file" "stale-file-admin-key"

    output="$(
        env -u API_URL -u ADMIN_KEY \
            PATH="$tmp_dir/bin:$PATH" \
            AWS_ACCESS_KEY_ID="ambient-invalid-access" \
            AWS_SECRET_ACCESS_KEY="ambient-invalid-secret" \
            AWS_DEFAULT_REGION="us-east-1" \
            CUSTOMER_BROADCAST_TEST_AWS_LOG="$tmp_dir/aws.log" \
            CUSTOMER_BROADCAST_TEST_CURL_LOG="$tmp_dir/curl.log" \
            FJCLOUD_SECRET_FILE="$secret_file" \
            bash "$REPO_ROOT/scripts/customer_broadcast.sh" \
            --subject "Staging admin hydration" \
            --text-body "body" \
            --dry-run 2>&1
    )" || exit_code=$?
    curl_log="$(cat "$tmp_dir/curl.log" 2>/dev/null || true)"

    assert_eq "$exit_code" "0" "staging file-provided admin key should hydrate before request"
    assert_contains "$output" '"accepted":true' "hydrated staging request should print server response"
    assert_contains "$(cat "$tmp_dir/aws.log" 2>/dev/null || true)" "/fjcloud/staging/admin_key" "staging hydration should use canonical SSM admin-key owner"
    assert_contains "$(cat "$tmp_dir/aws.log" 2>/dev/null || true)" "aws_access_key_id=secret-file-access-key" "staging hydration should use AWS credentials from the configured secret file"
    assert_contains "$(cat "$tmp_dir/aws.log" 2>/dev/null || true)" "aws_default_region=us-west-2" "staging hydration should use AWS region from the configured secret file"
    assert_contains "$curl_log" "url=https://api.staging.flapjack.foo/admin/broadcast" "request should use canonical staging API URL from the hydrator"
    assert_contains "$curl_log" "x_admin_key=resolved-staging-admin-key" "request header should use hydrated admin key"
    assert_not_contains "$curl_log" "stale-file-admin-key" "request header must not use stale file-provided admin key"
...
test_prod_file_admin_key_does_not_use_staging_hydration() {
    ...
    setup_staging_hydration_mocks "$tmp_dir"
    secret_file="$tmp_dir/.env.secret"
    make_prod_secret_file "$secret_file" "prod-file-admin-key"
    ...
    output="$(
        env -u API_URL -u ADMIN_KEY \
            ...
            FJCLOUD_SECRET_FILE="$secret_file" \
            bash "$REPO_ROOT/scripts/customer_broadcast.sh" \
            --subject "Production host stays production" \
            --text-body "body" \
            --dry-run 2>&1
    )" || exit_code=$?
    curl_log="$(cat "$tmp_dir/curl.log" 2>/dev/null || true)"
    aws_log="$(cat "$tmp_dir/aws.log" 2>/dev/null || true)"

    assert_eq "$exit_code" "0" "prod file-provided admin key should preserve wrapper success"
    assert_contains "$output" '"accepted":true' "prod-host request should print server response"
    assert_contains "$curl_log" "url=https://api.flapjack.foo/admin/broadcast" "prod API host must not be rewritten to staging"
    assert_contains "$curl_log" "x_admin_key=prod-file-admin-key" "prod API host must use the file-provided prod admin key"
    assert_eq "$aws_log" "" "prod API host must not invoke staging hydrator"
...
test_shared_dev_secret_file_hydrates_staging_for_dry_run() {
    ...
    tmp_dir="$(mktemp -d)"
    setup_staging_hydration_mocks "$tmp_dir"
    secret_file="$tmp_dir/gridl-infra-dev/fjcloud_dev/.secret/.env.secret"
    make_shared_dev_secret_file "$secret_file" "stale-shared-dev-admin-key"
    ...
    output="$(
        env -u API_URL -u ADMIN_KEY \
            ...
            FJCLOUD_SECRET_FILE="$secret_file" \
            bash "$REPO_ROOT/scripts/customer_broadcast.sh" \
            --subject "Shared dev secret staging proof" \
            --text-body "body" \
            --dry-run 2>&1
    )" || exit_code=$?
    curl_log="$(cat "$tmp_dir/curl.log" 2>/dev/null || true)"

    assert_eq "$exit_code" "0" "shared dev secret dry-run should hydrate staging before request"
    assert_contains "$output" '"accepted":true' "shared dev secret dry-run should print server response"
    assert_contains "$(cat "$tmp_dir/aws.log" 2>/dev/null || true)" "/fjcloud/staging/admin_key" "shared dev secret dry-run should use canonical SSM admin-key owner"
    assert_contains "$curl_log" "url=https://api.staging.flapjack.foo/admin/broadcast" "shared dev secret dry-run should use canonical staging API URL"
    assert_contains "$curl_log" "x_admin_key=resolved-staging-admin-key" "shared dev secret dry-run should use hydrated admin key"
    assert_not_contains "$curl_log" "stale-shared-dev-admin-key" "shared dev secret dry-run must not use stale file-provided admin key"
...
test_explicit_admin_key_precedence_skips_staging_hydration() {
    ...
    output="$(
        env -u API_URL ADMIN_KEY="explicit-admin-key" \
            PATH="$tmp_dir/bin:$PATH" \
            CUSTOMER_BROADCAST_TEST_AWS_LOG="$tmp_dir/aws.log" \
            CUSTOMER_BROADCAST_TEST_CURL_LOG="$tmp_dir/curl.log" \
            FJCLOUD_SECRET_FILE="$secret_file" \
            bash "$REPO_ROOT/scripts/customer_broadcast.sh" \
            --subject "Explicit admin precedence" \
            --text-body "body" \
            --dry-run 2>&1
    )" || exit_code=$?
    curl_log="$(cat "$tmp_dir/curl.log" 2>/dev/null || true)"
    ...
    assert_eq "$exit_code" "0" "explicit admin key should preserve wrapper success"
    assert_contains "$output" '"accepted":true' "explicit-key request should print server response"
    assert_contains "$curl_log" "x_admin_key=explicit-admin-key" "request header should preserve explicit admin key"
    assert_eq "$aws_log" "" "explicit admin key should not invoke staging hydrator"
