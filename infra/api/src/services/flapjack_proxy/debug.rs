...
impl FlapjackProxy {
    /// POST /1/events with a route-owned, tenant-scoped preview event payload.
    pub async fn post_preview_event(
        ...
        let url = format!("{flapjack_url}/1/events");
        let resp = self
            .send_authenticated_request(reqwest::Method::POST, url, api_key, Some(event))
            .await?;
        Self::check_response_status(resp.status, &resp.body)?;
        Self::parse_json_response(&resp.body, "failed to parse preview event response")
    }
