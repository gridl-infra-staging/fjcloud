...
echo ""
echo "--- API panic paging alarm ---"
assert_contains_active "$monitor_main_file" 'resource "aws_cloudwatch_metric_alarm" "api_panics_high"' "API panic paging alarm resource exists"
assert_contains_active "$monitor_main_file" 'alarm_name[[:space:]]*=[[:space:]]*"fjcloud-\$\{var\.env\}-api-panics-high"' "API panic paging alarm name follows naming convention"
assert_resource_block_contains "$monitor_main_file" "aws_cloudwatch_metric_alarm" "api_panics_high" 'metric_name[[:space:]]*=[[:space:]]*"PanicsPerPeriod"' "API panic paging alarm watches PanicsPerPeriod"
assert_resource_block_contains "$monitor_main_file" "aws_cloudwatch_metric_alarm" "api_panics_high" 'namespace[[:space:]]*=[[:space:]]*"fjcloud/api"' "API panic paging alarm uses fjcloud/api namespace"
assert_resource_block_contains "$monitor_main_file" "aws_cloudwatch_metric_alarm" "api_panics_high" 'statistic[[:space:]]*=[[:space:]]*"Sum"' "API panic paging alarm sums per-period panics"
assert_resource_block_contains "$monitor_main_file" "aws_cloudwatch_metric_alarm" "api_panics_high" 'period[[:space:]]*=[[:space:]]*60' "API panic paging alarm uses one-minute periods"
assert_resource_block_contains "$monitor_main_file" "aws_cloudwatch_metric_alarm" "api_panics_high" 'threshold[[:space:]]*=[[:space:]]*0' "API panic paging alarm threshold is zero"
assert_resource_block_contains "$monitor_main_file" "aws_cloudwatch_metric_alarm" "api_panics_high" 'comparison_operator[[:space:]]*=[[:space:]]*"GreaterThanThreshold"' "API panic paging alarm fires above zero"
assert_resource_block_contains "$monitor_main_file" "aws_cloudwatch_metric_alarm" "api_panics_high" 'evaluation_periods[[:space:]]*=[[:space:]]*1' "API panic paging alarm evaluates one period"
assert_resource_block_contains "$monitor_main_file" "aws_cloudwatch_metric_alarm" "api_panics_high" 'datapoints_to_alarm[[:space:]]*=[[:space:]]*1' "API panic paging alarm pages on one breaching datapoint"
assert_resource_block_contains "$monitor_main_file" "aws_cloudwatch_metric_alarm" "api_panics_high" 'treat_missing_data[[:space:]]*=[[:space:]]*"notBreaching"' "API panic paging alarm treats missing panic metrics as non-breaching"
assert_resource_block_contains "$monitor_main_file" "aws_cloudwatch_metric_alarm" "api_panics_high" 'Env[[:space:]]*=[[:space:]]*var\.env' "API panic paging alarm dimensions include canonical Env"
assert_resource_block_contains "$monitor_main_file" "aws_cloudwatch_metric_alarm" "api_panics_high" 'alarm_actions[[:space:]]*=[[:space:]]*\[aws_sns_topic\.alerts\.arn\]' "API panic paging alarm wires alarm_actions to SNS topic"
assert_resource_block_contains "$monitor_main_file" "aws_cloudwatch_metric_alarm" "api_panics_high" 'ok_actions[[:space:]]*=[[:space:]]*\[aws_sns_topic\.alerts\.arn\]' "API panic paging alarm wires ok_actions to SNS topic"
