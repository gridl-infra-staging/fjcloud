<script lang="ts">
	...
	function parseCountryCounts(
		payload: AnalyticsCountriesResponse | null | undefined
	): CountryCountByCode | null {
		if (!payload || typeof payload !== 'object') return null;
		if (!('countries' in payload) || !payload.countries || typeof payload.countries !== 'object') {
			return null;
		}
	...
	function activateCountryDrilldown(countryCode: string) {
		selectedCountryCode = countryCode.toUpperCase();
	...
	$effect(() => {
		void page.state;
		selectedCountryCode = (page.url.searchParams.get('country') ?? '').toUpperCase();
	});
</script>
	...
	use:enhance={({ formData }) => {
		...
		return async ({ result }) => {
			...
			if (result.type === 'success') {
				const parsedCountries = parseCountryCounts(readAnalyticsCountriesPayload(result.data));
				if (parsedCountries === null) {
					countriesError = 'Failed to load geography analytics';
					countryCounts = {};
					return;
				}
				countriesError = '';
				countryCounts = parsedCountries;
				return;
			}
...
<section
	...
	data-testid="analytics-subtab-panel-geography"
>
	...
	{#if selectedCountryCode}
		<div class="space-y-4" data-testid="geo-country-detail">
			...
			<p
				class="rounded-md border border-flapjack-ink/20 bg-flapjack-cream/70 p-4 text-sm text-flapjack-ink/80"
			>
	...
	{:else}
		...
		{:else}
			...
			<div
				...
				data-testid="geo-countries-table"
			>
