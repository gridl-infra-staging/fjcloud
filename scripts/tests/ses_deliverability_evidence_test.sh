...
setup_workspace() {
    ...
    mkdir -p "$TEST_WORKSPACE/infra/api/tests/integration"
    cp "$REPO_ROOT/infra/api/tests/integration/email_test.rs" "$TEST_WORKSPACE/infra/api/tests/integration/email_test.rs"
...
test_live_send_delegation_to_canonical_ignored_test() {
    ...
    assert_contains "$cargo_line" "test -p api --test platform -- --exact email_test::ses_live_smoke_sends_verification_email --ignored" "wrapper must target canonical ignored live smoke seam"
