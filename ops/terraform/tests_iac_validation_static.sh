...
assert_file_contains "$bootstrap_script" '/var/lib/flapjack/data/.admin_key' "bootstrap.sh converges Flapjack persisted admin key from the node API key"
assert_file_contains "$bootstrap_script" 'FLAPJACK_APPLICATION_ID=flapjack' "bootstrap.sh writes Flapjack application id for metering auth"
assert_file_contains "$bootstrap_script" 'API_BASE_URL="https://api\.staging\.\$DNS_DOMAIN"' "bootstrap.sh points staging metering agents at the staging control-plane API"
assert_file_contains "$bootstrap_script" 'TENANT_MAP_URL=\$API_BASE_URL/internal/tenant-map' "bootstrap.sh derives tenant-map URL from the environment-aware API base"
assert_file_contains "$bootstrap_script" 'COLD_STORAGE_USAGE_URL=\$API_BASE_URL/internal/cold-storage-usage' "bootstrap.sh derives cold-storage URL from the environment-aware API base"
...
assert_file_contains "$cloud_init_file" '/var/lib/flapjack/data/.admin_key' "cloud_init.rs converges Flapjack persisted admin key from the node API key"
assert_file_contains "$cloud_init_file" 'FLAPJACK_APPLICATION_ID=flapjack' "cloud_init.rs writes Flapjack application id for metering auth"
assert_file_contains "$cloud_init_file" 'API_BASE_URL="https://api\.staging\.\$DNS_DOMAIN"' "cloud_init.rs points staging metering agents at the staging control-plane API"
assert_file_contains "$cloud_init_file" 'TENANT_MAP_URL=\$API_BASE_URL/internal/tenant-map' "cloud_init.rs derives tenant-map URL from the environment-aware API base"
assert_file_contains "$cloud_init_file" 'COLD_STORAGE_USAGE_URL=\$API_BASE_URL/internal/cold-storage-usage' "cloud_init.rs derives cold-storage URL from the environment-aware API base"
