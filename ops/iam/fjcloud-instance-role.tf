  ...
  policy = jsonencode({
    ...
    Statement = [{
      ...
      Resource = [
        ...
        "arn:aws:s3:::fjcloud-releases-*/*",
        "arn:aws:s3:::flapjack-cloud-releases",
        "arn:aws:s3:::flapjack-cloud-releases/*"
      ]
    }]
  })
...
# --------------------------------------------------------------------------
# Policy — CloudWatch Logs read for SES send-events evidence
# --------------------------------------------------------------------------

resource "aws_iam_role_policy" "fjcloud_ses_send_events_read" {
  name = "fjcloud-ses-send-events-read"
  role = aws_iam_role.fjcloud_instance.id

  policy = jsonencode({
    Version = "2012-10-17"
    # CloudWatch Logs only supports "*" for DescribeLogGroups, while
    # FilterLogEvents authorizes against the log-group ARN with trailing :*
    # and GetLogEvents authorizes against log-stream ARNs under that group.
    Statement = [
      {
        Effect   = "Allow"
      ...
      },
      {
        ...
        Resource = "arn:aws:logs:us-east-1:${data.aws_caller_identity.current.account_id}:log-group:/fjcloud/staging/ses/send-events:*"
      },
      {
        Effect   = "Allow"
        Action   = ["logs:GetLogEvents"]
        Resource = "arn:aws:logs:us-east-1:${data.aws_caller_identity.current.account_id}:log-group:/fjcloud/staging/ses/send-events:log-stream:*"
      },
    ]
  })
}
