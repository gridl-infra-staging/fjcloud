...
- Current launch blockers and prioritization are tracked in `README.md`,
  `PROJECT_OVERVIEW.md`, and `ROADMAP.md`.
...
- **Current status authority**: See `README.md`, `PROJECT_OVERVIEW.md`, and `ROADMAP.md` for canonical blocker and priority state.
...
For current blocker and prioritization status, see `README.md`, `PROJECT_OVERVIEW.md`, and `ROADMAP.md`.
...
- `scripts/set_status.sh` writes `SERVICE_STATUS`, `SERVICE_STATUS_UPDATED`, and
  `SERVICE_STATUS_MESSAGE` into `[vars]`, `[env.production.vars]`, and
  `[env.preview.vars]` in `web/wrangler.toml`.
- `web/src/routes/status/+page.server.ts` reads those vars at runtime and returns
  `StatusRouteData` (shape owned by `web/src/routes/status/status_contract.ts`).
- `scripts/deploy_status.sh` remains the deployed-version readback seam.
- Canonical operator procedures are:
  - `docs/runbooks/incident-response.md`
  - `docs/checklists/PAID_BETA_LAUNCH_CHECKLIST.md`
  - `LAUNCH.md`
- This section is a dated pointer only and is not a second procedure.
