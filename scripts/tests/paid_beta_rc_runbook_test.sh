...
test_runbook_documents_canonical_coordinator_commands() {
    ...
    assert_contains "$content" "--staging-smoke-api-ami-id=<ami-id>" "runbook should document staging API smoke AMI flag"
    assert_contains "$content" "--staging-smoke-flapjack-ami-id=<ami-id>" "runbook should document staging Flapjack smoke AMI flag"
    assert_not_contains "$content" "--staging-smoke-ami-id=<ami-id>" "runbook should not document removed coupled staging smoke AMI flag"
}
...
test_runbook_documents_operator_facing_json_contract() {
    ...
    assert_contains "$content" "summary.json" "runbook should describe canonical summary.json artifact"
    ...
    assert_contains "$content" "external_secret_missing" "runbook should describe external_secret_missing status"
    assert_contains "$content" "live_evidence_gap" "runbook should describe live_evidence_gap status"
    assert_contains "$content" "skipped" "runbook should describe skipped status"
    assert_contains "$content" 'legacy `blocked` is removed' "runbook should state blocked was removed"
...
test_runbook_links_delegated_proof_owners() {
    ...
    assert_contains "$content" "docs/runbooks/local-dev.md" "runbook should link local signoff owner runbook"
    assert_contains "$content" "docs/checklists/LOCAL_SIGNOFF_CHECKLIST.md" "runbook should link local signoff checklist"
...
test_runbook_documents_blocker_reasons_and_ready_false_rule() {
    ...
    assert_contains "$content" "ready=false" "runbook should state required proof gaps force ready=false"
    assert_contains "$content" "Critical browser rule" "runbook should document critical browser skip promotion"
    assert_contains "$content" "critical_surface_skipped" "runbook should document critical browser skip promoted reason"
...
test_runbook_discoverability_and_evidence_safety() {
    local runbook_content local_runbook_content
    runbook_content="$(cat "$RUNBOOK_PATH")"
    local_runbook_content="$(cat "$LOCAL_RUNBOOK_DOC")"

    assert_contains "$local_runbook_content" "local-signoff.sh" "local-dev runbook should document local signoff"
