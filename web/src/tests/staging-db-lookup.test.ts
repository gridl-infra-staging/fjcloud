...
describe('staging DB lookup helper (LB-2/LB-3)', () => {
	...
	describe('buildVerificationTokenLookupSql', () => {
		...
		it('canonicalizes mixed-case fixture emails to match API-persisted lowercase rows', () => {
			expect(
				buildVerificationTokenLookupSql('Stage3-Proof-20260605T205003Z@E2E.Griddle.Test')
			).toBe(
				"SELECT email_verify_token FROM customers WHERE email = 'stage3-proof-20260605t205003z@e2e.griddle.test'"
			);
		});
	...
	describe('buildCustomerStatusLookupSql (Stage 2 shared seam)', () => {
		...
		it('canonicalizes mixed-case emails before building the customer-status lookup', () => {
			const sql = buildCustomerStatusLookupSql('Stage3-Proof-20260605T205003Z@E2E.Griddle.Test');
			expect(sql).toContain("WHERE email = 'stage3-proof-20260605t205003z@e2e.griddle.test'");
		});
