...
assert_file_contains "docs/launch_verification_matrix.md" 'bundle age reaches 7 days' "matrix section 4 freshness rule uses a 7-day rerun threshold"
assert_file_contains "docs/launch_verification_matrix.md" '7 days old or older' "matrix maintenance contract matches the 7-day freshness threshold"
assert_file_not_contains "docs/launch_verification_matrix.md" 'Stale evidence \(>14 days\)' "matrix maintenance contract no longer carries the stale 14-day threshold"
...
assert_file_contains "$runbook_file" 'PGPASSFILE=' "runbook verification example uses PGPASSFILE credentials instead of URI credentials"
assert_file_contains "$runbook_file" 'postgres://<user>@<restored-target-endpoint>:5432/fjcloud' "runbook verification URI omits password and points directly at restored target"
assert_file_not_contains "$runbook_file" '/fjcloud/staging/database_url' "runbook verification example does not fetch canonical DB URL from SSM for URI rewrite"
assert_file_not_contains "$runbook_file" 'RESTORED_HOST=<restored-target-endpoint>' "runbook verification example does not rewrite a password-bearing DATABASE_URL"
...
assert_file_contains "$runbook_file" '2026-04-23' "runbook pins the canonical failed staging restore date"
...
assert_file_contains "$runbook_file" 'docs/runbooks/evidence/database-recovery/20260423T201333Z_staging_restore_verification.txt' "runbook records the captured RDS restore verification evidence path"
assert_file_contains "$staging_evidence_file" 'docs/runbooks/evidence/database-recovery/20260423T201333Z_staging_restore_verification.txt' "staging evidence records the captured RDS restore verification evidence path"
...
assert_file_contains "$script_file" 'redact_rds_endpoint()' "wrapper defines redact_rds_endpoint function"
assert_file_contains "$script_file" 'rds\.amazonaws\.com' "endpoint redaction targets standard RDS endpoint pattern"
assert_file_contains "$script_file" '"failed"' "poll_target_available handles terminal status failed"
assert_file_contains "$script_file" '"incompatible-restore"' "poll_target_available handles terminal status incompatible-restore"
assert_file_contains "$script_file" 'terminal status' "poll_target_available records terminal status in reason"
assert_file_contains "$script_file" 'STATUS == "blocked" || .* == "fail"' "write_summary_artifact gates reason serialization on blocked/fail status"
