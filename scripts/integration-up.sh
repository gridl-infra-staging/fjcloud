...
# shellcheck source=lib/integration_db_access.sh
...
PID_DIR="${FJCLOUD_INTEGRATION_PID_DIR:-$REPO_ROOT/.integration}"
...
require_absolute_executable() {
    ...
    case "$path" in
        /*) ;;
        *) die_with_reason "prerequisite_missing" "$label must be an absolute executable path" ;;
    esac
...
# 2. Build binaries
# ---------------------------------------------------------------------------
API_BIN="${FJCLOUD_INTEGRATION_API_BINARY:-}"
if [ -n "$API_BIN" ]; then
    require_absolute_executable "FJCLOUD_INTEGRATION_API_BINARY" "$API_BIN"
else
    log "Building fjcloud API and metering-agent..."
    _build_log="$(mktemp)"
    if ! (cd "$INFRA_DIR" && cargo build -p api -p metering-agent >"$_build_log" 2>&1); then
        tail -20 "$_build_log" >&2 || true
        rm -f "$_build_log"
        die_with_reason "build_failed" "cargo build failed for fjcloud API and metering-agent"
    fi
    tail -3 "$_build_log"
    rm -f "$_build_log"
    API_BIN="$INFRA_DIR/target/debug/fjcloud-api"
...
if [ "${FJCLOUD_INTEGRATION_SKIP_METERING_AGENT:-}" != "1" ]; then
    if [ -n "${FJCLOUD_INTEGRATION_API_BINARY:-}" ]; then
        log "Building metering-agent..."
        _build_log="$(mktemp)"
        if ! (cd "$INFRA_DIR" && cargo build -p metering-agent >"$_build_log" 2>&1); then
            tail -20 "$_build_log" >&2 || true
            rm -f "$_build_log"
            die_with_reason "build_failed" "cargo build failed for metering-agent"
        fi
        tail -3 "$_build_log"
        rm -f "$_build_log"
    fi
...
FLAPJACK_BIN="${FJCLOUD_INTEGRATION_ENGINE_BINARY:-}"
if [ -n "$FLAPJACK_BIN" ]; then
    require_absolute_executable "FJCLOUD_INTEGRATION_ENGINE_BINARY" "$FLAPJACK_BIN"
else
    ...
    if [ "$FLAPJACK_RESOLUTION_STATUS" -eq "$FJCLOUD_FLAPJACK_SOURCE_RESOLUTION_FAILURE_STATUS" ]; then
        die_with_reason "flapjack_source_resolution_failed" \
            "selected FLAPJACK_DEV_DIR source build or provenance validation failed: $FLAPJACK_DEV_DIR"
    fi
    if [ -n "$FLAPJACK_BIN" ]; then
...
INTEGRATION_INTERNAL_AUTH_TOKEN="${INTEGRATION_INTERNAL_AUTH_TOKEN:-$(generate_hex_secret 24)}"
INTEGRATION_AUTH_RATE_LIMIT_RPM="${AUTH_RATE_LIMIT_RPM:-120}"
DATABASE_URL="$INTEGRATION_RUNTIME_DB_URL" \
    ...
    FJCLOUD_FLAPJACK_VERSION="$INTEGRATION_FJCLOUD_FLAPJACK_VERSION" \
    ...
    FLAPJACK_ADMIN_KEY="$INTEGRATION_FLAPJACK_ADMIN_KEY" \
    AUTH_RATE_LIMIT_RPM="$INTEGRATION_AUTH_RATE_LIMIT_RPM" \
...
if [ -n "$FLAPJACK_BIN" ] && [ -x "$FLAPJACK_BIN" ] && [ "${FJCLOUD_INTEGRATION_SKIP_METERING_AGENT:-}" != "1" ]; then
