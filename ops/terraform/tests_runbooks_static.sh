...
assert_file_contains "$database_recovery_runbook" 'SELECT COUNT\(\*\) FROM tenants;' "database recovery: tenants sanity query documented"
assert_file_contains "$database_recovery_runbook" 'SELECT COUNT\(\*\) FROM invoices WHERE created_at > now\(\) - interval '\''7 days'\'';' "database recovery: invoice recency sanity query documented"
assert_file_contains "$database_recovery_runbook" 'SELECT COUNT\(\*\) FROM deployments WHERE status = '\''running'\'';' "database recovery: deployment sanity query documented"
assert_file_contains "$database_recovery_runbook" 'SELECT COUNT\(\*\) FROM usage_records WHERE recorded_at > now\(\) - interval '\''1 day'\'';' "database recovery: usage sanity query documented"
