...
Usage:
  ...
  staging_billing_rehearsal.sh --env-file <path> --reset-first --month YYYY-MM --confirm-live-mutation --confirm-test-tenant <uuid>
...
Options:
  ...
  --reset-first                Reset test tenant state, then run the live rehearsal.
  --confirm-test-tenant <uuid> Required with reset modes.
...
validate_parsed_args() {
    ...
    if [ -n "$BILLING_MONTH" ] && ! rc_is_valid_billing_month "$BILLING_MONTH"; then
        set_blocked_summary "billing_month_invalid" \
            "--month must use YYYY-MM format with month 01-12."
        return 1
    fi
    ...
    if [ "$RESET_TEST_STATE" -eq 1 ] && [ "$RESET_FIRST" -eq 1 ]; then
        set_blocked_summary "reset_mode_reset_first_conflict" \
            "--reset-first cannot be combined with --reset-test-state."
        return 1
    fi
    ...
    if [ "$RESET_FIRST" -eq 1 ] && [ -z "$BILLING_MONTH" ]; then
        set_blocked_summary "billing_month_required" \
            "--month is required when --reset-first is provided."
        return 1
    fi
    if [ "$RESET_FIRST" -eq 1 ] && [ "$CONFIRM_LIVE_MUTATION" -ne 1 ]; then
        set_blocked_summary "live_mutation_confirmation_required" \
            "--confirm-live-mutation is required when --reset-first is provided."
        return 1
    fi
    if { [ "$RESET_TEST_STATE" -eq 1 ] || [ "$RESET_FIRST" -eq 1 ]; } && [ -z "$CONFIRM_TEST_TENANT_ID" ]; then
        set_blocked_summary "test_tenant_confirmation_required" \
            "--confirm-test-tenant is required when reset mode is provided."
        return 1
    fi
    if [ -n "$CONFIRM_TEST_TENANT_ID" ] && [ "$RESET_TEST_STATE" -ne 1 ] && [ "$RESET_FIRST" -ne 1 ]; then
        set_blocked_summary "reset_test_state_required" \
            "--reset-test-state or --reset-first is required when --confirm-test-tenant is provided."
        return 1
    fi
    return 0
}
...
run_rehearsal_flow() {
    ...
    if ! hydrate_staging_tool_env_from_ssm staging; then
        set_blocked_summary "staging_ssm_hydration_failed" \
            "Failed to hydrate canonical staging runtime credentials from SSM."
        handle_env_parse_failure
        return 0
    fi
    derive_staging_contract_env_aliases
    CAPTURE_SUMMARY_DEPLOYABLE_CURRENCY=1
