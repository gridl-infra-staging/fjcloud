...
- [ ] Keep synthetic-traffic and billing rehearsal lanes anchored to existing owners.
      ...
      Status: The seeder is now implemented in-repo and covered by `scripts/tests/seed_synthetic_traffic_test.sh`, but its truthful live surface is still partial: execute mode supports Tenant A only and rejects B/C/all. A 2026-04-25 live rerun cleared the earlier seeded-deployment admin-key verification blocker in the SSM-backed node-secret path, but the same owner script now stops later when direct-node `GET /internal/storage` returns HTTP 403 `Invalid Application-ID or API key`. Billing rehearsal reruns remain script-owned and must use canonical evidence artifacts after that direct-node auth/env blocker is fixed.
      Supplementary dispatch prerequisite (without replacing the 403 blocker): dispatches that source only `/Users/stuart/repos/gridl-infra-dev/fjcloud_dev/.secret/.env.secret` fail seeder preflight because that file does not export `DATABASE_URL`, `API_URL`, `ADMIN_KEY`, `FLAPJACK_URL`, and `FLAPJACK_API_KEY`; canonical hydration ownership is maintained in `docs/launch/synthetic_traffic_seeder_plan.md`.
