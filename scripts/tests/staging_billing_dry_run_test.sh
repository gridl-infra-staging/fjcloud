...
test_dry_run_derives_staging_aliases_from_api_url_only() {
    # When the env file sets only API_URL (the .env.secret convention), the
    # dry-run preflight must derive STAGING_API_URL and STAGING_STRIPE_WEBHOOK_URL
    # per the hydration contract documented in
    # scripts/launch/hydrate_seeder_env_from_ssm.sh:97-98 — otherwise the
    # preflight would block at staging_api_url_missing / dns_or_cloudflare_blocked
    # despite the operator providing the canonical secret file.
    make_test_tmp_dir
    write_mock_curl 'exit 99'
    ...
    assert_eq "$RUN_EXIT_CODE" "0" "dry-run should pass when STAGING_* contract vars are derived from API_URL"
    assert_valid_json "$RUN_STDOUT" "derived-aliases dry-run output should be valid JSON"
    assert_json_bool_field "$RUN_STDOUT" "passed" "true" "derived-aliases dry-run should report passed=true"
}
...
test_explicit_env_file_overrides_stale_parent_billing_inputs() {
    make_test_tmp_dir
    write_mock_curl 'exit 99'
    local env_file="$TEST_TMP_DIR/explicit-billing.env"
    cat > "$env_file" <<'EOF'
STAGING_API_URL=https://staging-api.example.test
STAGING_STRIPE_WEBHOOK_URL=https://staging-api.example.test/webhooks/stripe
STRIPE_SECRET_KEY=sk_test_explicit_contract
STRIPE_WEBHOOK_SECRET=whsec_explicit_contract
ADMIN_KEY=explicit-admin-key
DATABASE_URL=postgres://explicit-db
INTEGRATION_DB_URL=postgres://explicit-integration-db
MAILPIT_API_URL=https://explicit-mailpit.example.test
EOF

    run_dry_run_script --check --env-file "$env_file" \
        "STAGING_API_URL=http://stale-parent.invalid" \
        "STAGING_STRIPE_WEBHOOK_URL=http://stale-parent.invalid/not-stripe" \
        "STRIPE_SECRET_KEY=sk_live_stale_parent" \
        "STRIPE_WEBHOOK_SECRET=stale-parent-webhook-secret" \
        "ADMIN_KEY=" \
        "DATABASE_URL=" \
        "INTEGRATION_DB_URL=" \
        "MAILPIT_API_URL=http://stale-parent-mailpit.invalid"

    assert_eq "$RUN_EXIT_CODE" "0" "explicit env file should override stale parent billing inputs"
    assert_valid_json "$RUN_STDOUT" "explicit env override output should be valid JSON"
    assert_json_bool_field "$RUN_STDOUT" "passed" "true" "explicit env override should report passed=true"
    assert_json_string_field "$RUN_STDOUT" "classification" "ready" "explicit env override should classify ready"
}
