...
describe('getStripe', () => {
	...
	it('clears a failed runtime bootstrap so retry can fetch and load Stripe again', async () => {
		const stripeInstance = { elements: vi.fn() };
		loadStripeMock.mockResolvedValue(stripeInstance);
		fetchMock
			...
			.mockResolvedValueOnce(
				new Response(JSON.stringify({ publishableKey: 'pk_test_runtime_retry' }), {
					status: 200,
					headers: { 'Content-Type': 'application/json' }
				})
			);

		const { getStripe, resetStripeBootstrapForRetry } = await import('./stripe');

		await expect(getStripe()).resolves.toBeNull();
		resetStripeBootstrapForRetry();
		await expect(getStripe()).resolves.toBe(stripeInstance);

		expect(fetchMock).toHaveBeenCalledTimes(2);
		expect(loadStripeMock).toHaveBeenCalledTimes(1);
		expect(loadStripeMock).toHaveBeenCalledWith('pk_test_runtime_retry');
	});
