...
use crate::services::panics::PanicsPublisher;
...
use crate::services::vm_autorepair::{
    VmAutorepairDeps, VmAutorepairReconciler, VmAutorepairSettings,
};
...
pub const VM_AUTOREPAIR_TASK_NAME: &str = "VM autorepair";
    ...
    match startup_env.ses_startup_mode() {
        SesStartupMode::Noop => {
            if let Ok(mailpit_url) = std::env::var("MAILPIT_API_URL") {
                ...
                let from_name = mailpit_from_name_from_env(startup_env);
...
fn mailpit_from_name_from_env(startup_env: &StartupEnvSnapshot) -> String {
    crate::services::email::SesConfig::from_name_from_reader(|key| {
        startup_env.env_value(key).map(str::to_string)
    })
}
...
pub fn spawn_background_tasks(
    ...
    let region_failover_monitor = Arc::new(RegionFailoverMonitor::new(
        ...
        state.index_replica_repo.clone(),
        state.alert_service.clone(),
        state.index_lifecycle_lease.clone(),
    ...
    let cold_tier_service = Arc::new(ColdTierService::new(
        ...
        ColdTierDependencies {
            ...
            node_client,
            node_secret_manager,
            lifecycle_lease: Some(state.index_lifecycle_lease.clone()),
        },
    ));
    ...
    let autorepair_http = reqwest::Client::builder()
        .timeout(Duration::from_secs(5))
        .build()?;
    let autorepair_health_client =
        Arc::new(crate::services::health_monitor::ReqwestHealthCheckClient::new(autorepair_http));
    let vm_autorepair = Arc::new(VmAutorepairReconciler::new_with_enabled_reader(
        VmAutorepairDeps {
            vm_inventory_repo: state.vm_inventory_repo.clone(),
            tenant_repo: state.tenant_repo.clone(),
            lifecycle_event_repo: state.vm_lifecycle_event_repo.clone(),
            provisioning_service: state.provisioning_service.clone(),
            health_client: autorepair_health_client,
        },
        VmAutorepairSettings::from_env()?,
        Arc::new(crate::config::parse_vm_autorepair_enabled_from_env),
    ));
    let vm_autorepair_handle = {
        let reconciler = Arc::clone(&vm_autorepair);
        let rx = shutdown_rx.clone();
        tokio::spawn(async move {
            reconciler.run(rx).await;
        })
    };
    named_handles.push((VM_AUTOREPAIR_TASK_NAME, vm_autorepair_handle));
    ...
    if startup_env.is_staging_or_production() {
        ...
        let panics_publisher = PanicsPublisher::new(
            cloudwatch_client,
            environment,
            Duration::from_secs(60),
        ...
        let rx = shutdown_rx.clone();
        let panics_handle = tokio::spawn(async move {
            panics_publisher.run(rx).await;
        });
        named_handles.push(("panics", panics_handle));
    }

    BackgroundHandles::new(named_handles, access_tracker_handle)
}
