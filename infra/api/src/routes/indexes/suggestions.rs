...
/// `POST /indexes/{name}/suggestions/build` — trigger query suggestions build.
///
/// **Auth:** JWT (`AuthenticatedTenant`).
/// Rejects cold/restoring indexes (503). Starts a query suggestions build
/// in flapjack for this index.
#[utoipa::path(
    post,
    path = "/indexes/{name}/suggestions/build",
    tag = "Suggestions",
    params(("name" = String, Path, description = "Index name")),
    responses(
        (status = 200, description = "Query suggestions build started", body = serde_json::Value),
        (status = 401, description = "Authentication required", body = ErrorResponse),
        (status = 410, description = "Index is in cold storage", body = ErrorResponse),
        (status = 503, description = "Index is restoring or endpoint not ready", body = ErrorResponse),
        (status = 404, description = "Index not found", body = ErrorResponse),
