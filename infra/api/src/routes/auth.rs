...
use crate::repos::{
    CustomerHardDeleteKind, CustomerHardDeleteOutcome, ResendPasswordResetOutcome,
...
/// Create a customer account and send the initial email verification message.
...
async fn rollback_failed_registration(
        ...
        .await
    {
        ...
        Ok(CustomerHardDeleteOutcome::NotFound | CustomerHardDeleteOutcome::NotSoftDeleted) => {
            tracing::error!(
...
/// Reissue a password-reset email when the previous reset link is missing or expired.
