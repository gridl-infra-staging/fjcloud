...
check_prerequisites() {
    ...
    if [ -n "$flapjack_bin" ] && [ -x "$flapjack_bin" ]; then
        ...
        log "prerequisite ok: flapjack_provenance=$(flapjack_source_provenance_summary)"
    else
...
init_artifact_dir() {
    ARTIFACT_DIR="$(mktemp -d "${TMPDIR:-/tmp}/fjcloud-local-signoff-XXXXXX")" || \
        die "Unable to allocate local-signoff artifact directory"
    chmod 700 "$ARTIFACT_DIR" || \
        die "Unable to secure local-signoff artifact directory"
}
...
verify_post_ha_health() {
    ...
    local region_port region port normalized_regions
    normalized_regions="${FLAPJACK_REGIONS//,/ }"
    for region_port in $normalized_regions; do
        [ -n "$region_port" ] || continue
        case "$region_port" in
            *:*) ;;
            *)
                set_post_ha_failure "$idx" "post-HA health check failed: malformed FLAPJACK_REGIONS entry '${region_port}'"
                return 1
                ;;
        esac
        region="${region_port%%:*}"
