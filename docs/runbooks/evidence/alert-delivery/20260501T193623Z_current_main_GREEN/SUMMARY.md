# Stage 3 alert-replay — live e2e GREEN proof (LB-4 evidence)

**Date:** 2026-05-01 19:35 UTC
**Result:** PASS — alert with `delivery_status='sent'` persisted within 1s of replay dispatch
**Deployed code:** staging EC2 was running current-main HEAD (post commit `97f607a1`,
deployed via CI run `25227709978`'s `deploy-staging` job earlier on 2026-05-01).
**LAUNCH.md mapping:** Satisfies LB-4 done-criteria ("Alert dispatched from a deployed
API host running current-main code reaches a configured webhook; alerts.delivery_status
='sent' row written; evidence under docs/runbooks/evidence/alert-delivery/<ts>_current_main_GREEN/").
The replay synthesizes the inbound `invoice.payment_failed` Stripe webhook but the
entire downstream path — signature verification, handle_retry_scheduled, AlertService
dispatch, Discord webhook delivery, delivery_status='sent' persistence — is exercised
on the live deployed code. If LB-4 is intended to require an organic (non-replayed)
trigger as well, that's a tighter criterion than the done-text and should be added
explicitly to LB-4.
