...
describe('Dashboard indexes card', () => {
	it('indexes card shows count of indexes', () => {
		...
		expect(
			within(card).getByText((_content, element) => {
				return element?.tagName === 'P' && /\b2\s+active indexes\b/.test(element.textContent ?? '');
			})
		).toBeInTheDocument();
	...
	it('indexes card uses the singular label for one index', () => {
		...
		expect(
			within(card).getByText((_content, element) => {
				return element?.tagName === 'P' && /\b1\s+active index\b/.test(element.textContent ?? '');
			})
		).toBeInTheDocument();
	});
	...
	it('indexes card shows region health indicators', () => {
		...
		expect(within(card).getByText(`1 ${indexStatusLabel('ready')}`)).toBeInTheDocument();
		expect(within(card).getByText(`1 ${indexStatusLabel('unhealthy')}`)).toBeInTheDocument();
		expect(within(card).getByText(`1 ${indexStatusLabel('provisioning')}`)).toBeInTheDocument();
	});
	...
	it('empty state shows onboarding CTA when no indexes', () => {
		...
		const card = screen.getByTestId('indexes-card');
		expect(card).toBeInTheDocument();
		expect(within(card).getByText(/creating an index is the next setup step/i)).toBeInTheDocument();
		...
		expect(link.getAttribute('href')).toBe('/console/onboarding');
	});
	...
	it('onboarding banner shown when onboarding incomplete', () => {
		...
		expect(banner).not.toHaveTextContent(/billing|credit card|payment/i);
	...
	it('indexes card links to /console/indexes', () => {
		render(DashboardPage, {
			data: {
				...layoutTestDefaults,
				user: null,
		...
		expect(link.getAttribute('href')).toBe('/console/indexes');
	});
	...
	it('keeps usage chart series palette pinned to explicit readable diner hex values', () => {
		expect(dashboardSource).toContain("color: '#d65479'");
		expect(dashboardSource).toContain("color: '#7b314a'");
		expect(dashboardSource).not.toContain("color: 'var(--color-flapjack-rose)'");
		expect(dashboardSource).not.toContain("color: 'var(--color-flapjack-plum)'");
	});
...
describe('Plan-aware billing prompts', () => {
	it('does not duplicate the layout-owned billing setup prompt on the dashboard page', () => {
		...
		expect(
			within(screen.getByTestId('onboarding-banner')).getByRole('link', {
				name: /continue setup/i
			})
		).toHaveAttribute('href', '/console/onboarding');
	});

	it('hides billing setup prompt for Paid plan with payment method', () => {
		const sharedWithPay = {
			billing_plan: 'shared' as const,
			free_tier_limits: null,
	...
	it('hides billing setup prompt when Paid-plan billing status is unavailable', () => {
		render(DashboardPage, {
			data: {
				...layoutTestDefaults,
				planContext: {
...
describe('Free-plan onboarding CTA', () => {
	it('keeps no-credit-card-required copy out of the dashboard onboarding banner', () => {
		render(DashboardPage, {
			data: {
				...layoutTestDefaults,
				user: null,
	...
	it('omits no-credit-card-required copy for Paid plan', () => {
