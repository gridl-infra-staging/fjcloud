...
test_rate_limited_forgot_does_not_skip_reset_wire_check() {
  local forgot_skip_refs reset_skip_refs
  forgot_skip_refs="$(grep -c 'forgot_auth_wire_skip_reason' "$CONTRACT_SCRIPT")"
  reset_skip_refs="$(grep -c 'reset_auth_wire_skip_reason' "$CONTRACT_SCRIPT")"
  if [[ "$forgot_skip_refs" -ge 2 && "$reset_skip_refs" -ge 2 ]]; then
    pass "auth-wire skip reasons are endpoint-scoped"
  else
    fail "auth-wire skip reasons must be endpoint-scoped. forgot_refs=$forgot_skip_refs reset_refs=$reset_skip_refs"
  fi
}
