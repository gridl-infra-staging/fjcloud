...
#[tokio::test]
async fn version_returns_provenance_and_capabilities() {
    ...
    assert_eq!(
        json["capabilities"],
        serde_json::json!(["preview_events_v1"]),
        "/version must advertise the API capabilities browser clients require"
    );
}
