...
initialize_frozen_legacy_safe_mirror() {
  ...
  [[ "$frozen_sha" =~ ^[0-9a-f]{40}$ ]] && return 0
  [[ "$PREV_SHA" =~ ^[0-9a-f]{40}$ ]] || return 0

  if aws ssm put-parameter \
    --region "$REGION" \
    --name "$SSM_LEGACY_SAFE_SHA" \
    --value "$PREV_SHA" \
    --type String >/dev/null 2>&1; then
    echo "==> Froze legacy Algolia-import rollback mirror at ${PREV_SHA}"
    return 0
  fi
  ...
  [[ "$frozen_sha" =~ ^[0-9a-f]{40}$ ]] || {
    echo "ERROR: could not initialize ${SSM_LEGACY_SAFE_SHA}" >&2
    return 1
  }
}
...
ROLLBACK_LIBRARY_DIR="/usr/local/lib/fjcloud"
ROLLBACK_LIBRARY_PATH="${ROLLBACK_LIBRARY_DIR}/rollback_compatibility.sh"
...
aws s3 cp "s3://${S3_BUCKET}/${S3_PREFIX}/scripts/lib/rollback_compatibility.sh" "${SCRIPTS_DIR}/rollback_compatibility.sh" --region "$REGION"
chmod +x "${SCRIPTS_DIR}/migrate.sh" "${SCRIPTS_DIR}/generate_ssm_env.sh" "${SCRIPTS_DIR}/rollback_compatibility.sh"
...
# Install the release-published rollback gate alongside the candidate. Its
# prior version is restored if the new release fails its health check.
mkdir -p "$ROLLBACK_LIBRARY_DIR"
if [[ -f "$ROLLBACK_LIBRARY_PATH" ]]; then
  cp "$ROLLBACK_LIBRARY_PATH" "${ROLLBACK_LIBRARY_PATH}.old"
fi
install -m 0755 "$SCRIPTS_DIR/rollback_compatibility.sh" "${ROLLBACK_LIBRARY_PATH}.new"
mv "${ROLLBACK_LIBRARY_PATH}.new" "$ROLLBACK_LIBRARY_PATH"
...
if [[ "$HEALTHY" == "true" ]]; then
  ...
  rm -f "${ROLLBACK_LIBRARY_PATH}.old"
  exit 0
fi
...
if [[ -f "${ROLLBACK_LIBRARY_PATH}.old" ]]; then
  mv "${ROLLBACK_LIBRARY_PATH}.old" "$ROLLBACK_LIBRARY_PATH"
else
  rm -f "$ROLLBACK_LIBRARY_PATH"
fi
systemctl restart fjcloud-api
