...
import { makeActionArgs, makeLoadArgs, setupDefaultLoadMocks } from './detail.server.test.shared';
...
describe('Index detail page server -- actions', () => {
	...
	it('redirects to login when the detail load hits an expired session', async () => {
		...
		await expect(
			load({
				...(makeLoadArgs() as Record<string, unknown>),
	...
	it('restoreIndex action starts the restore for the route index', async () => {
		restoreIndexMock.mockResolvedValue({
			restore_job_id: 'restore-123',
			status: 'pending',
			poll_url: '/indexes/products/restore-status'
		});

		const result = await actions.restoreIndex(
			makeActionArgs('restoreIndex', new FormData()) as never
		);

		expect(restoreIndexMock).toHaveBeenCalledWith('products');
		expect(result).toEqual({ restoreStarted: true });
	});
	it('restoreIndex action keeps capacity failures visible', async () => {
		restoreIndexMock.mockRejectedValue(
			new ApiRequestError(429, 'Maximum concurrent restores reached. Try again later.')
		);
	...
	it('saveSettings action calls updateIndexSettings and returns success', async () => {
		...
		const formData = new FormData();
		formData.set('settings', JSON.stringify(fullSettings));
		const result = await actions.saveSettings(makeActionArgs('saveSettings', formData) as never);
		expect(updateIndexSettingsMock).toHaveBeenCalledWith('products', fullSettings);
		expect(result).toEqual({ settingsSaved: true });
	});
	it('saveSettings action rejects non-object JSON before calling updateIndexSettings', async () => {
		const formData = new FormData();
		formData.set('settings', JSON.stringify(['searchableAttributes']));
		const result = await actions.saveSettings(makeActionArgs('saveSettings', formData) as never);
		...
		expect(result).toEqual(
			expect.objectContaining({
				status: 400,
				data: expect.objectContaining({
					settingsError: 'settings must be a JSON object'
				})
			})
		);
	});
