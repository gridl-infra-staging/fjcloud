...
describe('Index detail page server -- actions (documents)', () => {
	...
	it('addDocument action wraps JSON record in addObject request and returns refreshed browse data', async () => {
		...
		expect(browseObjectsMock).toHaveBeenCalledWith('products', { query: 'obj-2', hitsPerPage: 20 });
		expect(result).toEqual({
	...
	it('addDocument action waits briefly for the written object to appear in refreshed browse data', async () => {
		addObjectsMock.mockResolvedValue({ taskID: 100, objectIDs: ['obj-2'] });
		browseObjectsMock
		...
		expect(browseObjectsMock).toHaveBeenCalledTimes(2);
		expect(browseObjectsMock).toHaveBeenNthCalledWith(1, 'products', {
			query: 'obj-2',
			hitsPerPage: 20
		});
		expect(result).toEqual({
