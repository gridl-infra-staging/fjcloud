# Console Billing Screen Spec

## Scope

- Primary route: `/console/billing`
- Related routes: `/console/billing/invoices`
...
The page shows `Billing` and starts from a server-owned availability seam in `web/src/routes/console/billing/+page.server.ts`: `load()` calls `getPaymentMethods()` and returns only `billingUnavailable` in the page-data contract.

UI ownership stays in `web/src/routes/console/billing/+page.svelte`: the available state renders a single native form action (`Manage billing`) posting to `?/manageBilling`; the unavailable state renders `BillingUnavailableCard` and hides the manage form.
...
- `Manage billing` submits `POST ?/manageBilling` on `/console/billing`.
- Server action ownership is `web/src/routes/console/billing/+page.server.ts::actions.manageBilling`: it derives `return_url` as `<request origin>/console/billing`, calls `POST /billing/portal`, and redirects with HTTP 303 to Stripe.
- Action failures render a visible alert in `web/src/routes/console/billing/+page.svelte`.
...
- Component tests: `web/src/routes/console/billing/billing.test.ts`
- Server/contract tests: `web/src/routes/console/billing/billing.server.test.ts`
