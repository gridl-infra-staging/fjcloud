...
async function expectSettingsSaveFeedback(page: Page, settingsSection: Locator): Promise<void> {
	await expect(
		page
			.getByText('Settings saved.')
			.or(settingsSection.getByText(/Failed to save settings|backend temporarily unavailable/))
	).toBeVisible();
}

async function saveSettingsAndWaitForCapturedResult(
	page: Page,
	settingsSection: Locator
): Promise<void> {
	await settingsSection.getByRole('button', { name: 'Save Settings' }).click();
	await expectSettingsSaveFeedback(page, settingsSection);
}
...
test.describe('Dashboard page', () => {
	...
	test('logs route shows save-settings entry from shared dashboard log path', async ({
		...
		await saveSettingsAndWaitForCapturedResult(page, settingsSection);

		const logsLink = page.getByRole('navigation').getByRole('link', { name: 'Logs', exact: true });
		await expect(logsLink).toHaveAttribute('href', '/console/logs');
		...
		await expect(logPanel.getByRole('heading', { name: 'API Activity Log' })).toBeVisible();
		await expect(logPanel.getByText(/captured in this browser session/i)).toBeVisible();
		...
		await expect(logPanel.getByText('Request', { exact: true })).toBeVisible();
		await expect(logPanel.getByText('Response', { exact: true })).toBeVisible();
		...
		await firstDataRow.click();
		await expect(logPanel.getByText('Request', { exact: true })).toHaveCount(0);
		await expect(logPanel.getByText('Response', { exact: true })).toHaveCount(0);
	...
	test('logs route deep-link view modes survive reload and keep row interaction', async ({
		...
		const customer = await arrangeTrackedCustomerSession(page, {
			emailPrefix: 'dash-console-logs-view'
		});
	...
	test('dashboard shows "Manage indexes" link when indexes exist', async ({
		...
		const customer = await arrangeTrackedCustomerSession(page, {
			emailPrefix: 'dash-console-index-card'
		});
		const name = `dash-idx-${Date.now()}`;
		await seedCustomerIndex(customer, name);
...
test.describe('Dashboard verification banner', () => {
	test('unverified user can resend verification in shell and keeps success message across dashboard navigation', async ({
		...
		await Promise.all([
			page.waitForURL(/\/console\/account/),
	...
	test('unverified banner renders deterministic 400 resend error without redirecting to settings', async ({
		...
		await page.route('**/console/resend-verification', async (route) => {
			await route.fulfill({
				status: 400,
...
test.describe('Plan-aware dashboard features', () => {
	test('plan badge displays the user plan type', async ({ page }) => {
		...
		await expect(badge).toHaveText(/(?:Free|Shared|Paid) Plan/);
	});

	test('Paid-plan billing prompt navigates to billing setup', async ({ page, setBillingPlan }) => {
		await setBillingPlan('shared');
		await page.goto('/console');

		// The billing prompt appears for Paid-plan users without a payment method.
		const billingPrompt = page.getByTestId('billing-prompt');
		if ((await billingPrompt.count()) === 0) {
			test.skip(true, 'Paid plan fixture already has payment method configured');
		}
	...
	test('layout billing CTA navigates to billing setup and hides on billing routes @p0_coverage', async ({
		page,
		setBillingPlan
	}) => {
		await setBillingPlan('shared');
		...
		await expect(cta.getByText('Your Paid plan requires billing setup to continue.')).toBeVisible();

		// Navigate via the "Set up billing" link in the layout CTA
		const setupLink = cta.getByRole('link', { name: 'Set up billing' });
		await expect(setupLink).toHaveAttribute('href', '/console/billing/setup');
	...
	test('free-tier progress is hidden for Paid-plan users', async ({ page, setBillingPlan }) => {
		await setBillingPlan('shared');
		await page.goto('/console');

		await expect(page.getByTestId('plan-badge')).toHaveText('Paid Plan');
		// Free-tier progress section must not appear for Paid-plan users.
		await expect(page.getByTestId('free-tier-progress')).toBeHidden();
	});

	test('free-plan dashboard shows free-tier usage without Paid-plan billing prompts', async ({
		page,
		setBillingPlan
	}) => {
...
test.describe('Dashboard error boundary', () => {
	test('unmapped dashboard route renders public recovery copy with one support reference', async ({
		...
		await page.goto(`/console/missing-route-${Date.now()}`);
