...
fn tenant_router() -> Router {
    tenant_router_with_repo(crate::common::mock_repo())
}

fn tenant_router_with_repo(repo: std::sync::Arc<crate::common::MockCustomerRepo>) -> Router {
...
async fn tenant_expired_token_returns_401() {
    ...
    let token = crate::common::create_expired_jwt(customer_id);
...
async fn tenant_wrong_secret_returns_401() {
    let customer_id = Uuid::new_v4();
    let token = crate::common::create_jwt_with_secret(customer_id, "wrong-secret");
...
async fn tenant_non_uuid_sub_returns_401() {
    ...
    let token = jsonwebtoken::encode(
        ...
        &claims,
        &jsonwebtoken::EncodingKey::from_secret(crate::common::TEST_JWT_SECRET.as_bytes()),
    )
    .unwrap();
...
async fn tenant_unknown_customer_returns_401() {
    ...
    let token = crate::common::create_test_jwt(Uuid::new_v4());
