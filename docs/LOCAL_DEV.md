...
- **Docker** — any docker daemon with `docker compose` v2: Docker Desktop, Colima, OrbStack, Rancher Desktop, or native dockerd on Linux. The scripts probe daemon reachability via `scripts/lib/docker.sh` and fail-fast with a context-aware hint if the daemon is offline. On macOS, Colima's SSH-tunneled socket forward can go stale after sleep/resume cycles ([colima#460](https://github.com/abiosoft/colima/issues/460)); recover with `colima restart`. (OrbStack is faster on Apple Silicon but had several open macOS Tahoe (26.x) compatibility issues as of mid-2026 — [#2222](https://github.com/orbstack/orbstack/issues/2222), [#2237](https://github.com/orbstack/orbstack/issues/2237), [#2335](https://github.com/orbstack/orbstack/issues/2335) — revisit once those close.)
- **Rust toolchain** — install via [rustup.rs](https://rustup.rs)
- **Node.js** — v20.19+ or v22.12+ with npm. Node 22 LTS is the recommended local signoff runtime.
- **flapjack_dev repo** — clone adjacent to this repo. When `FLAPJACK_DEV_DIR` selects a source checkout, the shared helper builds `flapjack-server` with Cargo and validates the resulting `engine/target/debug/flapjack` against a source-build receipt and exact engine identity.
...
flapjack binary path, helper-owned source receipt provenance when a selected
...
`scripts/lib/local_stack_contract.sh`. Local startup no longer accepts an
existing selected-checkout binary just because it reports version `1.0.10`. The
API advertises named contracts from `/version`; `scripts/e2e-preflight.sh`
requires the dashboard capabilities before browser tests run, so a stale but
healthy API fails before any workflow.

After the stack is up, use [`docs/runbooks/local-dev.md`](./runbooks/local-dev.md)
...
[`docs/runbooks/local-dev.md`](./runbooks/local-dev.md). For the exact checklist execution order, see
...
Project-scoped status checks for multi-worktree safety:
...
helper owns source builds, source receipts, release-artifact exceptions, and
provenance output. Restart-critical prerequisite callers
(`./scripts/local-signoff.sh --check-prerequisites`,
`bash scripts/integration-up.sh --check-prerequisites`, and HA restart helpers)
try that explicit directory first, continue through configured/default nearby
directory candidates, and only then fall back to `PATH`; selected source build
or receipt failures fail closed instead of substituting another binary.
`scripts/local-dev-up.sh` keeps its warning-only startup path only when no local
flapjack binary/source checkout resolves.
