...
#[tokio::test]
async fn auth_rate_limit_allows_only_admin_keyed_canary_register_bypass() {
    let _lock = security_env_lock()
        .lock()
        .unwrap_or_else(|poisoned| poisoned.into_inner());
    ...
    let ip = "203.0.113.212";

    for email in ["anon-canary-1@example.com", "anon-canary-2@example.com"] {
        let resp = app
            .clone()
            .oneshot(json_post(
                "/auth/register",
                json!({
                    "name": "Anonymous Canary",
                    "email": email,
    ...
    let anonymous_limited = app
        .clone()
        .oneshot(json_post(
            "/auth/register",
            json!({
                "name": "Anonymous Canary",
                "email": "anon-canary-limited@example.com",
                "password": "strongpassword123"
            }),
            ip,
        ))
    ...
    let invalid_admin_key = app
        .clone()
        .oneshot(json_post_with_admin_key(
            "/auth/register",
            json!({
                "name": "Wrong Key Canary",
                "email": "wrong-key-canary@example.com",
                "password": "strongpassword123"
            }),
            ip,
            "wrong-admin-key",
        ...
        .await
        .unwrap();
    assert_eq!(invalid_admin_key.status(), StatusCode::TOO_MANY_REQUESTS);

    let keyed_canary = app
        .oneshot(json_post_with_admin_key(
            "/auth/register",
            json!({
                "name": "Keyed Canary",
                "email": "keyed-canary@example.com",
                "password": "strongpassword123"
            }),
            ip,
        ...
        .await
        .unwrap();
    assert_eq!(keyed_canary.status(), StatusCode::CREATED);
}
