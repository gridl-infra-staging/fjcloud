...
impl TenantRepo for PgTenantRepo {
    ...
    async fn publish_delete_lifecycle_intent(
        ...
        let published = sqlx::query_as::<_, CustomerTenant>(
            "UPDATE customer_tenants SET tier = 'deleting'
             WHERE customer_id = $1
               AND tenant_id = $2
               AND deployment_id = $3
               AND vm_id IS NOT DISTINCT FROM $4
               AND tier = 'active'
               AND cold_snapshot_id IS NOT DISTINCT FROM $5
               AND service_type = $6
             RETURNING *",
        )
        .bind(customer_id)
        ...
        let removed = sqlx::query_scalar::<_, Uuid>(
            "DELETE
             ...
             WHERE customer_id = $1
               ...
               AND vm_id IS NOT DISTINCT FROM $4
               AND tier = $5
               AND cold_snapshot_id IS NOT DISTINCT FROM $6
               AND service_type = $7
