...
struct TestHarness {
    ...
    migration_repo: Arc<crate::common::MockIndexMigrationRepo>,
...
impl TestHarness {
    fn new() -> Self {
        ...
        Self {
            ...
            vm_inventory_repo,
            discovery_service,
            migration_repo: crate::common::mock_index_migration_repo(),
            ...
            node_secret_manager: crate::common::mock_node_secret_manager(),
        }
    }
    ...
    fn build_service_with_config(&self, config: ColdTierConfig) -> ColdTierService {
        self.build_service_with_config_and_object_store(config, self.object_store.clone())
    }
    ...
    fn build_service_with_object_store(
        ...
        self.build_service_with_config_and_object_store(ColdTierConfig::default(), object_store)
    }

    fn build_service_with_config_and_object_store(
        &self,
    ...
    async fn seed_idle_index(&self, tenant_id: &str) -> (Uuid, Uuid) {
        crate::common::seed_idle_cold_tier_index(
            &self.customer_repo,
            &self.tenant_repo,
...
async fn snapshot_pipeline_exports_uploads_evicts() {
    ...
    let candidate = h.candidate(customer_id, "snap-index", vm_id).await;
    ...
    let calls_before_post_snapshot_discovery = h.tenant_repo.find_raw_call_count();
    ...
    assert_eq!(
        h.tenant_repo.find_raw_call_count(),
        calls_before_post_snapshot_discovery + 1
    );
...
async fn snapshot_failure_retries_next_cycle() {
    ...
    let candidate = h.candidate(customer_id, "retry-index", vm_id).await;
...
async fn snapshot_terminal_failure_fires_critical_alert() {
    ...
    let candidate = h.candidate(customer_id, "terminal-index", vm_id).await;
...
enum SnapshotFailureBoundary {
    Export,
    Upload,
    Delete,
}

impl SnapshotFailureBoundary {
    fn target(self) -> &'static str {
        match self {
            Self::Export => "export-boundary",
            Self::Upload => "upload-boundary",
            Self::Delete => "delete-boundary",
        }
    }

    fn expected_reason(self) -> &'static str {
        match self {
            Self::Export => "export error: export refused",
            Self::Upload => "upload error: object store error: 403 Forbidden: access denied",
            Self::Delete => "evict error: delete refused",
        }
    }

    fn expected_calls(self) -> (usize, usize, usize) {
        match self {
            Self::Export => (1, 0, 0),
            Self::Upload => (1, 1, 0),
            Self::Delete => (1, 1, 1),
    ...
    fn configure(self, harness: &TestHarness, flapjack_uid: &str, upload_fails: &AtomicBool) {
        harness.node_client.set_export_response(
            flapjack_uid,
            match self {
                Self::Export => Err("export refused".to_string()),
                Self::Upload | Self::Delete => Ok(b"boundary-snapshot".to_vec()),
            },
        );
        if matches!(self, Self::Upload) {
            upload_fails.store(true, Ordering::SeqCst);
        }
        if matches!(self, Self::Delete) {
            harness
                .node_client
                .set_delete_response(flapjack_uid, Err("delete refused".to_string()));
        }
    }
}

fn assert_failure_variant(
    boundary: SnapshotFailureBoundary,
    error: &api::services::cold_tier::ColdTierError,
) {
    match (boundary, error) {
        (
            SnapshotFailureBoundary::Export,
            api::services::cold_tier::ColdTierError::Export(message),
        ) => {
            assert_eq!(message, "export refused");
        }
        (
            SnapshotFailureBoundary::Upload,
            api::services::cold_tier::ColdTierError::Upload(message),
        ) => {
            assert_eq!(message, "object store error: 403 Forbidden: access denied");
        }
        (
            SnapshotFailureBoundary::Delete,
            api::services::cold_tier::ColdTierError::Evict(message),
        ) => {
            assert_eq!(message, "delete refused");
        }
        _ => panic!("unexpected {boundary:?} error: {error:?}"),
    }
}

struct UnrelatedSnapshotControl {
    customer_id: Uuid,
    snapshot_id: Uuid,
...
async fn seed_unrelated_snapshot_control(harness: &TestHarness) -> UnrelatedSnapshotControl {
    let (customer_id, vm_id) = harness.seed_idle_index("unrelated-control").await;
    let snapshot = harness
        .cold_snapshot_repo
        .create(NewColdSnapshot {
            customer_id,
            tenant_id: "unrelated-control".to_string(),
            source_vm_id: vm_id,
            object_key: "cold/unrelated-control.fj".to_string(),
        })
        .await
        .expect("seed unrelated snapshot control");
    harness
        .cold_snapshot_repo
    ...
    harness
        .cold_snapshot_repo
        .set_completed(snapshot.id, 7, "control-checksum")
        .await
        .unwrap();
    let tenant = harness
        .tenant_repo
        .find_raw(customer_id, "unrelated-control")
        .await
        .unwrap()
        .unwrap();
    let snapshot = harness
        ...
        .unwrap()
        .unwrap();
    UnrelatedSnapshotControl {
        customer_id,
        snapshot_id: snapshot.id,
        tenant: serde_json::to_value(tenant).unwrap(),
        snapshot: serde_json::to_value(snapshot).unwrap(),
    }
}

fn assert_compensated_tenant(
    tenant: &CustomerTenant,
    original: &CustomerTenant,
    source_vm_id: Uuid,
) {
    assert_eq!(tenant.tier, "active");
    assert_eq!(tenant.vm_id, Some(source_vm_id));
    assert_eq!(tenant.cold_snapshot_id, None);
    assert_eq!(tenant.deployment_id, original.deployment_id);
    assert_eq!(tenant.service_type, original.service_type);
}

fn assert_compensated_snapshot(snapshot: &ColdSnapshot, boundary: SnapshotFailureBoundary) {
    assert_eq!(snapshot.status, "failed");
    assert_eq!(snapshot.error.as_deref(), Some(boundary.expected_reason()));
    let completed_before_failure = matches!(boundary, SnapshotFailureBoundary::Delete);
    assert_eq!(
        snapshot.size_bytes,
        if completed_before_failure { 17 } else { 0 }
    );
    assert_eq!(
        snapshot.checksum.as_deref(),
        completed_before_failure
            .then_some("3389cac608782bf7a335fcf4f5a4a590c761899634d122b9f05bfc91dd0efcf6")
    );
}

fn assert_boundary_calls(
    boundary: SnapshotFailureBoundary,
    harness: &TestHarness,
    object_store: &FailableObjectStore,
) {
    let (exports, puts, deletes) = boundary.expected_calls();
    assert_eq!(harness.node_client.export_call_count(), exports);
    assert_eq!(object_store.put_call_count(), puts);
    assert_eq!(harness.node_client.delete_call_count(), deletes);
}

async fn assert_unrelated_control_unchanged(
    harness: &TestHarness,
    control: &UnrelatedSnapshotControl,
) {
    let tenant = harness
        .tenant_repo
        .find_raw(control.customer_id, "unrelated-control")
        .await
        .unwrap()
        .unwrap();
    let snapshot = harness
        .cold_snapshot_repo
        .get(control.snapshot_id)
        .await
        .unwrap()
        .unwrap();
    assert_eq!(serde_json::to_value(tenant).unwrap(), control.tenant);
    assert_eq!(serde_json::to_value(snapshot).unwrap(), control.snapshot);
}

async fn run_snapshot_failure_boundary(boundary: SnapshotFailureBoundary) {
    let harness = TestHarness::new();
    let target = boundary.target();
    let (customer_id, vm_id) = harness.seed_idle_index(target).await;
    let original_tenant = harness
        .tenant_repo
        .find_raw(customer_id, target)
        .await
        .unwrap()
        .unwrap();
    let unrelated_control = seed_unrelated_snapshot_control(&harness).await;
    let upload_fails = Arc::new(AtomicBool::new(false));
    let object_store = Arc::new(FailableObjectStore::new(
        harness.object_store.clone(),
        upload_fails.clone(),
    ));
    let flapjack_uid = flapjack_index_uid(customer_id, target);
    boundary.configure(&harness, &flapjack_uid, &upload_fails);
    let service = harness.build_service_with_object_store(object_store.clone());
    let candidate = harness.candidate(customer_id, target, vm_id).await;

    let error = service
        .snapshot_candidate(&candidate, "http://vm-1.flapjack.foo", "us-east-1")
        .await
        .expect_err("failure boundary must stop the pipeline");
    assert_failure_variant(boundary, &error);
    assert_eq!(error.to_string(), boundary.expected_reason());
    let snapshot = harness
        .cold_snapshot_repo
        .find_active_for_index(customer_id, target)
        .await
        .unwrap()
        .expect("failed pipeline retains its operation snapshot until compensation");

    service
        .handle_snapshot_failure(&candidate, Some(snapshot.id), boundary.expected_reason())
        .await;
    ...
    let tenant = harness
        ...
        .unwrap()
        .unwrap();
    assert_compensated_tenant(&tenant, &original_tenant, vm_id);
    let snapshot = harness
        .cold_snapshot_repo
        .get(snapshot.id)
        .await
        .unwrap()
        .unwrap();
    assert_compensated_snapshot(&snapshot, boundary);
    assert_boundary_calls(boundary, &harness, object_store.as_ref());
    assert_unrelated_control_unchanged(&harness, &unrelated_control).await;
}

#[tokio::test]
async fn snapshot_failure_boundaries_restore_operation_owned_state() {
    for boundary in [
        SnapshotFailureBoundary::Export,
        SnapshotFailureBoundary::Upload,
        SnapshotFailureBoundary::Delete,
    ] {
        run_snapshot_failure_boundary(boundary).await;
    }
}
...
async fn cold_snapshot_from_hetzner_vm_succeeds() {
    ...
    let candidate = h.candidate(customer_id, "hetzner-index", vm_id).await;
