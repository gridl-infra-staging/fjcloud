...
source "$SCRIPT_DIR/lib/hydrate_staging_env.sh"
...
uses_staging_db_runner() {
    [ "$PROBE_DB_URL_FROM_SSM" = true ] && staging_db_database_url_ssm_environment >/dev/null 2>&1
}

strip_sql_trailing_semicolon() {
    local sql="$1"
    sql="$(printf '%s' "$sql" | sed -E 's/[[:space:]]*;[[:space:]]*$//')"
    printf '%s\n' "$sql"
}
...
run_db_query() {
    ...
    if uses_staging_db_runner; then
        staging_db_run_sql "$db_url" "$sql"
        return $?
    fi

    psql -v ON_ERROR_STOP=1 -X -t -A "$db_url" -c "$sql"
}
...
run_psql_query() {
    ...
    if ! run_db_query "$db_url" "$sql"; then
        return 1
    fi
}
...
run_psql_scalar() {
    ...
    if uses_staging_db_runner; then
        sql="$(strip_sql_trailing_semicolon "$sql")"
        output="$(run_db_query "$db_url" "COPY ($sql) TO STDOUT;")" || return 1
    else
...
main() {
    ...
    if [ -z "$db_url" ] && [ -n "${DATABASE_URL_SSM_PARAM:-}" ]; then
        # .env.secret intentionally omits the DB URL; SSM owns the canonical
        # /fjcloud/<env>/database_url value, and VPC-private RDS reads must go
        # through scripts/lib/staging_db.sh::staging_db_run_sql.
        local hydrate_error_file hydrate_error hydrate_environment
        if ! hydrate_environment="$(staging_db_database_url_ssm_environment)"; then
            usage_failure "DATABASE_URL_SSM_PARAM must be /fjcloud/staging/database_url or /fjcloud/prod/database_url when no direct DATABASE_URL is set."
        fi
        hydrate_error_file="$(mktemp)"
        if ! hydrate_env_from_ssm "$hydrate_environment" 2>"$hydrate_error_file"; then
            hydrate_error="$(cat "$hydrate_error_file" 2>/dev/null || true)"
            rm -f "$hydrate_error_file"
            usage_failure "Failed to hydrate $hydrate_environment env from SSM: $(trim_compact "$hydrate_error")"
        fi
        rm -f "$hydrate_error_file"
        db_url="${DATABASE_URL:-${INTEGRATION_DB_URL:-}}"
        PROBE_DB_URL_FROM_SSM=true
    fi
    ...
    if ! uses_staging_db_runner && ! command -v psql >/dev/null 2>&1; then
...
RETURNING id;")" || runtime_failure "seed_probe_customer" "Failed to upsert probe customer row for $recipient_email."
    ...
    local first_assert_stderr
    first_assert_stderr="$(probe_temp_file)" || runtime_failure "first_live_send" "Failed to allocate a private temp file for the first broadcast response assertion."
    if ! assert_live_send_response "$first_response" false 2>"$first_assert_stderr"; then
        local parse_error
        parse_error="$(trim_compact "$(cat "$first_assert_stderr" 2>/dev/null || true)")"
        rm -f "$first_assert_stderr"
    ...
    local second_assert_stderr
    second_assert_stderr="$(probe_temp_file)" || runtime_failure "second_live_send" "Failed to allocate a private temp file for the second broadcast response assertion."
    if ! assert_live_send_response "$second_response" true 2>"$second_assert_stderr"; then
        local second_parse_error
        second_parse_error="$(trim_compact "$(cat "$second_assert_stderr" 2>/dev/null || true)")"
        rm -f "$second_assert_stderr"
