...
test_rejects_invalid_billing_month_format() {
    setup_workspace
    wrap_preflight_owner_with_call_log
    run_rehearsal --args "--env-file $TEST_WORKSPACE/inputs/staging_rehearsal.explicit.env --month 2026-13 --confirm-live-mutation"

    assert_rehearsal_fails_as_blocker
    assert_valid_json "$RUN_STDOUT" "invalid --month output should be valid JSON"
    assert_contains "$RUN_STDOUT" "billing_month_invalid" "invalid --month should emit stable blocker classification"
    assert_contains "$RUN_STDOUT" "YYYY-MM format with month 01-12" \
        "invalid --month should reuse the canonical strict billing-month guidance"

    local artifact_dir calls
    artifact_dir="$(find_artifact_dir)"
    calls="$(read_file_or_empty "$TEST_CALL_LOG")"
    assert_eq "$(json_file_field "$artifact_dir/summary.json" "classification")" "billing_month_invalid" \
        "invalid --month blocker should be recorded in summary classification"
    assert_not_contains "$calls" "dry_run|" \
        "invalid --month should block before preflight owner invocation"
    assert_not_contains "$calls" "psql|" \
        "invalid --month should block before rehearsal DB queries"
    assert_not_contains "$calls" "curl|" \
        "invalid --month should block before health or billing HTTP calls"
}
...
test_preflight_runs_before_metering_and_mutation_paths() {
    ...
    assert_contains "$(read_file_or_empty "$artifact_dir/billing_run.json")" "\"11111111-1111-4111-8111-111111111111\"" \
        "billing_run artifact should record created invoice IDs"
    assert_contains "$(read_file_or_empty "$artifact_dir/billing_run.json")" "\"22222222-2222-4222-8222-222222222222\"" \
        "billing_run artifact should record every created invoice ID"
    assert_contains "$(read_file_or_empty "$artifact_dir/invoice_rows.json")" '"hosted_invoice_url": "https://invoice.stripe.com/i/acct_test/stage3_a"' \
        "invoice_rows artifact should include hosted_invoice_url for dunning body matching"
}

test_rehearsal_env_file_overrides_stale_parent_aws_credentials() {
    setup_workspace
    wrap_preflight_owner_with_call_log

    run_rehearsal --args "--env-file $TEST_WORKSPACE/inputs/staging_rehearsal.explicit.env --month 2026-03 --confirm-live-mutation" \
        "AWS_ACCESS_KEY_ID=stale-parent-key" "AWS_SECRET_ACCESS_KEY=stale-parent-secret" "AWS_DEFAULT_REGION=us-west-2"

    assert_rehearsal_succeeds
    assert_contains "$(read_file_or_empty "$TEST_CALL_LOG")" "aws|ssm get-parameter" \
        "SSM hydration should run after stale parent AWS credentials are cleared"
}
...
test_live_mutation_repeat_same_month_reuses_existing_invoice_without_second_billing_run() {
    setup_workspace "11111111-1111-1111-1111-111111111111,22222222-2222-2222-2222-222222222222"
    wrap_preflight_owner_with_call_log
...
test_live_mutation_uses_ses_cloudwatch_logs_when_mailpit_is_absent() {
    setup_workspace
    wrap_preflight_owner_with_call_log
    ...
    run_rehearsal --args "--env-file $TEST_WORKSPACE/inputs/staging_rehearsal.no_mailpit.env --month 2026-03 --confirm-live-mutation" \
        "REHEARSAL_MOCK_SES_CLOUDWATCH_MODE=paginated_emails_found"

    assert_rehearsal_succeeds

    local artifact_dir aws_call invoice_email_body
    artifact_dir="$(find_artifact_dir)"
    aws_call="$(grep '^aws|.*logs filter-log-events' "$TEST_CALL_LOG" | head -1 || true)"
    invoice_email_body="$(read_file_or_empty "$artifact_dir/invoice_email.json")"
    assert_stage3_evidence_artifacts_exist "$artifact_dir"
    assert_eq "$(json_file_field "$artifact_dir/summary.json" "classification")" "rehearsal_completed" \
        "staging SES email evidence should let the rehearsal complete without Mailpit"
    assert_eq "$(json_file_field "$artifact_dir/invoice_email.json" "classification")" "invoice_email_ready" \
        "invoice_email artifact should report ready when SES SEND CloudWatch Logs evidence is correlated"
    assert_contains "$aws_call" "logs filter-log-events" "missing Mailpit should fall back to CloudWatch Logs SES send-events evidence"
    assert_contains "$aws_call" "/fjcloud/staging/ses/send-events" "CloudWatch Logs SES send-events query should target the dns-module-owned log group"
    assert_contains "$(read_file_or_empty "$TEST_CALL_LOG")" "--next-token invoice-email-page-2" "CloudWatch Logs SES send-events evidence should follow filter-log-events pagination"
    assert_contains "$invoice_email_body" "ses-msg-11111111-1111-4111-8111-111111111111" "invoice_email artifact should record sanitized SES message IDs from the log events"
    assert_not_contains "$invoice_email_body" "ses-msg-ready-11111111-1111-4111-8111-111111111111" "invoice_email artifact should ignore invoice-ready SES events from the billing-run source"
    assert_contains "$invoice_email_body" "invoice_ready" "invoice_email artifact should record the paid-webhook email type gate"
    assert_contains "$invoice_email_body" "invoice_payment_succeeded" "invoice_email artifact should record the paid-webhook dispatch source gate"
    assert_contains "$invoice_email_body" "aws_cloudwatch_logs_ses_send_events" "invoice_email artifact should record the CloudWatch Logs evidence source"
    assert_not_contains "$invoice_email_body" "alpha@example.test" "invoice_email artifact must not retain recipient addresses"
}
...
test_failure_path_keeps_json_summary_and_blocked_later_step_artifacts() {
    ...
    run_rehearsal --args "--env-file $TEST_WORKSPACE/inputs/staging_rehearsal.no_stripe.env --month 2026-03 --confirm-live-mutation" \
        "REHEARSAL_MOCK_SSM_MISSING_KEYS=stripe_secret_key"
    ...
    artifact_dir="$(find_artifact_dir)"
    assert_eq "$(json_file_field "$artifact_dir/summary.json" "result")" "blocked" \
        "missing hydrated staging config should report result=blocked"
    assert_contains "$(read_file_or_empty "$artifact_dir/steps/preflight.json")" "staging_ssm_hydration_failed" \
        "preflight blocker artifact should preserve staging SSM hydration classification"
...
test_refuses_live_mutation_without_admin_key() {
    ...
    run_rehearsal --args "--env-file $TEST_WORKSPACE/inputs/staging_rehearsal.no_admin.env --month 2026-03 --confirm-live-mutation" \
        "REHEARSAL_MOCK_SSM_MISSING_KEYS=admin_key"

    assert_rehearsal_fails_as_blocker

    local artifact_dir
    artifact_dir="$(find_artifact_dir)"
    assert_eq "$(json_file_field "$artifact_dir/summary.json" "classification")" "staging_ssm_hydration_failed" \
        "missing ADMIN_KEY in staging runtime config should emit stable blocker classification"
    assert_contains "$(read_file_or_empty "$artifact_dir/steps/live_mutation_attempt.json")" "blocked" \
...
test_live_mutation_fails_when_mailpit_messages_missing_created_invoice_ids() {
    ...
    assert_contains "$(read_file_or_empty "$artifact_dir/invoice_email.json")" "11111111-1111-4111-8111-111111111111" \
...
test_live_mutation_fetches_full_mailpit_message_payload_for_invoice_id_correlation() {
    ...
    message_call="$(grep '^curl|.*api/v1/message/msg-11111111-1111-4111-8111-111111111111' "$TEST_CALL_LOG" | head -1 || true)"
    assert_contains "$message_call" "/api/v1/message/msg-11111111-1111-4111-8111-111111111111" \
...
TESTS=(test_requires_explicit_env_file_flag test_rejects_repo_default_env_filename test_rejects_nonexistent_explicit_env_file test_requires_month_when_live_mutation_confirmation_present test_rejects_invalid_billing_month_format test_requires_confirm_live_mutation_when_month_provided test_preflight_runs_before_metering_and_mutation_paths test_rehearsal_env_file_overrides_stale_parent_aws_credentials test_live_mutation_redacts_admin_key_and_locks_artifact_permissions test_live_mutation_fails_when_batch_response_has_no_created_invoices test_live_mutation_repeat_same_month_reuses_existing_invoice_without_second_billing_run test_live_mutation_ignores_unrelated_same_month_invoice_before_first_run test_live_mutation_ignores_successful_same_month_lookup_noise_before_first_run test_live_mutation_blocks_when_repeat_pass_lookup_fails_before_mutation test_live_mutation_fails_when_billing_request_times_out test_live_mutation_fails_when_billing_request_times_out_with_native_curl_exit_28 test_live_mutation_fails_when_invoice_rows_never_converge test_live_mutation_fails_when_invoice_rows_query_times_out test_live_mutation_fails_when_invoice_rows_query_hits_statement_timeout test_live_mutation_fails_when_invoice_rows_missing_required_fields)
TESTS+=(test_live_mutation_fails_when_webhook_query_times_out test_live_mutation_fails_when_webhook_rows_never_converge test_live_mutation_fails_when_webhook_rows_unprocessed test_live_mutation_uses_ses_cloudwatch_logs_when_mailpit_is_absent test_blocker_path_keeps_json_summary_and_blocked_step_artifacts test_failure_path_keeps_json_summary_and_blocked_later_step_artifacts test_malformed_env_file_still_emits_blocker_artifacts test_refuses_live_mutation_without_admin_key test_accepts_database_url_without_integration_db_url test_accepts_integration_db_url_without_database_url test_refuses_live_mutation_without_month test_refuses_live_mutation_without_confirmation_flag)
TESTS+=(test_live_mutation_fails_when_mailpit_messages_missing_created_invoice_ids test_live_mutation_fails_when_mailpit_search_returns_invalid_json test_live_mutation_fails_when_mailpit_message_fetch_returns_invalid_json test_live_mutation_fetches_full_mailpit_message_payload_for_invoice_id_correlation test_live_mutation_fails_when_mailpit_query_times_out test_live_mutation_fails_when_mailpit_query_times_out_with_native_curl_exit_28 test_live_mutation_uses_bounded_http_timeouts_for_health_billing_and_mailpit test_live_mutation_urlencodes_mailpit_query_for_plus_alias_emails test_rehearsal_live_evidence_shell_files_stay_under_hard_limits test_rehearsal_runner_stays_under_repo_warning_thresholds)
