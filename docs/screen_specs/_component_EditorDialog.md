...
- Current: Component not yet implemented in `web/src/lib/components/`. Theme A consumers (Rules, Synonyms, Recommendations, Personalization tabs) currently use raw JSON `<textarea>` patterns and require users to hand-write Algolia-format JSON.
- Target: A single `EditorDialog.svelte` + companion `FormField.svelte` (or schema-driven dispatcher) under `web/src/lib/components/`, consumed by the 4 Theme A tabs and (later) Dictionaries, SecuritySources, Suggestions, Experiments-create.
