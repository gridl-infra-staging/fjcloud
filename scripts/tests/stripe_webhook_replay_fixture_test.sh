...
test_run_mode_supports_invoice_payment_failed_exhausted_payload() {
    make_test_tmp_dir
    write_mock_curl
    ...
    run_fixture_script \
        ...
        --target-url "https://api.staging.flapjack.foo/webhooks/stripe" \
        --timestamp 1704067200 \
        --event-id "evt_exhausted_fixture" \
        --event-type "invoice.payment_failed" \
        --invoice-id "in_exhausted_fixture_01" \
        --next-payment-attempt "null" \
        --attempt-count "3" \
        "STRIPE_WEBHOOK_SECRET=whsec_exhausted_payload_secret" \
        "CURL_CALL_LOG=$call_log" \
        "CURL_ARGS_LOG=$args_log" \
        "MOCK_CURL_HTTP_CODE=200"
    ...
    assert_eq "$RUN_EXIT_CODE" "0" "invoice.payment_failed exhausted replay should pass on 2xx"
    assert_valid_json "$RUN_STDOUT" "invoice.payment_failed exhausted replay should return valid JSON"
    assert_contains "$curl_args" '"type":"invoice.payment_failed"' \
        "exhausted replay should send invoice.payment_failed event type"
    assert_contains "$curl_args" '"id":"in_exhausted_fixture_01"' \
        "exhausted replay should send invoice id in data.object.id"
    assert_contains "$curl_args" '"next_payment_attempt":null' \
        "exhausted replay should send null next_payment_attempt"
    assert_contains "$curl_args" '"attempt_count":3' \
        "exhausted replay should send attempt_count for exhausted contract"
}

test_run_mode_supports_invoice_payment_succeeded_payload() {
    make_test_tmp_dir
    write_mock_curl
    ...
    run_fixture_script \
        ...
        --target-url "https://api.staging.flapjack.foo/webhooks/stripe" \
        --timestamp 1704067200 \
        --event-id "evt_recovered_fixture" \
        --event-type "invoice.payment_succeeded" \
        --invoice-id "in_recovered_fixture_01" \
        "STRIPE_WEBHOOK_SECRET=whsec_recovered_payload_secret" \
        "CURL_CALL_LOG=$call_log" \
        "CURL_ARGS_LOG=$args_log" \
        "MOCK_CURL_HTTP_CODE=200"
    ...
    assert_eq "$RUN_EXIT_CODE" "0" "invoice.payment_succeeded replay should pass on 2xx"
    assert_valid_json "$RUN_STDOUT" "invoice.payment_succeeded replay should return valid JSON"
    assert_contains "$curl_args" '"type":"invoice.payment_succeeded"' \
        "recovery replay should send invoice.payment_succeeded event type"
    assert_contains "$curl_args" '"id":"in_recovered_fixture_01"' \
        "recovery replay should send invoice id in data.object.id"
}
...
test_same_timestamp_different_event_types_produce_unique_event_ids() {
    make_test_tmp_dir
    write_mock_curl
    ...
    run_fixture_script \
        ...
        --timestamp "$timestamp" \
        --event-type invoice.payment_failed \
        --invoice-id in_test_dedup_001 \
        --next-payment-attempt 1704070800 \
        --attempt-count 1 \
        "STRIPE_WEBHOOK_SECRET=whsec_dedup_test" \
        "API_URL=http://127.0.0.1:4010" \
        "CURL_CALL_LOG=$call_log" \
        "CURL_ARGS_LOG=$args_log"

    local failed_event_id
    failed_event_id="$(json_field "$RUN_STDOUT" "event_id")"
    ...
    local succeeded_event_id
    succeeded_event_id="$(json_field "$RUN_STDOUT" "event_id")"

    assert_ne "$failed_event_id" "$succeeded_event_id" \
        "same timestamp with different event types must produce different event IDs to avoid webhook deduplication"
    ...
    local exhausted_event_id
    exhausted_event_id="$(json_field "$RUN_STDOUT" "event_id")"

    assert_ne "$failed_event_id" "$exhausted_event_id" \
        "two payment_failed replays at same timestamp must produce different event IDs (retry vs exhausted)"
    assert_ne "$succeeded_event_id" "$exhausted_event_id" \
        "payment_succeeded and second payment_failed must have different event IDs"
}
