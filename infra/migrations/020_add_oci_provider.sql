-- Add 'oci' to provider CHECK constraints for OCI provisioner support.
...
ALTER TABLE customer_deployments ADD CONSTRAINT customer_deployments_vm_provider_check
    CHECK (vm_provider IN ('aws', 'hetzner', 'gcp', 'oci', 'bare_metal'));
...
ALTER TABLE vm_inventory ADD CONSTRAINT vm_inventory_provider_check
    CHECK (provider IN ('aws', 'hetzner', 'gcp', 'oci', 'bare_metal'));
