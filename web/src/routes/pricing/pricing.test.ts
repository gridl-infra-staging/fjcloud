...
const EXPECTED_TAX_DISCLAIMER =
	'Prices are quoted in USD and exclusive of any applicable tax in your jurisdiction.';
...
describe('Pricing page', () => {
	...
	it('renders through the shared layout trust owner while preserving pricing-specific route content', () => {
		...
		expect(
			screen.getByRole('heading', { level: 1, name: 'Start free, scale into Paid storage' })
		).toBeInTheDocument();
		expect(screen.getByTestId('pricing-page-main')).toBeInTheDocument();
		expect(screen.getByRole('contentinfo')).toBeInTheDocument();
	});

	it('renders pricing-specific heading/body copy and free-tier allowance without signup discovery', async () => {
		...
		expect(screen.getByText(MARKETING_PRICING.tax_disclaimer)).toBeInTheDocument();
		expect(
	...
	it('renders mutated CTA, free-tier MB, minimum spend, and region ordering from route payload data', async () => {
		...
		const mutatedPricing = {
			...
			tax_disclaimer: 'Custom jurisdiction tax copy from payload.',
		...
		expect(
			screen.getByText(
				`Your free tier includes ${mutatedPricing.free_tier_mb} MB of hot index storage before paid billing starts.`
			)
		).toBeInTheDocument();
		expect(
		...
		expect(within(pricingMain).getByText(mutatedPricing.tax_disclaimer)).toBeInTheDocument();
		expect(
	...
	it('renders hot/cold storage rows, minimum spend, and ordered region multipliers from MARKETING_PRICING', async () => {
		...
		expect(within(pricingMain).getByText('Paid-plan minimum')).toBeInTheDocument();
		expect(
