<script lang="ts">
	...
	const selectedEntry: StoredLogEntry | null = $derived(
		selectedEntryId
			? logEntries.find((entry) => entry.id === selectedEntryId) ?? null
			: null
	);
...
<div class="mb-6 rounded-lg border border-gray-200 bg-white p-4 shadow" data-testid="search-log-panel">
				...
				{:else}
					{#each logEntries as entry, index (entry.id)}
						...
						>
							...
							<td class="px-3 py-2">
								<span class="inline-flex rounded-full px-2 py-0.5 text-xs font-medium {entry.status >= 400 ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'}">
									{entry.status}
								</span>
							</td>
	...
	{#if selectedEntry}
		<div class="mt-3 rounded-md border border-gray-200 p-3">
			...
			<pre class="overflow-x-auto rounded bg-gray-50 p-2 text-xs text-gray-700">{JSON.stringify(selectedEntry, null, 2)}</pre>
		</div>
	{/if}
</div>
