...
- Primary route: `/terms`
- Related routes: `/`, `/privacy`, `/dpa`
- Audience: unauthenticated prospects and authenticated customers reviewing legal terms
- Priority: P0

## User Goal

Read terms-of-service content with clear section headings, effective date, and support/legal navigation.

## Target Behavior

The page renders a single legal article card with a back link to `/`, the `Terms of Service` heading, an effective-date header, canonical legal sections, and the shared legal footer/support contract.
...
- Success: Terms heading, effective date, all expected section headings, and shared support footer are visible and consistent.
...
- [ ] The page body renders `Terms of Service` and the route-specific effective date.
...
None known for this static public legal route. JSDOM axe coverage now proves the route component in `web/src/routes/terms/terms.test.ts`.

## Automated Coverage

- Browser-unmocked tests: `web/tests/e2e-ui/full/public-pages.spec.ts`
- Component tests: `web/src/routes/terms/terms.test.ts`
