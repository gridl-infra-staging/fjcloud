...
| Directory | Summary |
| --- | --- |
| canary | The canary directory contains continuous health monitoring and synthetic integration tests that validate end-to-end customer flows (signup through billing setup) and external system availability across AWS, email, OAuth, and Stripe. |
| chaos | The chaos directory contains proof scripts for testing high availability and resilience scenarios, including region failover detection, health monitoring recovery, and metering system breaker alerts. |
| dev | Migrates top-level integration test files from `infra/api/tests/` to `infra/api/tests/integration/` while rewriting module imports from relative `common::` and `integration_helpers::` paths to crate-rooted `crate::common::*` paths. |
| launch | The launch directory contains shell scripts that orchestrate staging deployment verification, evidence capture for launch blockers, and synthetic traffic generation. |
| lib | This directory contains reusable bash helper functions organized by responsibility: HTTP requests, environment loading, Stripe operations, database access, alerts, metering validation, and ops-layer probes. |
| load | The load directory contains a bash library for regression testing that compares load test results against baseline measurements across five target API endpoints, evaluating latency percentiles, throughput, and error rates with configurable degradation thresholds to classify performance changes as pass, warn, or fail. |
| reliability | The reliability directory contains capacity profiling, backend validation, and security automation scripts that measure system performance across document tiers, orchestrate security checks like cargo audit and secret scanning, and validate infrastructure consistency between VM inventory and EC2. |
| stripe | This directory contains operational scripts for managing Stripe integration, including tools to configure the billing portal and create the product catalog against specific Stripe accounts. |
| tests | The tests directory contains shell script-based smoke tests for key operational workflows like customer broadcast and SES bounce/complaint handling, along with a shared lib/ of testing utilities including assertion frameworks, mocking helpers, and specialized harness scripts for scenarios like chaos testing and billing rehearsals. |
| vlm | The vlm directory contains a verdict aggregation module and environment helper functions that support VLM judge operations, specifically a self-contained 5-function closure for reading environment values extracted from deployment utilities. |
| w3_triage | w3_triage is an orchestrated audit remediation workflow that probes live state, discovers audit summaries, and authors dispatch manifests and lane files to coordinate triage work across multiple stages. |
| canary | The canary directory contains synthetic probes and health checks for staging, including a customer-loop canary (signup through Stripe integration), an external AWS health checker, and email deliverability validation. |
| chaos | Chaos engineering scripts that validate Flapjack's high-availability failover and resilience mechanisms by deliberately killing regions, primaries, and metrics targets while monitoring for proper alert detection and recovery. |
| dev | This file contains a stub summary placeholder for migrate_integration_tests_consolidation.py, indicating it is an undocumented automation tool related to consolidating integration tests. |
| launch | The launch directory contains staging deployment and verification scripts that handle evidence capture for billing rehearsals, tenant-map validation, browser testing of signup/payment flows, environment hydration from AWS SSM, synthetic traffic seeding, and direct EC2 command execution via AWS Systems Manager. |
| lib | The lib directory contains shared shell script helpers that provide reusable functionality for alert dispatch, environment loading, health checks, HTTP/JSON operations, database migrations, metering validation, security checks, Stripe integration, and infrastructure probes used across the fjcloud project's operational and testing scripts. |
| load | The 'load' directory contains load testing utilities, specifically a shell script that performs regression checks and validation by comparing offline and live load test harness results. |
| reliability | This directory contains shell scripts for backend reliability testing, capacity profiling across three document-tier levels (1k, 10k, 100k), and security validation gates that aggregate cargo audit, secret scanning, and unsafe code pattern checks. |
| stripe | This directory contains shell scripts for managing Stripe account configuration, including scripts to set up the Customer Portal and create the canonical Flapjack product catalog. |
| tests | Tests directory containing shell script-based smoke tests for ops-layer verification (SES bounce/complaint handling, customer broadcast operations) and a shared test harness library with assertion functions and mock builders. |
| vlm | The vlm directory contains utilities for VLM (Vision Language Model) verdict bundle aggregation and environment variable processing, with helper functions implementing the VLM judge's environment value reading contract for deployment workflows. |
| w3_triage | The w3_triage directory contains scripts for bootstrapping W3C compliance audits, parsing audit recommendations, applying remediation rules, and authoring automated dispatch lanes. |
<!-- [scrai:end] -->
