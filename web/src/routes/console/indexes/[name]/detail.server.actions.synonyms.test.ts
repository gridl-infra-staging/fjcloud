...
describe('Index detail page server -- actions (synonyms-preview)', () => {
	...
	it('createPreviewKey action calls createIndexKey with search and addObject ACLs and returns key', async () => {
		...
		expect(createIndexKeyMock).toHaveBeenCalledWith('products', 'Search preview', [
			'search',
			'addObject'
		]);
		expect(result).toEqual({
