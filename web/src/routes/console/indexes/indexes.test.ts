...
vi.mock('$app/forms', () => ({
	...
	enhance: (
		...
		submitHandler: (args: {
			formData: FormData;
		}) => void | ((args: { result: unknown; formData: FormData }) => Promise<void>)
	) => {
		latestEnhanceResultHandler = async ({ result, formData }) => {
			const resultHandler = submitHandler({ formData });
			if (typeof resultHandler === 'function') {
				await resultHandler({ result, formData });
			}
		};
		return { destroy: () => {} };
	}
...
describe('Index list page', () => {
	...
	function formDataWithIndexName(name: string): FormData {
	...
	it('renders index table with name, region, status, entries, and data size', () => {
		...
		expect(productsCells.getByText('Available')).toBeInTheDocument();
	...
	it('canceling or successful submit resets route-owned region and dialog visibility state', async () => {
		...
		await latestEnhanceResultHandler?.({
			...
			formData: formDataWithIndexName('fresh-products')
		});
	...
	it('emits a shared toast when enhanced create redirects after success', async () => {
		const callOrder: string[] = [];
		vi.mocked(applyAction).mockImplementationOnce(async () => {
			callOrder.push('applyAction');
		});
		...
		expect(toastSuccessMock).toHaveBeenCalledWith("Index 'fresh-products' created", {
	...
	it('emits a shared toast when enhanced delete succeeds in place', async () => {
		...
		await latestEnhanceResultHandler?.({
			...
			formData: formDataWithIndexName('products')
		});

		expect(toastSuccessMock).toHaveBeenCalledWith("Index 'products' deleted", {
			duration: TOAST_DURATION_MS
		});
		expect(callOrder).toEqual(['applyAction', 'toast']);
	});

	it('does not emit a shared toast when enhanced delete fails page-scoped', async () => {
		render(IndexesPage, {
			data: { ...layoutTestDefaults, user: null, indexes: sampleIndexes, regions: sampleRegions },
			form: null
		});
		...
		await latestEnhanceResultHandler?.({
			result: {
				type: 'failure',
				status: 400,
				data: { error: 'Failed to delete index' }
			},
			formData: formDataWithIndexName('products')
		});
	...
	it('empty state renders correctly', () => {
		...
		expect(
			screen.getByText(/create your first index to add searchable documents/i)
		).toBeInTheDocument();
	...
	it('status badges show correct colors', () => {
		...
		// Available indexes should have green badge text
		const readyBadges = screen.getAllByText('Available');
