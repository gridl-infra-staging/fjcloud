...
async function expectInvoiceTimelineLabelHasNonEmptyValue(
	...
	const timelineLabelWithDate = page
		.getByRole('main')
		.getByText(
			new RegExp(`^\\s*${escapeRegex(label)}\\s+[A-Z][a-z]{2}\\s+\\d{1,2},\\s+\\d{4}\\s*$`)
		);
	await expect(timelineLabelWithDate).toHaveCount(1);
	await expect(timelineLabelWithDate).toBeVisible();
}
