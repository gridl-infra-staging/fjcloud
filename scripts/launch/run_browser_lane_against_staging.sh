...
# │ This is the FINAL GATE, not the debug loop. To develop/iterate on the     │
# │ LB-2/LB-3 specs or their Stripe billing flows, use the LOCAL lane:        │
# │   scripts/launch/run_browser_lane_locally.sh                              │
# │ It runs the SAME specs against a local stack in real Stripe TEST mode     │
# │ (pk_test/sk_test hydrated from SSM — same sandbox as staging, so Stripe   │
# │ Payment-Element behavior reproduces faithfully) in ~1 min/run, NO deploy. │
# │ Run THIS staging launcher ONCE to capture launch-gate evidence after the  │
# │ fix is already green locally and deployed. Iterating a code fix by        │
# │ repeatedly redeploying to staging is a 30-60 min/loop anti-pattern that   │
# │ once burned a 12-hour session. See docs/runbooks/browser-lane-local-first.md │
# └──────────────────────────────────────────────────────────────────────────┘
#
...
run_one_lane() {
  ...
  local scrubbed
  scrubbed="$(sed "s|${REPO_ROOT}/||g" "$stdout_path")"
...
copy_trace_artifacts_into_bundle() {
  ...
  for source_dir in "${source_dirs[@]}"; do
    source_dir_json="${source_dir_json}${json_separator}\"${source_dir#$REPO_ROOT/}\""
    json_separator=", "
