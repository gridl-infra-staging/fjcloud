<script lang="ts">
	...
	function merchandisingSourceHits(): Array<Record<string, unknown>> {
		if (!merchandisingSubmittedQuery || !searchResult || searchQuery !== merchandisingSubmittedQuery) {
			return [];
		}
	...
	function buildMerchandisingRule() {
		...
		<div class="mb-6 rounded-lg bg-white p-6 shadow" data-testid="merchandising-section" data-index={index.name}>
			<h2 class="mb-4 text-lg font-medium text-gray-900">Merchandising</h2>

			<form method="POST" action="?/search" use:enhance class="mb-4 flex gap-3" onsubmit={submitMerchandisingQuery}>
				<input
					type="text"
					name="query"
			...
			{:else}
								...
								<div class="flex items-start justify-between gap-4">
									...
									<div class="flex items-center gap-2">
										{#if isPinned(String(hit.objectID ?? ''))}
											<span class="inline-flex rounded-full bg-blue-100 px-2 py-0.5 text-xs font-medium text-blue-800">#{idx + 1}</span>
										{/if}
										<button
											type="button"
					...
					<div class="mt-6 rounded-md border border-gray-200 p-4">
						...
						<div class="space-y-2 text-sm text-gray-700">
							{#each merchandisingHides as hidden (hidden.objectID)}
								{@const hiddenHit = merchandisingSourceHits().find((hit) => String(hit.objectID ?? '') === hidden.objectID)}
				...
				{#if merchandisingRule}
					<form method="POST" action="?/saveRule" use:enhance class="mt-4 rounded-md border border-gray-200 p-4">
