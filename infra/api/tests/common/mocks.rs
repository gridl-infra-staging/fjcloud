...
impl CustomerRepo for MockCustomerRepo {
    ...
    async fn hard_delete(
        ...
        let mut customers = self.customers.lock().unwrap();
        let Some(position) = customers.iter().position(|customer| customer.id == id) else {
            return Ok(api::repos::CustomerHardDeleteOutcome::NotFound);
        };
        if kind == api::repos::CustomerHardDeleteKind::PrivacyErasure
            && customers[position].status != "deleted"
        {
            return Ok(api::repos::CustomerHardDeleteOutcome::NotSoftDeleted);
        }
        customers.remove(position);
        drop(customers);
        let mut links = self.oauth_identities.lock().unwrap();
        links.retain(|_, customer_id| *customer_id != id);
        Ok(api::repos::CustomerHardDeleteOutcome::Erased {
            seal_scrub_work: Vec::new(),
        })
    }
...
impl MockDeploymentRepo {
    ...
    pub fn update_provisioning_call_count(&self) -> usize {
        self.update_provisioning_calls.load(Ordering::SeqCst)
    }

    pub fn set_status(&self, id: Uuid, status: &str) -> bool {
    ...
    /// Seed a failed deployment with a persisted failure reason once the
    /// canonical `Deployment` model owns that nullable field.
    #[allow(clippy::too_many_arguments)]
    pub fn seed_with_failure_reason(
        &self,
        customer_id: Uuid,
        node_id: &str,
        region: &str,
        ...
        let deployment = self.seed(customer_id, node_id, region, vm_type, vm_provider, status);
    ...
    pub fn set_failure_reason_for_test(&self, id: Uuid, failure_reason: &str) -> Deployment {
        ...
        let deployment = deployments
            .iter_mut()
            .find(|d| d.id == id)
            .expect("seeded deployment should exist before setting failure_reason");

        deployment.failure_reason = Some(failure_reason.to_string());
        deployment.clone()
    }
...
impl DeploymentRepo for MockDeploymentRepo {
    ...
    async fn mark_failed_provisioning(
            ...
            .find(|d| d.id == id && d.status == "provisioning")
        {
            Some(d) => {
                ...
                d.failure_reason = failure_reason.map(str::to_owned);
    ...
    async fn update_provisioning(
        ...
        self.update_provisioning_calls
...
impl MockInvoiceRepo {
    ...
    pub fn set_hosted_invoice_url_for_test(&self, id: Uuid, hosted_invoice_url: Option<&str>) {
        let mut invoices = self.invoices.lock().unwrap();
        let invoice = invoices
            .iter_mut()
            .find(|invoice| invoice.id == id)
            .expect("invoice must exist before overriding hosted_invoice_url");
        invoice.hosted_invoice_url = hosted_invoice_url.map(str::to_string);
    }
}
...
pub struct FailableEmailService {
    ...
    broadcast_delay_ms: AtomicU64,
    invalid_request_observed_attempt_count: AtomicUsize,
    attempted_recipients: Mutex<Vec<String>>,
    broadcast_failures: Mutex<HashMap<String, String>>,
    invalid_broadcast_requests: Mutex<HashMap<String, String>>,
...
impl FailableEmailService {
    pub fn new(delegate: Arc<dyn EmailService>) -> Self {
        Self {
            ...
            broadcast_delay_ms: AtomicU64::new(0),
            invalid_request_observed_attempt_count: AtomicUsize::new(0),
            attempted_recipients: Mutex::new(Vec::new()),
            broadcast_failures: Mutex::new(HashMap::new()),
            invalid_broadcast_requests: Mutex::new(HashMap::new()),
    ...
    pub fn invalid_request_observed_attempt_count(&self) -> Option<usize> {
        match self
    ...
    pub fn invalidate_broadcast_recipient(&self, email: &str, reason: &str) {
        self.invalid_broadcast_requests
            .lock()
            .unwrap()
            .insert(email.to_string(), reason.to_string());
    }

    pub fn set_broadcast_delay_ms(&self, delay_ms: u64) {
        self.broadcast_delay_ms.store(delay_ms, Ordering::SeqCst);
    }
...
impl EmailService for FailableEmailService {
    ...
    async fn send_invoice_ready_email(
        ...
        self.delegate
            .send_invoice_ready_email(to, invoice_id, invoice_url, pdf_url, dispatch_source)
            .await
    }
    ...
    async fn send_broadcast_email(
        ...
        let delay_ms = self.broadcast_delay_ms.load(Ordering::SeqCst);
        if delay_ms > 0 {
        ...
        if let Some(reason) = self
            .invalid_broadcast_requests
            .lock()
            .unwrap()
            .get(to)
            .cloned()
        {
            let observed_attempt_count = self.broadcast_attempts.load(Ordering::SeqCst);
            self.invalid_request_observed_attempt_count
                .store(observed_attempt_count, Ordering::SeqCst);
            return Err(EmailError::InvalidRequest(reason));
        }
...
mod failable_email_service_tests {
    ...
    async fn failable_email_service_records_invalid_request_after_in_flight_attempts_start() {
        ...
        service.set_broadcast_delay_ms(10);
        service.invalidate_broadcast_recipient(
            "invalid-broadcast@example.com",
            "forced invalid broadcast request",
        );

        let (invalid_result, valid_result) = tokio::join!(
            service.send_broadcast_email(
                "invalid-broadcast@example.com",
                "subject",
                None,
                Some("body")
        ...
        assert!(
            matches!(invalid_result, Err(EmailError::InvalidRequest(_))),
            "invalid recipient should return InvalidRequest"
        );
        valid_result.expect("valid in-flight recipient should still complete");
        assert_eq!(
            service.invalid_request_observed_attempt_count(),
            Some(2),
            "InvalidRequest observation should include work already in flight"
        );
        assert_eq!(
            service.attempted_recipients(),
            vec![
                "invalid-broadcast@example.com".to_string(),
                "valid-broadcast@example.com".to_string(),
            ]
        );
    }
}
...
pub struct MockTenantRepo {
    ...
    fail_next_set_vm_id: AtomicBool,
    fail_next_set_tier_to_active: AtomicBool,
}
...
impl MockTenantRepo {
    pub fn new() -> Self {
        Self {
            ...
            fail_next_set_vm_id: AtomicBool::new(false),
            fail_next_set_tier_to_active: AtomicBool::new(false),
        }
    }
    ...
    pub fn fail_next_set_tier_to_active(&self) {
        self.fail_next_set_tier_to_active
            .store(true, Ordering::SeqCst);
    }

    pub fn fail_next_set_vm_id(&self) {
        self.fail_next_set_vm_id.store(true, Ordering::SeqCst);
    }
...
impl TenantRepo for MockTenantRepo {
    ...
    async fn create_lifecycle_intent(
        ...
        let mut tenant = self.create(customer_id, tenant_id, deployment_id).await?;
        tenant.tier = tier.to_string();
        let mut tenants = self.tenants.lock().unwrap();
        if let Some(stored) = tenants
            .iter_mut()
            .find(|stored| stored.customer_id == customer_id && stored.tenant_id == tenant_id)
        {
            stored.tier = tenant.tier.clone();
        }
        Ok(tenant)
    }
    ...
    async fn publish_delete_lifecycle_intent(
        ...
        if expected_identity.tier != "active" {
        ...
        let matches_identity = tenant.deployment_id == expected_identity.deployment_id
            && tenant.vm_id == expected_identity.vm_id
            && tenant.tier == "active"
            && tenant.cold_snapshot_id == expected_identity.cold_snapshot_id
            && tenant.service_type == expected_identity.service_type;
        if !matches_identity {
            return Err(RepoError::Conflict("destination_changed".into()));
        }

        tenant.tier = "deleting".to_string();
        Ok(tenant.clone())
    }
    ...
    async fn publish_lifecycle_placement(
        ...
        let matches_identity = tenant.deployment_id == expected_identity.deployment_id
            && tenant.vm_id == expected_identity.vm_id
            && tenant.tier == expected_identity.tier
            && tenant.cold_snapshot_id == expected_identity.cold_snapshot_id
            && tenant.service_type == expected_identity.service_type;
        if !matches_identity {
            return Err(RepoError::Conflict("destination_changed".into()));
    ...
    async fn remove_lifecycle_intent(
        ...
        let mut tenants = self.tenants.lock().unwrap();
        let Some(index) = tenants
            .iter()
            .position(|t| t.customer_id == customer_id && t.tenant_id == tenant_id)
        else {
            return Ok(false);
        };
    ...
    async fn set_tier(
        ...
        if tier == "active"
            && self
                .fail_next_set_tier_to_active
                .swap(false, Ordering::SeqCst)
        {
            return Err(RepoError::Other(
                "injected set_tier active failure".to_string(),
            ));
        }
