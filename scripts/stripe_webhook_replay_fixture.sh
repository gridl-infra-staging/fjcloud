...
print_usage() {
    cat <<'USAGE' >&2
Usage:
  stripe_webhook_replay_fixture.sh [--check|--run] [--allow-staging-target] [--env-file <path>] [--target-url <url>] [--timestamp <unix-seconds>] [--event-id <id>] [--event-type <type>] [--invoice-id <id>] [--next-payment-attempt <unix-seconds|null>] [--attempt-count <integer>]
...
Modes:
  ...
  --event-type  Supported values: customer.updated (default), invoice.payment_failed, invoice.payment_succeeded.
USAGE
}
...
resolve_timestamp_and_event_id() {
    ...
    if [ -n "$EVENT_ID_OVERRIDE" ]; then
        ...
        event_type_component="${EVENT_TYPE_OVERRIDE:-$EVENT_TYPE_VALUE}"
        event_type_component="${event_type_component//./_}"
        invoice_component="${INVOICE_ID_OVERRIDE:-none}"
        next_attempt_component="${NEXT_PAYMENT_ATTEMPT_OVERRIDE:-none}"
        attempt_count_component="${ATTEMPT_COUNT_OVERRIDE:-none}"
        EVENT_ID_VALUE="evt_replay_${TIMESTAMP_VALUE}_${event_type_component}_${invoice_component}_${next_attempt_component}_${attempt_count_component}"
    fi
...
resolve_event_contract() {
    ...
    case "$EVENT_TYPE_VALUE" in
        ...
        invoice.payment_failed)
            ...
            if [[ "$NEXT_PAYMENT_ATTEMPT_OVERRIDE" != "null" && ! "$NEXT_PAYMENT_ATTEMPT_OVERRIDE" =~ ^[0-9]+$ ]]; then
                append_step "resolve_event_contract" false "--next-payment-attempt must be a unix-seconds integer or null"
                set_outcome "failed" "event_contract_invalid" "--next-payment-attempt must be a unix-seconds integer or null" 1
                return 1
            fi
        ...
        invoice.payment_succeeded)
            if [ -z "$INVOICE_ID_OVERRIDE" ]; then
                append_step "resolve_event_contract" false "--invoice-id is required when --event-type=invoice.payment_succeeded"
                set_outcome "failed" "event_contract_invalid" "--invoice-id is required when --event-type=invoice.payment_succeeded" 1
                return 1
            fi
            append_step "resolve_event_contract" true "using invoice.payment_succeeded recovery payload contract"
            return 0
            ;;
        *)
            append_step "resolve_event_contract" false "unsupported --event-type value"
            set_outcome "failed" "event_contract_invalid" "unsupported --event-type value; allowed: customer.updated, invoice.payment_failed, invoice.payment_succeeded" 1
            return 1
            ;;
    esac
}
...
if event_type == "invoice.payment_failed":
    next_payment_attempt_value = None if next_payment_attempt == "null" else int(next_payment_attempt)
    payload = {
        "id": event_id,
        ...
        "data": {
            "object": {
                "id": invoice_id,
                "next_payment_attempt": next_payment_attempt_value,
