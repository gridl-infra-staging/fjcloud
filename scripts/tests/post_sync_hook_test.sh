...
test_post_sync_hook_strip_then_dirty_commit_push_contract() {
    ...
    local mode_line
    local commit_line
    add_line="$(grep -n "git:-C $target_repo add -A" "$call_log" | head -n 1 | cut -d: -f1)"
    mode_line="$(grep -n "git:-C $target_repo update-index --chmod=+x scripts/seed_local.sh" "$call_log" | head -n 1 | cut -d: -f1)"
    commit_line="$(grep -n "git:-C $target_repo commit -m" "$call_log" | head -n 1 | cut -d: -f1)"
    if [[ "$add_line" -lt "$mode_line" && "$mode_line" -lt "$commit_line" ]]; then
        pass "executable mode must be staged after add and before commit"
    else
        fail "executable mode must be staged after add and before commit"
    fi
