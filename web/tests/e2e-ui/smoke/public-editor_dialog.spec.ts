...
async function expectClassTokens(
	classNameSource: Locator | (() => Promise<string | null>),
	expectedTokens: string[]
): Promise<void> {
	const className =
		typeof classNameSource === 'function'
			? ((await classNameSource()) ?? '')
			: ((await classNameSource.getAttribute('class')) ?? '');
	const actualTokens = new Set(className.split(/\s+/).filter(Boolean));

	for (const expectedToken of expectedTokens) {
		expect(actualTokens, `expected class token "${expectedToken}" in "${className}"`).toContain(
			expectedToken
		);
	}
}
...
test('submits all field types and echoes normalized payload', async ({ page }) => {
	...
	await expectClassTokens(page.getByRole('dialog'), [
		'max-w-2xl',
		'rounded-lg',
		'bg-white',
		'shadow-xl'
	]);
	await expectClassTokens(page.getByTestId('editor-dialog-footer-actions'), [
		'flex',
		'justify-end',
		'gap-3',
		'border-t',
...
test('enforces modal focus, dirty confirm, pending save, reject alert, and successful reopen', async ({
	...
	await expect(page.getByRole('dialog')).toBeVisible();
	await page.getByTestId('demo-resolve-pending-save').evaluate((element) => {
	...
	await page.getByTestId('editor-dialog-save').click();
	await expect(page.getByTestId('editor-dialog-server-error')).toContainText('Demo rejected save');
	await page.getByTestId('editor-dialog-cancel').click();
	await page.getByTestId('editor-dialog-discard').click();
	await expect(page.getByRole('dialog')).toBeHidden();
