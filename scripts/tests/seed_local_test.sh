...
test_seed_local_sources_contract_and_shared_db_helpers() {
    ...
    assert_contains "$script_text" 'source "$SCRIPT_DIR/lib/local_seed_contract.sh"' \
        "seed_local.sh should source the local seed contract library"
    assert_contains "$script_text" 'source "$SCRIPT_DIR/lib/local_db_access.sh"' \
        "seed_local.sh should source the shared local DB access helper"
    assert_not_contains "$script_text" "require_local_database_access() {" \
        "seed_local.sh should not retain a private require_local_database_access definition"
    assert_not_contains "$script_text" "run_local_psql() {" \
        "seed_local.sh should not retain a private run_local_psql definition"
}
...
test_seeds_multi_region_inventory_and_multi_user_indexes() {
    ...
    assert_contains "$curl_calls" "/indexes/test-index/batch" \
        "should seed docs for shared us-east-1 index"
    assert_contains "$curl_calls" "/indexes/test-index-eu/batch" \
        "should seed docs for shared eu-west-1 index"
    assert_contains "$curl_calls" "/indexes/test-index-eu2/batch" \
        "should seed docs for shared eu-central-1 index"
    assert_contains "$curl_calls" "/indexes/free-test-index/batch" \
        "should seed docs for the free-plan index"
    assert_contains "$curl_calls" '"createIfNotExists":true' \
        "should repair a missing engine index through the canonical batch contract"
    assert_contains "$curl_calls" "/indexes/test-index-eu2/search" \
        "should prove the repaired regional index returns a known seeded document"
    ...
    assert_contains "$sql" "temporary_seed_vms AS" \
        "should identify API-created e2e-seed VM rows after canonical seed placement"
    assert_contains "$sql" "customer_tenants.vm_id IN (SELECT id FROM temporary_seed_vms)" \
        "should clear tenant references to temporary e2e-seed VM rows"
    assert_contains "$sql" "hostname LIKE :'synthetic_vm_hostname_like'" \
        "should use the shared synthetic VM pattern when cleaning temporary seed VMs"
    assert_contains "$sql" "status = 'decommissioned'" \
        "should decommission temporary e2e-seed VM rows so dev_state_audit passes after local_demo"
...
test_fails_when_engine_document_seed_fails() {
    local tmp_dir
    tmp_dir=$(mktemp -d)
    trap 'restore_repo_env_file "'"$tmp_dir"'/.env.local.backup"; rm -rf "'"$tmp_dir"'"' RETURN
    ...
    output=$(
        ...
        DATABASE_URL="postgres://griddle:griddle_local@127.0.0.1:15432/fjcloud_dev" \
        FLAPJACK_URL="http://localhost:7701" \
        MOCK_SEED_DOCUMENT_BATCH_FAIL="1" \
        bash "$REPO_ROOT/scripts/seed_local.sh" 2>&1
    ) || exit_code=$?

    assert_eq "$exit_code" "1" "should fail when a regional engine rejects document seeding"
    assert_contains "$output" "Document seed failed for test-index with HTTP 404" \
        "should identify the false-green engine seed failure"
    if [[ "$output" == *"Local dev environment seeded successfully"* ]]; then
        fail "should not report local seed success after an engine batch failure"
    else
        pass "should not report local seed success after an engine batch failure"
    fi
}

test_fails_when_seeded_document_is_not_searchable() {
    local tmp_dir
    tmp_dir=$(mktemp -d)
    trap 'restore_repo_env_file "'"$tmp_dir"'/.env.local.backup"; rm -rf "'"$tmp_dir"'"' RETURN
    ...
    output=$(
        ...
        DATABASE_URL="postgres://griddle:griddle_local@127.0.0.1:15432/fjcloud_dev" \
        FLAPJACK_URL="http://localhost:7701" \
        MOCK_SEED_SEARCH_MISSING_DOC="1" \
        SEED_SEARCH_VERIFY_ATTEMPTS="1" \
        bash "$REPO_ROOT/scripts/seed_local.sh" 2>&1
    ) || exit_code=$?

    assert_eq "$exit_code" "1" "should fail when an accepted batch is not searchable"
    assert_contains "$output" "Search verification failed for test-index after 1 attempts" \
        "should identify the index whose known-answer search failed"
    if [[ "$output" == *"Local dev environment seeded successfully"* ]]; then
        fail "should not report local seed success without a known-answer search hit"
    else
        pass "should not report local seed success without a known-answer search hit"
    fi
}
...
test_escapes_seed_payload_fields_and_index_path() {
    ...
    assert_contains "$curl_calls" "http://127.0.0.1:3001/indexes/folder%2Fname/batch" \
