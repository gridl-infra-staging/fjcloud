...
- Given a Merchandising hub with one rule, when the user clicks `Delete` on a row, then the standard-mode dialog opens; clicking Confirm removes the rule.
- Given a Merchandising hub with N rules, when the user clicks `Clear All rules` and types `CLEAR` into the typed-mode dialog, then after Confirm the list renders the empty state.
