...
search_response_has_object_id() {
    ...
    python3 - "$response_body" "$expected_object_id" <<'PY'
import json
import sys
...
raise SystemExit(
    0
    if any(
        isinstance(hit, dict) and hit.get("objectID") == sys.argv[2]
        for hit in hits
    )
    else 1
)
...
verify_seeded_document_search() {
    ...
    local user_token="$3"
    local attempts="${SEED_SEARCH_VERIFY_ATTEMPTS:-10}"
    local attempt search_response search_code search_body

    for ((attempt = 1; attempt <= attempts; attempt++)); do
        search_response="$(api_call_with_token POST "/indexes/${index_path_component}/search" "$user_token" \
            -d '{"query":"Sample Document 1","hitsPerPage":5}' \
            -w "\n%{http_code}" 2>/dev/null || true)"
        search_code="$(http_response_status "$search_response")"
        search_body="$(http_response_body "$search_response")"

        if [ "$search_code" = "200" ] && search_response_has_object_id "$search_body" "doc-1"; then
            log "Verified search result doc-1 in ${index_name}"
            return 0
        fi
...
SEED_USER_NAME="${SEED_USER_NAME:-$LOCAL_SEED_SHARED_USER_NAME}"
SEED_USER_EMAIL="${SEED_USER_EMAIL:-$LOCAL_SEED_SHARED_USER_EMAIL}"
SEED_USER_PASSWORD="${SEED_USER_PASSWORD:-$LOCAL_SEED_SHARED_USER_PASSWORD}"
SEED_FREE_USER_NAME="${SEED_FREE_USER_NAME:-$LOCAL_SEED_FREE_USER_NAME}"
SEED_FREE_USER_EMAIL="${SEED_FREE_USER_EMAIL:-$LOCAL_SEED_FREE_USER_EMAIL}"
SEED_FREE_USER_PASSWORD="${SEED_FREE_USER_PASSWORD:-$LOCAL_SEED_FREE_USER_PASSWORD}"

SEED_INDEX_NAME="${SEED_INDEX_NAME:-$LOCAL_SEED_PRIMARY_INDEX_NAME}"
SEED_INDEX_REGION="${SEED_INDEX_REGION:-$LOCAL_SEED_PRIMARY_INDEX_REGION}"
SEED_INDEX_TARGETS=()
while IFS= read -r seed_index_target; do
    [ -n "$seed_index_target" ] || continue
    SEED_INDEX_TARGETS+=("$seed_index_target")
done < <(local_seed_index_targets "$SEED_INDEX_NAME" "$SEED_INDEX_REGION")
...
seed_vm_inventory() {
    ...
    escaped_capacity="$(sql_escape_literal "$LOCAL_SEED_VM_CAPACITY_JSON")"
    escaped_current_load="$(sql_escape_literal "$LOCAL_SEED_VM_CURRENT_LOAD_JSON")"
...
if [ "$flapjack_reachable" -eq 1 ]; then
    ...
    for seed_target in "${SEED_INDEX_TARGETS[@]}"; do
        ...
        for i in $(seq 1 5); do
            batch_payload="$(printf '{\"requests\":[{\"action\":\"addObject\",\"createIfNotExists\":true,\"body\":{\"objectID\":\"doc-%s\",\"title\":\"Sample Document %s\",\"body\":\"This is sample document number %s for local development testing.\"}}]}' "$i" "$i" "$i")"
            batch_response="$(api_call_with_token POST "/indexes/${index_path_component}/batch" "$user_token" \
                -d "$batch_payload" -w "\n%{http_code}" 2>/dev/null || true)"
            batch_code="$(http_response_status "$batch_response")"
            batch_body="$(http_response_body "$batch_response")"
            if [ "$batch_code" != "200" ]; then
                die "Document seed failed for ${target_index_name} with HTTP ${batch_code}: ${batch_body}"
            fi
        done
        verify_seeded_document_search "$target_index_name" "$index_path_component" "$user_token"
...
# shellcheck source=lib/local_seed_replicas.sh
source "$SCRIPT_DIR/lib/local_seed_replicas.sh"
