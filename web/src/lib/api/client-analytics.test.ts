...
describe('ApiClient - analytics and experiments', () => {
	...
	describe('debug events endpoints', () => {
		it('POST /indexes/:name/events sends one correlated preview event', async () => {
			const fetch = mockFetch(200, { accepted: true });
			client.setFetch(fetch);
			const event = {
				eventName: 'search_preview_result_opened' as const,
				objectID: 'doc-1',
				position: 21,
				queryID: 'q-123',
			...
			await client.postPreviewEvent('products', event);

			expect(fetch).toHaveBeenCalledWith(`${BASE_URL}/indexes/products/events`, {
				method: 'POST',
				headers: { 'Content-Type': 'application/json', Authorization: 'Bearer my-jwt-token' },
