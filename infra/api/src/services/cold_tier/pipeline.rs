...
impl ColdTierService {
    ...
    pub async fn snapshot_candidate(
        ...
        let cold_intent_identity = candidate.cold_intent_identity();
        let payload = self
        ...
        self.transition_tenant_to_cold_storage(candidate, snapshot_id, &cold_intent_identity)
            .await?;
    ...
    pub async fn handle_snapshot_failure(
        ...
        self.rollback_tenant_snapshot_state(candidate, snapshot_id, error)
            .await;
    ...
    async fn begin_snapshot_record(
        ...
        let snapshot = self
            .guarded_target_mutation(
                candidate.customer_id,
                &candidate.tenant_id,
                Some(&candidate.catalog_identity),
                || async {
                    if let Some(hook) = &self.begin_snapshot_pause_hook {
                        hook().await;
                    }
                    self.tenant_repo
                    ...
                    self.cold_snapshot_repo.set_exporting(snapshot.id).await?;
                    Ok(snapshot)
                },
            )
            .await?;
    ...
    async fn transition_tenant_to_cold_storage(
        ...
        self.guarded_target_mutation(
            ...
            || async {
                if let Some(hook) = &self.cold_publication_pause_hook {
                    hook().await;
                }
                self.tenant_repo
    ...
    async fn catalog_identity_for_candidate(
        &self,
        candidate: &ColdTierCandidate,
    ) -> Result<crate::repos::CatalogLifecycleTargetIdentity, ColdTierError> {
        let tenant = self
            .tenant_repo
            .find_raw(candidate.customer_id, &candidate.tenant_id)
            .await
            .map_err(|e| ColdTierError::Repo(e.to_string()))?
            .ok_or_else(|| ColdTierError::Repo("tenant not found".to_string()))?;
    ...
    async fn rollback_tenant_snapshot_state(
        ...
        let Some(expected_identity) = self
            .cold_intent_identity_for_rollback(candidate, snapshot_id)
            .await
        else {
            warn!(
                customer_id = %candidate.customer_id,
                tenant_id = %candidate.tenant_id,
                "skipping cold-tier rollback because catalog identity no longer matches this snapshot intent"
            );
            return;
        };
        ...
        let rollback = self
            .guarded_target_mutation(
                ...
                Some(&expected_identity),
                || async {
                    if let Some(hook) = &self.rollback_pause_hook {
                        hook().await;
                    }
                    if let Some(snapshot_id) = snapshot_id {
                        self.cold_snapshot_repo
                            .set_failed(snapshot_id, error)
                            .await?;
                    }
                    self.tenant_repo
        ...
        if let Err(repo_error) = rollback {
            warn!(
                customer_id = %candidate.customer_id,
                tenant_id = %candidate.tenant_id,
                error = %repo_error,
                "failed guarded cold-tier rollback"
            );
        }
    }

    async fn cold_intent_identity_for_rollback(
        &self,
        candidate: &ColdTierCandidate,
        snapshot_id: Option<Uuid>,
    ) -> Option<crate::repos::CatalogLifecycleTargetIdentity> {
        let current_identity = self.catalog_identity_for_candidate(candidate).await.ok()?;
        let begin_identity = candidate.cold_intent_identity();
        if current_identity == begin_identity {
            return Some(begin_identity);
        }

        let published_identity = candidate.published_cold_identity(snapshot_id?);
        (current_identity == published_identity).then_some(published_identity)
    }
...
mod tests {
    ...
    fn test_candidate() -> ColdTierCandidate {
        ColdTierCandidate {
            ...
            catalog_identity: crate::repos::CatalogLifecycleTargetIdentity {
