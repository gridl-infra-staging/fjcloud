...
| `FJCLOUD_ALGOLIA_MIGRATION_ENABLED` | No       | `false`                      | Runtime value generated from SSM `/fjcloud/{env}/algolia_migration_enabled`. Terraform provisions the parameter at `false`; `ops/scripts/set_algolia_migration_availability.sh` is the only post-provision mutation path. During the Stage 1 fail-closed state, `/migration/algolia/availability` still returns the typed unavailable payload even when this is `true`; after the replacement importer lands, `true` will admit that importer flow. |
