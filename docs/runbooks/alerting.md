...
For a planned deploy freeze expected to exceed `DRIFT_MAX_AGE_HOURS`, silence the schedule explicitly in `gridl-infra-staging/fjcloud` with `gh workflow disable deploy_currency.yml -R gridl-infra-staging/fjcloud`, record the freeze window in the incident or change log, then re-enable it with `gh workflow enable deploy_currency.yml -R gridl-infra-staging/fjcloud` and run a manual dispatch after the freeze. Do not silence this alarm by deleting `DISCORD_WEBHOOK_URL` or weakening the canary rule.
