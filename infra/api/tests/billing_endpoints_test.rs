...
use common::{
    ...
    mock_rate_card_repo, mock_repo, mock_stripe_service, mock_usage_repo, mock_webhook_event_repo,
    test_state_all_with_stripe, TestStateBuilder, TEST_ADMIN_KEY, TEST_WEBHOOK_SECRET,
};
...
fn webhook_request_with_signature(body: &str, signature: &str) -> Request<Body> {
...
fn local_stripe_webhook_app(
    ...
    webhook_event_repo: Arc<common::MockWebhookEventRepo>,
) -> axum::Router {
    ...
    state.webhook_event_repo = webhook_event_repo
        as Arc<dyn api::repos::webhook_event_repo::WebhookEventRepo + Send + Sync>;
...
async fn webhook_invalid_signature_returns_400() {
    ...
    let app = test_app_with_stripe(mock_repo(), mock_invoice_repo(), stripe_svc);

    let resp = app
        .oneshot(webhook_request(r#"{"id":"evt_1","type":"test","data":{}}"#))
        .await
        .unwrap();
...
#[tokio::test]
async fn webhook_unknown_event_returns_200() {
    let app = test_app_with_stripe(mock_repo(), mock_invoice_repo(), mock_stripe_service());

    let resp = app
        .oneshot(webhook_request(
            r#"{"id":"evt_unknown","type":"customer.updated","data":{"object":{}}}"#,
        ))
        .await
        .unwrap();
...
#[tokio::test]
async fn webhook_duplicate_event_returns_200() {
    let webhook_repo = mock_webhook_event_repo();

    // Use the standard helper, then override webhook_event_repo so both app
    // instances share the same mock — proving deduplication works.
    let mut state = test_state_all_with_stripe(
        mock_repo(),
        mock_deployment_repo(),
        mock_usage_repo(),
    ...
    state.webhook_event_repo = std::sync::Arc::clone(&webhook_repo)
        as std::sync::Arc<dyn api::repos::webhook_event_repo::WebhookEventRepo + Send + Sync>;

    let app1 = api::router::build_router(state.clone());
    let payload = r#"{"id":"evt_dup","type":"customer.updated","data":{"object":{}}}"#;

    // First call — new event
    let resp1 = app1.oneshot(webhook_request(payload)).await.unwrap();
    assert_eq!(resp1.status(), StatusCode::OK);

    // Second call — duplicate, should still return 200
    let app2 = api::router::build_router(state);
    let resp2 = app2.oneshot(webhook_request(payload)).await.unwrap();
    assert_eq!(resp2.status(), StatusCode::OK);

    // Idempotency: event should have been recorded only once
    assert_eq!(
        webhook_repo.event_count(),
        1,
        "duplicate event should be deduplicated — only 1 record in webhook_event_repo"
    );
}
...
#[tokio::test]
async fn webhook_unsupported_event_records_once_and_does_not_mutate_domain_state() {
    let customer_repo = mock_repo();
    let invoice_repo = mock_invoice_repo();
    let subscription_repo = common::mock_subscription_repo();
    let webhook_repo = mock_webhook_event_repo();
    ...
    assert_eq!(
        webhook_repo.event_count(),
        1,
        "unsupported events should still be inserted once for idempotency tracking"
    );
...
#[tokio::test]
async fn webhook_unsupported_event_replay_deduplicates_and_keeps_domain_state_unchanged() {
    let customer_repo = mock_repo();
    let invoice_repo = mock_invoice_repo();
    let subscription_repo = common::mock_subscription_repo();
    let webhook_repo = mock_webhook_event_repo();
    ...
    assert_eq!(
        webhook_repo.event_count(),
        1,
        "replayed unsupported events must stay deduplicated in webhook_event_repo"
    );
...
#[tokio::test]
async fn webhook_payment_succeeded_marks_invoice_paid() {
    let customer_repo = mock_repo();
    let invoice_repo = mock_invoice_repo();
    let customer = customer_repo.seed("Acme", "acme@example.com");
    ...
    invoice_repo
        .set_stripe_fields(invoice.id, "in_stripe_pay", "https://stripe.com/inv", None)
        .await
        .unwrap();
    ...
    let payload = format!(
        r#"{{"id":"evt_pay","type":"invoice.payment_succeeded","data":{{"object":{{"id":"in_stripe_pay"}}}}}}"#
    );
...
#[tokio::test]
async fn webhook_subscription_created_and_reconciles_local_row() {
    let customer_repo = mock_repo();
    let invoice_repo = mock_invoice_repo();
    let subscription_repo = common::mock_subscription_repo();
    ...
    let payload = serde_json::json!({
        ...
        "data": {
            "object": {
                ...
                "customer": customer.stripe_customer_id.as_ref().expect("seeded customer has stripe id"),
                "status": "active",
    ...
    let created = subscription_repo
        .find_by_stripe_id("sub_webhook_created")
        .await
        .unwrap()
    ...
    assert_eq!(created.cancel_at_period_end, false);
}
...
#[tokio::test]
async fn webhook_subscription_updated_changes_local_state() {
    let customer_repo = mock_repo();
    let invoice_repo = mock_invoice_repo();
    let subscription_repo = common::mock_subscription_repo();
    let stripe_service = mock_stripe_service();
    ...
    let existing = seed_subscription_row(
        &subscription_repo,
        customer.id,
        customer
            .stripe_customer_id
    ...
    let updated = subscription_repo
        .find_by_stripe_id("sub_webhook_updated")
        .await
        .unwrap()
...
#[tokio::test]
async fn webhook_subscription_deleted_marks_canceled() {
    let customer_repo = mock_repo();
    let invoice_repo = mock_invoice_repo();
    let subscription_repo = common::mock_subscription_repo();
    let stripe_service = mock_stripe_service();
    ...
    let updated = subscription_repo
        .find_by_stripe_id("sub_webhook_deleted")
        .await
        .unwrap()
...
#[tokio::test]
async fn webhook_dunning_payment_action_required_marks_subscription_past_due() {
    let customer_repo = mock_repo();
    let invoice_repo = mock_invoice_repo();
    let subscription_repo = common::mock_subscription_repo();
    let stripe_service = mock_stripe_service();
    ...
    invoice_repo.finalize(invoice.id).await.unwrap();
    invoice_repo
        .set_stripe_fields(invoice.id, "in_action", "https://stripe.com/inv", None)
        .await
        .unwrap();
    ...
    let payload = format!(
        r#"{{"id":"evt_action_required","type":"invoice.payment_action_required","data":{{"object":{{"id":"in_action","customer":"{}"}}}}}}"#,
        customer.stripe_customer_id.as_ref().unwrap()
    );
...
#[tokio::test]
async fn webhook_payment_failed_exhausted_updates_subscription_to_canceled() {
    let customer_repo = mock_repo();
    let invoice_repo = mock_invoice_repo();
    let subscription_repo = common::mock_subscription_repo();
    let stripe_service = mock_stripe_service();
    ...
    invoice_repo.finalize(invoice.id).await.unwrap();
    invoice_repo
        .set_stripe_fields(invoice.id, "in_fail", "https://stripe.com/inv", None)
        .await
        .unwrap();
    ...
    let payload = r#"{"id":"evt_fail_cancel","type":"invoice.payment_failed","data":{"object":{"id":"in_fail","next_payment_attempt":null}}}"#;
    let resp = app.oneshot(webhook_request(payload)).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);

    let sub = subscription_repo
        .find_by_stripe_id("sub_delinquent")
        .await
        .unwrap()
        .expect("subscription should exist");
...
#[tokio::test]
async fn webhook_charge_refunded_marks_invoice_refunded() {
    let customer_repo = mock_repo();
    let invoice_repo = mock_invoice_repo();
    let customer = customer_repo.seed("Acme", "acme@example.com");
    ...
    let payload = format!(
        r#"{{"id":"evt_refund","type":"charge.refunded","data":{{"object":{{"id":"ch_123","invoice":"in_stripe_refund"}}}}}}"#
    );
    let resp = app.oneshot(webhook_request(&payload)).await.unwrap();
