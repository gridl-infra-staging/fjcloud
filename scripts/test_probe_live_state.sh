...
JSON
        ...
        if [ "${1:-}" = "ssm" ] && [ "${2:-}" = "get-parameter" ]; then
            ...
            if [ "$param_name" = "/fjcloud/staging/admin_key" ]; then
                # AWS identity capability probe target. The value is a decrypted
                # SecureString — the capable arm prints a sentinel to stdout to
                # prove aws_identity_require_capability routes it to /dev/null and
                # it never reaches the row. The failure arms embed secret-token and
                # multiline sentinels in stderr to prove the fixed-finding path
                # never copies raw error text.
                case "${AWS_ADMIN_KEY_CAPABILITY:-capable}" in
                    access_denied)
                        printf 'An error occurred (AccessDeniedException) when calling the GetParameter operation: User arn:aws:iam::ACCOUNT_PLACEHOLDER:user/test is not authorized to perform ssm:GetParameter on resource /fjcloud/staging/admin_key\nMULTILINE_SENTINEL_c0ffee must never leak\n' >&2
                        exit 254
                        ;;
                    not_found)
                        printf 'An error occurred (ParameterNotFound) when calling the GetParameter operation: parameter /fjcloud/staging/admin_key not found\n' >&2
                        exit 254
                        ;;
                    probe_failed)
                        printf 'An error occurred (ThrottlingException) when calling the GetParameter operation: rate exceeded RAWERR_AKIALIKE_SENTINEL_do_not_leak RAWERR_SKLIVELIKE_SENTINEL_do_not_leak\nMULTILINE_SENTINEL_c0ffee must never leak\n' >&2
                        exit 254
                        ;;
                    *)
                        printf 'SSMVALUE_SENTINEL_admin_key_plaintext_must_not_leak\n'
                        exit 0
                        ;;
                esac
            fi
...
assert_file_not_contains() {
    local abs_path="$1" forbidden_substr="$2" msg="$3"
    ...
    if grep -Fq -- "$forbidden_substr" "$abs_path"; then
        fail "$msg (unexpected substring '$forbidden_substr' in '$abs_path')"
    else
        pass "$msg"
    fi
}
...
validate_live_state_artifact() {
    ...
    local -a pattern_labels=(
        'document title'
        'generator line'
        'aws identity capability row heading'
        'stripe row heading'
...
run_aws_identity_capability_probe() {
    # Runs the full probe against the stubbed AWS CLI and echoes the summary path.
    # Args: result_var aws_scenario admin_key_capability [extra_env...]
    local result_variable="$1" aws_scenario="$2" admin_key_capability="$3"
    shift 3
    local output_path primary_secret_path stub_dir
    create_temp_bundle_summary output_path
    primary_secret_path="$(mktemp)"
    stub_dir="$(mktemp -d)"
    register_tmp_path "$primary_secret_path"
    TMP_PATHS+=("$stub_dir")
    create_stubbed_vendor_tools "$stub_dir"
    printf 'PROBE_CAPABILITY_TEST_MARKER=1\n' > "$primary_secret_path"

    (
        for inherited in $(env | sed -n 's/^\(AWS_[A-Za-z0-9_]*\)=.*/\1/p'); do
            unset "$inherited"
        done
        export PATH="${stub_dir}:$PATH"
        export AWS_STUB_SCENARIO="$aws_scenario"
        export GH_STUB_SCENARIO="all_degraded"
        export AWS_ADMIN_KEY_CAPABILITY="$admin_key_capability"
        export FJCLOUD_SECRET_FILE="$primary_secret_path"
        export LIVE_STATE_SKIP_STAGING_RDS=1
        export LIVE_STATE_OUTPUT_PATH="$output_path"
        while [ "$#" -gt 0 ]; do
            export "$1"
            shift
        done
    ...
    printf -v "$result_variable" '%s' "$output_path"
}

assert_capability_no_leak() {
    local summary="$1" raw_file="$2" label="$3"
    local sentinel
    for sentinel in \
        SSMVALUE_SENTINEL_admin_key_plaintext_must_not_leak \
        RAWERR_AKIALIKE_SENTINEL_do_not_leak \
        RAWERR_SKLIVELIKE_SENTINEL_do_not_leak \
        MULTILINE_SENTINEL_c0ffee; do
        assert_file_not_contains "$summary" "$sentinel" \
            "${label}: SUMMARY excludes ${sentinel}"
        assert_file_not_contains "$raw_file" "$sentinel" \
            "${label}: raw artifact excludes ${sentinel}"
    done
}

run_aws_identity_capability_contract() {
    local summary raw_file status manifest
    local case_spec scenario capability admin_cap expected_status

    # Every reachable capability outcome. Fail-closed: OK only for `capable`.
    for case_spec in \
        "healthy access_denied ACTION_REQUIRED" \
        "healthy not_found ACTION_REQUIRED" \
        "healthy probe_failed ACTION_REQUIRED" \
        "all_degraded capable ACTION_REQUIRED" \
        "healthy capable OK"; do
        scenario="${case_spec%% *}"
        admin_cap="$(printf '%s' "$case_spec" | cut -d' ' -f2)"
        expected_status="${case_spec##* }"

        run_aws_identity_capability_probe summary "$scenario" "$admin_cap"
        raw_file="$(dirname "$summary")/aws_identity_capability.txt"
        manifest="$(dirname "$summary")/manifest.txt"

        validate_summary_row_contract "$summary" "aws_identity_capability"
        status="$(extract_vendor_status "$summary" "aws_identity_capability")"
        assert_eq "$status" "$expected_status" \
            "capability scenario=${scenario}/${admin_cap} maps to ${expected_status}"

        # Fail-closed guard: nothing but a real `capable` may render OK.
        if [ "$admin_cap" != "capable" ] || [ "$scenario" != "healthy" ]; then
            assert_ne "$status" "OK" \
                "non-capable capability scenario=${scenario}/${admin_cap} never renders OK"
        fi

        assert_file_exists "$raw_file" \
            "capability scenario=${scenario}/${admin_cap} writes raw artifact"
        assert_file_contains "$manifest" "aws_identity_capability.txt" \
            "capability scenario=${scenario}/${admin_cap} registers raw artifact in manifest"
        assert_file_occurrence_count "$summary" "### aws_identity_capability" "1" \
            "capability scenario=${scenario}/${admin_cap} emits exactly one row heading"

        assert_capability_no_leak "$summary" "$raw_file" "scenario=${scenario}/${admin_cap}"

        # Raw artifact must never persist the raw AWS error nor the SSM value.
        assert_file_not_contains "$raw_file" "ThrottlingException" \
            "scenario=${scenario}/${admin_cap}: raw artifact withholds raw-error text"
    done

    # Operator-local backup paths in a known-shape diagnostic must be reduced to
    # basenames, never persisted as absolute operator paths.
    local output_path primary_secret_path stub_dir backup_path backup_base backup_dir
    create_temp_bundle_summary output_path
    primary_secret_path="$(mktemp)"
    stub_dir="$(mktemp -d)"
    register_tmp_path "$primary_secret_path"
    ...
    create_stubbed_vendor_tools "$stub_dir"
    printf 'PROBE_CAPABILITY_TEST_MARKER=1\n' > "$primary_secret_path"
    backup_path="${primary_secret_path}.bak.20260101T000000Z"
    backup_base="$(basename "$backup_path")"
    backup_dir="$(dirname "$backup_path")"
    register_tmp_path "$backup_path"
    cat > "$backup_path" <<'EOF'
AWS_ACCESS_KEY_ID=backup_candidate_access_key_placeholder
AWS_SECRET_ACCESS_KEY=backup_candidate_secret_placeholder
...
EOF
    ...
    (
        ...
        export PATH="${stub_dir}:$PATH"
        export AWS_STUB_SCENARIO="healthy"
        export GH_STUB_SCENARIO="all_degraded"
        export AWS_ADMIN_KEY_CAPABILITY="access_denied"
        export FJCLOUD_SECRET_FILE="$primary_secret_path"
        export LIVE_STATE_SKIP_STAGING_RDS=1
        export LIVE_STATE_OUTPUT_PATH="$output_path"
        bash "$PROBE_SCRIPT_DEFAULT" >/dev/null 2>&1
    ...
    raw_file="$(dirname "$output_path")/aws_identity_capability.txt"
    assert_file_contains "$output_path" "$backup_base" \
        "insufficient-capability finding keeps discovered backup basename"
    assert_file_not_contains "$output_path" "$backup_dir/" \
        "insufficient-capability finding drops operator-absolute backup directory"
    assert_file_not_contains "$raw_file" "$backup_dir/" \
        "insufficient-capability raw artifact drops operator-absolute backup directory"
}

if [ -n "${PROBE_TEST_CASE:-}" ]; then
    case "$PROBE_TEST_CASE" in
        aws_identity_capability_contract)
            run_aws_identity_capability_contract
            ;;
