...
_run_api_integration_module_test() {
    local binary_name="$1"
    local module_name="$2"
    _integration_cargo_test -p api --test "$binary_name" "${module_name}::"
}
...
run_reliability_sql_guard_test() {
    _wrap_test_check "RELIABILITY_SQL_GUARD_TESTS_PASS" "RELIABILITY_SQL_GUARD_TESTS_FAIL" \
        _run_api_integration_module_test "auth_admin" "reliability_security_test"
}
