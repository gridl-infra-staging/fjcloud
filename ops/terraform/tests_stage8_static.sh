...
assert_contains_active "$iam_deploy_file" '"repo:gridl-infra-staging/fjcloud:ref:refs/heads/main"' "Deploy role trust subject is pinned to staging main branch"
assert_contains_active "$iam_deploy_file" '"repo:gridl-infra-staging@266610945/fjcloud@1304385248:ref:refs/heads/main"' "Deploy role trust subject accepts current staging numeric OIDC subject"
assert_contains_active "$iam_deploy_file" '"repo:gridl-infra-prod/fjcloud:ref:refs/heads/main"' "Deploy role trust subject is pinned to prod main branch"
assert_contains_active "$iam_deploy_file" '"repo:gridl-infra-prod@266611019/fjcloud@1304409978:ref:refs/heads/main"' "Deploy role trust subject accepts current prod numeric OIDC subject"
assert_contains_active "$iam_deploy_file" 'arn:aws:s3:::fjcloud-releases-prod' "Deploy role S3 policy grants prod release bucket root access"
assert_contains_active "$iam_deploy_file" 'arn:aws:s3:::fjcloud-releases-prod/\*' "Deploy role S3 policy grants prod release bucket object access"
