...
import { openIndexDetailTab, recommendationField } from '../../fixtures/index_detail_helpers';
...
test.describe('Recommendations request wire contract', () => {
	test('related-products request body is exact', async ({ page, seedIndex, testRegion }) => {
		...
		await recommendationField(section, 'threshold').fill('4');
		await recommendationField(section, 'maxRecommendations').fill('6');
	...
	test('bought-together request body is exact', async ({ page, seedIndex, testRegion }) => {
		...
		await recommendationField(section, 'threshold').fill('5');
		await recommendationField(section, 'maxRecommendations').fill('10');
		await section.getByRole('button', { name: 'Get Recommendations' }).click();
		await expect.poll(() => captured.length).toBe(1);
		expect(captured[0]).toEqual({
			requests: [
	...
	test('looking-similar request body is exact', async ({ page, seedIndex, testRegion }) => {
		...
		await recommendationField(section, 'objectId').fill(PRIMARY_OBJECT_ID);
		await recommendationField(section, 'threshold').fill('7');
		await recommendationField(section, 'maxRecommendations').fill('11');
		await section.getByRole('button', { name: 'Get Recommendations' }).click();
		await expect.poll(() => captured.length).toBe(1);
		expect(captured[0]).toEqual({
			requests: [
	...
	test('trending-items request body is exact', async ({ page, seedIndex, testRegion }) => {
		...
		await recommendationField(section, 'threshold').fill('2');
		await recommendationField(section, 'maxRecommendations').fill('4');
		await section.getByRole('button', { name: 'Get Recommendations' }).click();
		await expect.poll(() => captured.length).toBe(1);
		expect(captured[0]).toEqual({
			requests: [
	...
	test('trending-facets request body is exact', async ({ page, seedIndex, testRegion }) => {
		...
		await recommendationField(section, 'facetName').fill(FACET_NAME);
		await recommendationField(section, 'facetValue').fill(FACET_VALUE);
		await recommendationField(section, 'threshold').fill('3');
		await recommendationField(section, 'maxRecommendations').fill('5');
		await section.getByRole('button', { name: 'Get Recommendations' }).click();
		await expect.poll(() => captured.length).toBe(1);
		expect(captured[0]).toEqual({
			requests: [
