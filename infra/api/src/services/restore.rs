...
use crate::repos::{
    CatalogLifecycleTargetIdentity, ColdSnapshotRepo, RepoError, RestoreJobRepo, TenantRepo,
    VmInventoryRepo,
};
...
impl RestoreService {
    ...
    /// Attach a deterministic pause hook that fires while the lifecycle guard is
    /// held, just before the guarded restore mutation runs. Test-only seam used
    /// to prove an open restore service window excludes catalog admission.
    pub fn with_guard_pause_hook_for_tests(mut self, hook: LifecycleGuardPauseHook) -> Self {
        self.guard_pause_hook = Some(hook);
        self
    }
    ...
    pub async fn initiate_restore(
        ...
        let expected_identity = identity_from_tenant(&tenant);

        // 8. Publish the durable restore intent.
        let job = self
            .guarded_target_mutation(customer_id, tenant_id, Some(&expected_identity), || async {
                let job = self
                    .restore_job_repo
    ...
    pub async fn execute_restore_inner(&self, job_id: Uuid) -> Result<(), RestoreError> {
        let job = self
            .restore_job_repo
            .get(job_id)
            .await
        ...
        let expected_identity = self.restore_intent_identity(&job).await?;
    ...
    pub async fn handle_restore_failure(&self, job_id: Uuid, error: &str) {
        ...
        if let Ok(expected_identity) = self.restore_failure_expected_identity(&job).await {
            let _ = self
    ...
    async fn restore_intent_identity(
        &self,
        job: &crate::models::restore_job::RestoreJob,
    ) -> Result<CatalogLifecycleTargetIdentity, RestoreError> {
        let tenant = self
            .tenant_repo
            .find_raw(job.customer_id, &job.tenant_id)
            .await
            .map_err(map_repo_error)?
            .ok_or(RestoreError::NotFound)?;
    ...
    async fn restore_failure_expected_identity(
        ...
        Ok(CatalogLifecycleTargetIdentity {
            ...
            cold_snapshot_id: Some(job.snapshot_id),
        ...
        Fut: std::future::Future<Output = Result<T, RepoError>>,
    {
        match &self.lifecycle_lease {
            Some(lease) => lease
                .guarded_mutation(customer_id, tenant_id, expected_identity, || async {
                    if let Some(hook) = &self.guard_pause_hook {
                        hook().await;
                    }
                    mutation().await
                })
...
fn identity_from_tenant(tenant: &CustomerTenant) -> CatalogLifecycleTargetIdentity {
...
fn map_repo_error(error: RepoError) -> RestoreError {
    match error {
        RepoError::Conflict(message) if message == "destination_conflict" => {
            RestoreError::DestinationConflict
        }
        RepoError::Conflict(message) if message == "destination_changed" => {
            RestoreError::DestinationChanged
        }
        RepoError::NotFound => RestoreError::NotFound,
        other => RestoreError::Repo(other.to_string()),
    }
}
