...
use tokio::sync::{Mutex as AsyncMutex, MutexGuard as AsyncMutexGuard, Notify, Semaphore};
...
fn async_process_env_lock() -> &'static AsyncMutex<()> {
...
impl EnvVarGuard {
    ...
    fn set_then_unset(key: &'static str, value: &str) -> Self {
        let old_value = std::env::var(key).ok();
        std::env::set_var(key, value);
        std::env::remove_var(key);
        Self { key, old_value }
...
struct ProxyTestSetup {
    // Field order matters: restore LOCAL_DEV_FLAPJACK_URL before releasing the
    // process-wide env lock.
    _local_dev_url: EnvVarGuard,
    _env_guard: AsyncMutexGuard<'static, ()>,
    jwt: String,
    app: axum::Router,
...
async fn setup_proxy_test() -> ProxyTestSetup {
    let env_guard = async_process_env_lock().lock().await;
    let local_dev_url = EnvVarGuard::unset("LOCAL_DEV_FLAPJACK_URL");
    setup_proxy_test_with_locked_env(env_guard, local_dev_url).await
}

async fn setup_proxy_test_with_seeded_local_dev_flapjack_url(
    local_dev_flapjack_url: &str,
) -> ProxyTestSetup {
    let env_guard = async_process_env_lock().lock().await;
    let local_dev_url =
        EnvVarGuard::set_then_unset("LOCAL_DEV_FLAPJACK_URL", local_dev_flapjack_url);
    setup_proxy_test_with_locked_env(env_guard, local_dev_url).await
}

#[allow(clippy::await_holding_lock)]
async fn setup_proxy_test_with_locked_env(
    env_guard: AsyncMutexGuard<'static, ()>,
    local_dev_url: EnvVarGuard,
) -> ProxyTestSetup {
...
async fn setup_empty_region_auto_provision_with_vm_provisioner(
    vm_provisioner: Arc<dyn VmProvisioner>,
) -> Option<AutoProvisionRouteCore> {
    // These tests use advisory locks which require a real database connection.
    let pool = advisory_lock_test_pool().await?;
...
async fn setup_empty_region_auto_provision_test() -> Option<EmptyRegionAutoProvisionSetup> {
    let vm_provisioner = Arc::new(api::provisioner::mock::MockVmProvisioner::new());
    let core = setup_empty_region_auto_provision_with_vm_provisioner(
    ...
    Some(EmptyRegionAutoProvisionSetup {
        ...
        vm_provisioner,
        vm_inventory_repo: core.vm_inventory_repo,
        tenant_repo: core.tenant_repo,
...
async fn create_returns_201_with_index_details() {
    let setup = setup_proxy_test_with_seeded_local_dev_flapjack_url("http://localhost:10918").await;
...
#[tokio::test]
async fn create_auto_provisions_shared_vm_when_region_has_no_existing_vms() {
    ...
    let vms = setup
        ...
        .await
        .unwrap();
    assert_eq!(vms.len(), 1, "auto-provision should add one shared VM");
    ...
    assert_eq!(placed_vm_id, vms[0].id);
}
...
/// Regression (prod customer-loop canary, 2026-05-25): an existing active
/// shared VM can emit a single transient `Unreachable` (proxy GC pause /
/// momentary network blip). That must be retried rather than surfaced to the
/// customer as a premature 503 — the canary's `create_index` step hit exactly
/// this and the canary went into ALARM. After the retry the create must
/// succeed (201).
#[tokio::test]
async fn create_existing_shared_vm_retries_transient_unreachable_then_succeeds() {
    let setup = setup_proxy_test().await;
    setup
        .http_client
        .push_error(ProxyError::Unreachable("connection refused".into()));
    ...
    let resp = setup
        ...
        .oneshot(
            Request::builder()
                ...
                .header("authorization", format!("Bearer {}", setup.jwt))
                .body(Body::from(
                    json!({"name": "existing-transient", "region": "us-east-1"}).to_string(),
                ))
                .unwrap(),
        )
        .await
...
async fn create_returns_503_when_auto_provisioning_fails() {
    ...
    let vm_rows = setup
        ...
        .await
        .unwrap();
    assert_eq!(vm_rows.len(), 0, "failed provisioning must not add VM rows");
    assert_eq!(
...
async fn create_concurrent_capacity_exhaustion_provisions_exactly_one_vm() {
    ...
    let Some(setup) = setup_empty_region_auto_provision_with_vm_provisioner(
        controlled.clone() as Arc<dyn VmProvisioner>
    )
    .await
    else {
        return;
    ...
    let vms = setup
        ...
        .await
        .unwrap();
    assert_eq!(vms.len(), 1, "should keep exactly one newly provisioned VM");
...
async fn get_returns_index_with_stats() {
    ...
    assert_eq!(
        body["status"], "ready",
        "successful live stats must override stale deployment health for a reachable index"
    );
    assert_eq!(body["tier"], "active");
    assert!(body.get("last_accessed_at").is_none());
    assert!(body.get("cold_since").is_none());
...
// ---------------------------------------------------------------------------
// 22. delete preserves catalog for retry when downstream deletion fails
// ---------------------------------------------------------------------------

struct ReadyDeleteIndex {
    app: axum::Router,
    jwt: String,
    http_client: Arc<MockFlapjackHttpClient>,
    customer_id: uuid::Uuid,
...
async fn setup_ready_delete_index(index_name: &'static str) -> ReadyDeleteIndex {
    let customer_repo = mock_repo();
    let deployment_repo = mock_deployment_repo();
    let tenant_repo = mock_tenant_repo();
    let vm_inventory_repo = mock_vm_inventory_repo();
...
#[tokio::test]
async fn delete_index_returns_500_and_preserves_catalog_when_downstream_delete_fails() {
    use api::repos::tenant_repo::TenantRepo;

    let setup = setup_ready_delete_index("doomed-index").await;

    setup
        .http_client
        .push_json_response(500, json!({"error": "internal error"}));

    let resp = setup
        .app
        .oneshot(delete_index_request("doomed-index", &setup.jwt))
        .await
        .unwrap();
    ...
    let result = setup
        .tenant_repo
        .find_by_name(setup.customer_id, "doomed-index")
        .await
        .unwrap();
    assert!(
        result.is_some(),
    ...
    assert_eq!(
        ...
        format!(
            "https://vm-test.flapjack.foo/1/indexes/{}",
            test_flapjack_uid(setup.customer_id, "doomed-index")
        )
    );
}

#[tokio::test]
async fn delete_index_returns_204_when_downstream_delete_reports_404() {
    use api::repos::tenant_repo::TenantRepo;
    ...
    setup
        .http_client
        .push_json_response(404, json!({"error": "index not found"}));
    ...
    assert!(
        result.is_none(),
        "catalog row should be removed when downstream already lacks the index"
    );
...
#[tokio::test]
async fn delete_index_returns_204_when_downstream_delete_is_unreachable() {
    use api::repos::tenant_repo::TenantRepo;
    ...
    assert!(
        result.is_none(),
        "catalog row should be removed when flapjack is unreachable during delete"
    );
...
#[tokio::test]
async fn delete_index_returns_503_and_preserves_catalog_when_downstream_delete_times_out() {
    use api::repos::tenant_repo::TenantRepo;

    let setup = setup_ready_delete_index("doomed-index").await;

    setup.http_client.push_error(ProxyError::Timeout);
    ...
    assert!(
        result.is_some(),
        "index should remain in catalog when flapjack deletion times out"
    );
...
#[tokio::test]
async fn create_index_prefers_current_local_dev_flapjack_vm_over_stale_local_inventory() {
    use api::models::vm_inventory::NewVmInventory;
    use api::repos::tenant_repo::TenantRepo;
    use api::repos::vm_inventory_repo::VmInventoryRepo;

    let _env_guard = async_process_env_lock().lock().await;
    let _local_dev_url = EnvVarGuard::set("LOCAL_DEV_FLAPJACK_URL", "http://localhost:10918");
    ...
    let customer =
        customer_repo.seed_verified_shared_customer("SharedCo", "shared-local@example.com");
    let jwt = create_test_jwt(customer.id);

    let stale_vm = vm_inventory_repo
        .create(NewVmInventory {
            region: "us-east-1".to_string(),
            provider: "aws".to_string(),
            hostname: "stale-local-playwright".to_string(),
            flapjack_url: "http://localhost:10917".to_string(),
            capacity: serde_json::json!({
                "cpu_weight": 4.0,
                "mem_rss_bytes": 8_589_934_592_u64,
                "disk_bytes": 107_374_182_400_u64,
    ...
    let current_vm = vm_inventory_repo
        .create(NewVmInventory {
            region: "us-east-1".to_string(),
            provider: "aws".to_string(),
            hostname: "current-local-playwright".to_string(),
            flapjack_url: "http://127.0.0.1:10918".to_string(),
            capacity: serde_json::json!({}),
        })
        .await
        .unwrap();
    ...
    assert_eq!(
        status,
        StatusCode::CREATED,
        "create should ignore stale local VM inventory from prior Playwright ports: {body}"
    );

    let requests = http_client.take_requests();
    assert_eq!(requests.len(), 1);
    assert_eq!(requests[0].url, "http://127.0.0.1:10918/1/indexes");
    ...
    let tenant = tenant_repo
        ...
        .unwrap()
        .expect("tenant should exist");
    assert_eq!(tenant.vm_id, Some(current_vm.id));
}

#[tokio::test]
async fn create_index_ignores_non_loopback_local_dev_flapjack_url_when_selecting_shared_vm() {
    use api::models::vm_inventory::NewVmInventory;
    use api::repos::tenant_repo::TenantRepo;
    use api::repos::vm_inventory_repo::VmInventoryRepo;

    let _env_guard = async_process_env_lock().lock().await;
    let _local_dev_url = EnvVarGuard::set("LOCAL_DEV_FLAPJACK_URL", "https://example.com:10918");
    ...
    let customer =
        customer_repo.seed_verified_shared_customer("SharedCo", "shared-nonloopback@example.com");
    let jwt = create_test_jwt(customer.id);
    ...
    let shared_vm = vm_inventory_repo
        .create(NewVmInventory {
            region: "us-east-1".to_string(),
            provider: "aws".to_string(),
            hostname: "vm-existing.flapjack.foo".to_string(),
            flapjack_url: "https://shared-existing.flapjack.foo".to_string(),
            capacity: serde_json::json!({
                "cpu_weight": 4.0,
                "mem_rss_bytes": 8_589_934_592_u64,
                "disk_bytes": 107_374_182_400_u64,
    ...
    assert_eq!(
        status,
        StatusCode::CREATED,
        "non-loopback LOCAL_DEV_FLAPJACK_URL should not shadow existing shared VM placement: {body}"
    );
    ...
    assert_eq!(
        requests[0].url,
        "https://shared-existing.flapjack.foo/1/indexes"
    );
    ...
    assert_eq!(
        vm_provisioner.vm_count(),
        0,
        "non-loopback LOCAL_DEV_FLAPJACK_URL should not trigger a new shared VM provision"
    );
}
...
use crate::common::flapjack_proxy_test_support::{setup_ready_index, test_flapjack_uid};
...
async fn update_settings_proxies_to_flapjack() {
    ...
    assert_eq!(requests[0].json_body, Some(expected_proxy_body));
}
...
struct AnalyticsValidationCase {
    name: &'static str,
    uri: &'static str,
...
#[tokio::test]
async fn analytics_rejects_invalid_query_parameters() {
    let cases = [
        AnalyticsValidationCase {
            name: "analytics_rejects_invalid_start_date_format",
            uri: "/indexes/products/analytics/searches?startDate=2026-2-18&endDate=2026-02-25",
            expected_status: StatusCode::BAD_REQUEST,
            expected_error: "YYYY-MM-DD",
        },
        AnalyticsValidationCase {
            name: "analytics_rejects_end_date_before_start_date",
            uri: "/indexes/products/analytics/searches?startDate=2026-02-25&endDate=2026-02-18",
            expected_status: StatusCode::BAD_REQUEST,
            expected_error: "on or after",
        },
        AnalyticsValidationCase {
            name: "analytics_rejects_date_range_over_90_days",
            uri: "/indexes/products/analytics/searches?startDate=2025-10-01&endDate=2026-02-25",
            expected_status: StatusCode::BAD_REQUEST,
            expected_error: "90 days",
        },
        AnalyticsValidationCase {
            name: "analytics_rejects_non_integer_limit",
            uri: "/indexes/products/analytics/searches?startDate=2026-02-18&endDate=2026-02-25&limit=ten",
            expected_status: StatusCode::BAD_REQUEST,
            expected_error: "limit must be an integer",
        },
        AnalyticsValidationCase {
            name: "analytics_rejects_date_range_of_91_days_inclusive",
    ...
    assert_eq!(
        cases.len(),
        5,
        "analytics validation table must stay parity-matched with the deleted tests"
    );
    ...
    for case in cases {
        ...
        assert_eq!(
            ...
            "{} should reject invalid analytics query parameters with {:?}",
        ...
        assert!(
            ...
            "{} should include {:?} in error body, got {body}",
            case.name,
            case.expected_error
        );
        assert_eq!(
            http_client.take_requests().len(),
            0,
            "{} should reject before proxying upstream",
            case.name
        );
    }
}
...
#[tokio::test]
async fn preview_event_post_derives_tenant_index_and_forwards_query_id() {
    let (app, jwt, http_client, customer_id) = setup_ready_index("products").await;
    http_client.push_json_response(202, json!({"accepted": true}));

    let resp = app
        .oneshot(
            Request::builder()
                .method(http::Method::POST)
                .uri("/indexes/products/events")
                .header("authorization", format!("Bearer {jwt}"))
                .header("content-type", "application/json")
                .body(Body::from(
                    json!({
    ...
    assert_eq!(requests.len(), 1);
    assert_eq!(requests[0].method, reqwest::Method::POST);
    assert_eq!(requests[0].url, "https://vm-test.flapjack.foo/1/events");
    assert_eq!(
        requests[0].json_body,
        Some(json!({
            "events": [{
                "eventType": "click",
                "eventName": "search_preview_result_opened",
                "index": test_flapjack_uid(customer_id, "products"),
                "userToken": "preview-session-123",
                "queryID": "q-123",
                "objectIDs": ["doc-1"],
                "positions": [21],
