<script lang="ts">
	...
	type DictionaryToastCompletionInput = Omit<SuccessToastCompletionState, 'completionVersion'> & {
		message: string;
	};
	...
	let lastDictionarySavedToastState = $state(false);
	let lastToastedDictionarySavedActionVersion = $state(0);
	let lastDictionaryDeletedToastState = $state(false);
	let lastToastedDictionaryDeletedActionVersion = $state(0);
	let lastDictionaryClearedToastState = $state(false);
	let lastToastedDictionaryClearedActionVersion = $state(0);
	...
	function toastSuccessOnCompletion(input: DictionaryToastCompletionInput): number {
		const { lastToastedCompletionVersion, message } = input;
		if (
			shouldToastSuccessCompletion({
				success: input.success,
				completionVersion: dictionaryActionVersion,
				lastSuccess: input.lastSuccess,
				lastToastedCompletionVersion
			})
		) {
			toast.success(message, { duration: TOAST_DURATION_MS });
			return dictionaryActionVersion;
		}
	...
	$effect(() => {
		lastToastedDictionarySavedActionVersion = toastSuccessOnCompletion({
			success: dictionarySaved,
			lastSuccess: lastDictionarySavedToastState,
			lastToastedCompletionVersion: lastToastedDictionarySavedActionVersion,
			message: 'Dictionary entry saved.'
		});
		lastDictionarySavedToastState = dictionarySaved;
		lastToastedDictionaryDeletedActionVersion = toastSuccessOnCompletion({
			success: dictionaryDeleted,
			lastSuccess: lastDictionaryDeletedToastState,
			lastToastedCompletionVersion: lastToastedDictionaryDeletedActionVersion,
			message: 'Dictionary entry deleted.'
		});
		lastDictionaryDeletedToastState = dictionaryDeleted;
		lastToastedDictionaryClearedActionVersion = toastSuccessOnCompletion({
			success: dictionaryCleared,
			lastSuccess: lastDictionaryClearedToastState,
			lastToastedCompletionVersion: lastToastedDictionaryClearedActionVersion,
			message: 'Dictionary entries cleared.'
		});
		lastDictionaryClearedToastState = dictionaryCleared;
	});
...
<div
	...
	data-testid={INDEX_DETAIL_TAB_PANEL_TEST_IDS.dictionaries}
...
>
	...
	<EntryPanel
		...
		onDeleteEntry={openDeleteConfirmDialog}
