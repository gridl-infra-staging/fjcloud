...
run_cloudflare_pages_raw_json_redaction_regression() {
    local output_path primary_secret_path stub_dir aws_log_path probe_rc timestamp canonical_dir
    output_path="$(mktemp)"
    primary_secret_path="$(mktemp)"
    stub_dir="$(mktemp -d)"
    aws_log_path="$(mktemp)"
...
EOF
    ...
    canonical_dir="${REPO_ROOT}/docs/live-state/${timestamp}"
    assert_cloudflare_pages_json_has_no_env_values \
        "${canonical_dir}/cf_pages_projects.json" \
        "Cloudflare Pages projects raw JSON"
    assert_cloudflare_pages_json_has_no_env_values \
        "${canonical_dir}/cf_pages_project_flapjack_cloud.json" \
        "Cloudflare Pages project detail raw JSON"
    assert_file_contains \
        "${canonical_dir}/cloudflare_pages.txt" \
...
run_stripe_account_status_parse_error_regression() {
    ...
    create_stubbed_vendor_tools "$stub_dir"
    cat > "$primary_secret_path" <<'EOF'
STRIPE_SECRET_KEY=sk_live_probe_dummy
CLOUDFLARE_GLOBAL_API_KEY=test_key
CLOUDFLARE_EMAIL=test@example.com
EOF
...
run_flapjack_build_identity_probe() {
    # args: stub_class_staging stub_class_prod -> prints canonical SUMMARY path
    ...
    output_path="$(mktemp)"; register_tmp_path "$output_path"
...
run_flapjack_build_identity_evidence_regression() {
    local summary timestamp raw_file status section
    ...
    timestamp="$(sed -n 's/^# fjcloud live-state snapshot — //p' "$summary" | head -n1)"
    raw_file="${REPO_ROOT}/docs/live-state/${timestamp}/flapjack_build_identity.txt"
