<script lang="ts">
	...
	let { qsConfig, qsStatus, qsConfigError, qsConfigSaved, qsConfigDeleted, index }: Props = $props();
	let qsConfigText = $derived(
		...
		<div class="mb-6 rounded-lg bg-white p-6 shadow" data-testid="suggestions-section" data-index={index.name}>
			...
			<div class="mb-6 rounded-md border border-gray-200 p-4">
				<form method="POST" action="?/saveQsConfig" use:enhance>
					<label for="qs-config-json" class="mb-2 block text-sm font-medium text-gray-700">Query Suggestions JSON</label>
					<textarea
						id="qs-config-json"
						name="config"
