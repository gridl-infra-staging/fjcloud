...
Inspect one index's high-level state, run per-index export/import, and jump into deeper tools (Analytics, Settings, Search) from the index-detail landing surface.
...
5. Connect Your App (existing) — unchanged.
6. Read Replicas section (existing) — unchanged.
7. No duplicate setup/navigation footer. The Overview tab relies on the index-detail tab strip plus the Analytics summary `View Details →` link; search and browse discovery belongs to [search.md](search.md), while setup discoverability remains centralized on `/console` and `/console/onboarding`.

## State contract

### Loading
- Stat cards show shimmer placeholders. Analytics summary section shows three card-shaped skeletons. Data Management card shows disabled buttons. Connect Your App and Replicas keep their existing loading behavior.
...
- Stat cards show their values; Endpoint card shows `Preparing...` (existing). Connect Your App shows `Endpoint not ready` (existing). Data Management buttons disabled with tooltip `Available once your index is provisioned`.
...
- Given the analytics summary endpoint returns 500, when the Overview tab loads, then ONLY the Analytics summary section shows `role="alert"` with a Retry button; stat cards, Connect Your App, and Replicas remain functional.
- Given an unprovisioned index (`endpoint === null`), when the Overview tab loads, then Data Management buttons are disabled with the `Available once your index is provisioned` tooltip AND no duplicate setup/navigation footer is rendered.
...
- Current: tab strip is the canonical path from Overview to Settings / Documents, and the Analytics summary owns the only Overview-local deep link via `View Details →`.
  Target: no duplicate in-card navigation footer; this keeps setup/navigation discoverability out of the per-index Overview tab.
  Evidence: `web/src/routes/console/indexes/[name]/tabs/OverviewTab.svelte`; Stage 2 removed `data-testid="overview-navigation"`.

- Current: quick-search behavior has moved out of Overview.
  Target: Overview does not own a search widget; query, browse, facets, and result inspection belong to [search.md](search.md) at `/console/indexes/[name]?tab=search`.
  Evidence: unified Search tab owns the shareable Search URL state and customer-facing browse contract.
...
- Component tests: extend `web/src/routes/console/indexes/[name]/tabs/OverviewTab.test.ts` — KPI card rendering matrix (zero entries / populated / endpoint-null states); quick-search widget and duplicate setup/navigation footer remain absent.
