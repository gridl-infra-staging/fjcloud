...
| Directory | Summary |
| --- | --- |
| canary | The canary directory contains synthetic monitoring and contract validation scripts that continuously verify fjcloud's customer-facing workflows, external service availability, email deliverability, and API security contracts against the staging environment. |
| chaos | The chaos directory contains bash scripts for testing system resilience and failover behavior, including tests for HA failover detection when regions fail, metering breaker alerts when metric targets become unavailable, and recovery verification after region restarts. |
| dev | This file provides migration logic for consolidating integration tests, likely helping organize or refactor test infrastructure within the project. |
| launch | This launch directory contains scripts that verify and validate the fjcloud staging deployment for launch readiness, including Stripe integration checks, browser-based user flow testing, multi-tenant isolation probes, and synthetic traffic generation for testing billing workflows. |
| lib | This directory contains reusable bash shell script helpers and utilities that provide shared functionality across the fjcloud project's automation and validation scripts, covering areas like Stripe integration, database operations, HTTP requests, alert dispatch, security checks, and billing rehearsals. |
| load | The load directory contains regression checking utilities for performance testing, specifically the load_checks.sh script that compares offline and live load harness outputs to detect performance regressions. |
| reliability | The reliability directory contains scripts for capacity profiling, security checks, and backend validation gates that measure performance across document tiers and enforce automated security standards. |
| stripe | This directory contains shell scripts for Stripe account configuration and management, including scripts to configure the Stripe Customer Portal and create product catalogs for specific Stripe accounts using environment-based account selection. |
| tests | This directory contains shell script-based smoke tests for email delivery (SES bounce/complaint handling) and customer broadcast operations, along with a library of shared test helpers and fixtures for integration testing, chaos testing, mock management, and staging billing rehearsal scenarios. |
| vlm | The vlm directory contains tooling for verdict bundle aggregation and VLM judgment operations, with environment helper functions that implement contract compliance for reading trimmed environment values. |
| w3_triage | W3 triage is an automated system that bootstraps by probing live state and audit data, then applies rules to parse audit recommendations and author orchestration lane files, culminating in a dispatch manifest for downstream execution. |
<!-- [scrai:end] -->
