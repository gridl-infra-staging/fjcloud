...
print_usage() {
    cat <<'USAGE'
Usage:
  live_e2e_evidence.sh --env <staging|prod> --domain <domain> --artifact-dir <dir> [--env-file <path>] [--api-ami-id <ami-id>] [--flapjack-ami-id <ami-id>] [--apply] [--run-deploy] [--run-migrate] [--run-rollback] [--alert-email <email>] [--release-sha <sha>] [--rollback-sha <sha>] [--run-billing-rehearsal --month <YYYY-MM> --confirm-live-mutation]
...
parse_args_token() {
    ...
    case "$token" in
        --help|-h)
            SHOW_HELP=1
            ;;
        --env|--domain|--artifact-dir|--env-file|--api-ami-id|--flapjack-ami-id|--month|--alert-email|--release-sha|--rollback-sha)
            ...
            case "$token" in
                ...
                --env-file)
                    ...
                    ;;
                --api-ami-id)
                    LIVE_E2E_API_AMI_ID="$next_value"
                    ;;
                --flapjack-ami-id)
                    LIVE_E2E_FLAPJACK_AMI_ID="$next_value"
                    ;;
                --month)
...
run_runtime_smoke_check() {
    ...
    if [ "${#LIVE_E2E_RUNTIME_OWNER_ARGS[@]}" -gt 0 ]; then
        if capture_owner_logs \
            ...
            --api-ami-id "$LIVE_E2E_API_AMI_ID" \
            --flapjack-ami-id "$LIVE_E2E_FLAPJACK_AMI_ID" \
        ...
        if capture_owner_logs \
            ...
            --env-file "$LIVE_E2E_ENV_FILE" \
            --api-ami-id "$LIVE_E2E_API_AMI_ID" \
            --flapjack-ami-id "$LIVE_E2E_FLAPJACK_AMI_ID"; then
            exit_code=0
        else
            exit_code=$?
        fi
...
main() {
    ...
    if [ -z "$LIVE_E2E_API_AMI_ID" ]; then
        runtime_preconditions_complete=0
        append_external_blocker "missing_api_ami_id" "caller" "rerun with --api-ami-id <ami-id>"
    fi
    if [ -z "$LIVE_E2E_FLAPJACK_AMI_ID" ]; then
        runtime_preconditions_complete=0
        append_external_blocker "missing_flapjack_ami_id" "caller" "rerun with --flapjack-ami-id <ami-id>"
    fi
