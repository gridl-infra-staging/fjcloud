<script lang="ts">
	...
	const navItems = [
		...
		{ href: '/console/migrate' as const, label: 'Migrate', icon: 'upload' },
...
<div class="flex h-screen bg-flapjack-cream">
	...
	<div class="flex flex-1 flex-col overflow-hidden">
		...
		{#if planContext?.billing_plan === 'shared' && planContext.has_payment_method === false}
			...
			<div
				class="border-b border-flapjack-ink/15 bg-brand-cream px-6 py-3 shadow-elevation-card"
				data-testid="billing-cta"
			>
				<div class="flex items-center justify-between">
					<p class="text-sm text-flapjack-ink">
						Your shared plan requires billing setup to continue.
					</p>
					<a
