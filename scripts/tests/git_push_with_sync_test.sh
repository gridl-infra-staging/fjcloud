...
POST_SYNC_HOOK_PATH="$REPO_ROOT/.debbie/post-sync.sh"
...
test_wrapper_syncs_staging_only_by_default_on_main() {
    local tmp_dir
    tmp_dir="$(mktemp -d)"
    local call_log="$tmp_dir/calls.log"
    ...
    assert_eq "$exit_code" "0" "wrapper should exit 0 when push and staging sync succeed on main"
    ...
    # Prod promotion is a deliberate, gated step (post_wave_a_sync_prod.sh
    # --execute), NOT a side effect of every dev push — see the runbook.
    assert_not_contains "$calls" "debbie:sync prod"         "wrapper should NOT sync prod by default (prod promotion is gated)"

    rm -rf "$tmp_dir"
}

test_wrapper_syncs_prod_when_opted_in() {
    local tmp_dir
    tmp_dir="$(mktemp -d)"
    local call_log="$tmp_dir/calls.log"

    local exit_code=0
    # PROD_SYNC=1 is inherited by the wrapper through the environment.
    PROD_SYNC=1 run_wrapper_with_mocks "main" "0" "0" "0" "0" "$call_log" "" origin main || exit_code=$?

    local calls
    calls="$(read_calls "$call_log")"

    assert_eq "$exit_code" "0" "wrapper should exit 0 when push and both syncs succeed on main"
    assert_contains "$calls" "debbie:sync staging" "PROD_SYNC=1 should still sync staging mirror"
    assert_contains "$calls" "debbie:sync prod" "PROD_SYNC=1 should sync prod mirror on main"
...
test_wrapper_keeps_git_push_exit_contract_when_sync_fails_best_effort() {
    ...
    exit_code=0
    PROD_SYNC=1 run_wrapper_with_mocks "main" "0" "1" "0" "0" "$sync_fail_calls" "" origin main || exit_code=$?
...
test_runbook_documents_wrapper_contract() {
    ...
    assert_contains "$content" "PROD_SYNC=1" "runbook should document the prod sync opt-in env var"
    assert_contains "$content" "post_wave_a_sync_prod.sh" "runbook should name the gated prod promotion owner"
...
test_wrapper_prefers_debbie_bin_override_over_path_debbie() {
    local tmp_dir
    tmp_dir="$(mktemp -d)"
    local mock_dir
    mock_dir="$(new_mock_tools_dir)"
    local override_debbie="$tmp_dir/override-debbie"
    local call_log="$tmp_dir/calls.log"

    cat > "$override_debbie" <<'MOCK_DEBBIE_OVERRIDE'
#!/usr/bin/env bash
set -euo pipefail
log_path="${GIT_PUSH_WITH_SYNC_CALL_LOG:?}"
echo "debbie-override:$*" >> "$log_path"
exit 0
MOCK_DEBBIE_OVERRIDE
    chmod +x "$override_debbie"
    ...
    assert_eq "$exit_code" "0" "wrapper should succeed when both PATH debbie and DEBBIE_BIN are available"
    assert_contains "$calls" "debbie-override:sync staging" "wrapper should prefer DEBBIE_BIN over PATH debbie for staging sync"
    assert_contains "$calls" "debbie-override:sync prod" "wrapper should prefer DEBBIE_BIN over PATH debbie for prod sync"
    assert_not_contains "$calls" "debbie:sync staging" "wrapper should not call PATH debbie when DEBBIE_BIN override is set"
    assert_not_contains "$calls" "debbie:sync prod" "wrapper should not call PATH debbie when DEBBIE_BIN override is set"
...
test_post_sync_hook_restores_required_executable_modes() {
    local content
    content="$(load_required_file "$POST_SYNC_HOOK_PATH" "debbie post-sync hook should exist at .debbie/post-sync.sh")" || return
    local debbie_config=""
    if [[ -f "$DEBBIE_CONFIG_PATH" ]]; then
        debbie_config="$(cat "$DEBBIE_CONFIG_PATH")"
    fi

    if [[ -n "$debbie_config" ]]; then
        assert_contains "$debbie_config" '".debbie/post-sync.sh"' "debbie config should sync the post-sync hook required by mirror CI"
    else
        pass "debbie mirror does not require local .debbie.toml to validate post-sync hook content"
    fi
    assert_contains "$content" "required_executable_scripts=(" "post-sync hook should centralize required executable script modes"
    assert_contains "$content" "ops/scripts/deploy.sh" "post-sync hook should restore deploy script executable mode"
    assert_contains "$content" "scripts/engine_index_identity_live_probe.sh" "post-sync hook should restore engine identity probe executable mode"
    assert_contains "$content" "scripts/probe_flapjack_source_rebuild.sh" "post-sync hook should restore source rebuild probe executable mode"
    assert_contains "$content" "scripts/algolia_source_discovery_live_probe.sh" "post-sync hook should restore Algolia discovery probe executable mode"
    assert_contains "$content" 'git -C "$target_root" update-index --chmod=+x "$script_path"' "post-sync hook should stage executable modes from the same list"
}
