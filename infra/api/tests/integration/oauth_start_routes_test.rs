...
async fn assert_oauth_status_response(app: &Router, expected_body: Value) {
    let response = app
        .clone()
        .oneshot(
            Request::builder()
                .method("GET")
                .uri("/auth/oauth/_status")
                .body(Body::empty())
                .unwrap(),
        )
        .await
    ...
    let body: Value = serde_json::from_slice(
        &response
            ...
            .collect()
            .await
            .expect("collect oauth status body")
            .to_bytes(),
    )
    .expect("parse oauth status body");
...
#[tokio::test]
async fn oauth_status_reports_provider_availability_from_runtime_config() {
    let app_without_providers = crate::common::TestStateBuilder::new().build_app();
    assert_oauth_status_response(
        &app_without_providers,
        serde_json::json!({
    ...
    let app_with_both_providers = crate::common::TestStateBuilder::new()
    ...
    let app_with_google_provider = crate::common::TestStateBuilder::new()
        .with_oauth_google_provider(
            "google-client-id",
            "google-client-secret",
            "https://cloud.flapjack.foo/auth/oauth/google/callback",
...
async fn oauth_exchange_happy_path_returns_token_and_customer_id() {
    let provider_server = crate::common::spawn_test_oauth_provider().await;
    let app = crate::common::TestStateBuilder::new()
