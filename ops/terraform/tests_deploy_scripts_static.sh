...
assert_file_contains "$rollback_file" 'aws s3 cp.*s3://.*\$\{CANDIDATE_DIR\}' "on-instance: downloads previous binaries into the unswapped candidate bundle"
assert_file_contains "$rollback_file" 'mv.*\$\{CANDIDATE_DIR\}.*\$\{BIN_DIR\}' "on-instance: swaps the exact proven candidate bundle via mv"
...
assert_file_contains "$deploy_file" 'BINARIES=\(fjcloud-api fjcloud-aggregation-job fjcloud-retention-job\)' "deploy.sh target contract: manages exactly three API-host binaries"
assert_file_contains "$rollback_file" 'BINARIES=\(fjcloud-api fjcloud-aggregation-job fjcloud-retention-job\)' "rollback.sh target contract: manages exactly three API-host binaries"
assert_file_not_contains "$deploy_file" 'fjcloud-api fjcloud-aggregation-job fjcloud-retention-job fj-metering-agent' "deploy.sh target contract: must not include fj-metering-agent in API-host binary set"
assert_file_not_contains "$rollback_file" 'fjcloud-api fjcloud-aggregation-job fjcloud-retention-job fj-metering-agent' "rollback.sh target contract: must not include fj-metering-agent in API-host binary set"
