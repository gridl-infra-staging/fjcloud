...
| `web/tests/e2e-ui/full/migration-recovery.spec.ts` | `docs/screen_specs/admin_migrations.md` | Migration credential failure recovery and real-credential migration initiation contract for `/dashboard/migrate` flows. | B | Tighten external migration-source success/failure proofs with deterministic upstream contract checks. | Depends on external migration-source behavior and credentials, so classified as external-system proof. |
...
| `web/tests/e2e-ui/full/search-preview.spec.ts` | `docs/screen_specs/search_preview.md` | Search-preview tab visibility, generate-preview-key behavior, and mounted search box contract. | A | Add deterministic preview-response assertion checks tied to owner acceptance criteria. | First-party search-preview route behavior against staged index fixtures. |
...
- `web/tests/e2e-ui/full/search-preview.spec.ts`: retained one shell discoverability test and upgraded active/generate tests to require seeded tenant hit evidence through shared `seedSearchableIndex` + search-preview helpers.
...
- `web/tests/e2e-ui/full/migration-recovery.spec.ts`: migration success path remains env-gated but now explicitly requires visible `migration-success` task-id contract (`Task ID:\s+\S+`) after `?/migrate`, while credential-failure coverage stays UI-owned (no new vendor polling seam).
