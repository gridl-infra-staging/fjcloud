...
assert_not_contains_active "$monitor_main_file" 'resource[[:space:]]+"[^"]+"[[:space:]]+"support_email_canary[^"]*"' "monitoring/main.tf remains alarm-focused (no support_email_canary resources)"
...
assert_file_contains "$publish_script_file" 'source .*publish_canary_image_shared\.sh' "publish script sources shared canary publish helper"
assert_file_not_contains "$publish_script_file" '^[[:space:]]*docker build' "publish script does not own inline docker build body"
assert_file_not_contains "$publish_script_file" 'docker push' "publish script does not own inline docker push body"
assert_file_not_contains "$publish_script_file" 'aws ecr get-login-password' "publish script does not own inline ECR login flow"
