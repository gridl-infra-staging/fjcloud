...
pub fn panic_response(
    ...
    metrics_collector.record_panic();
    tracing::error!(
        panic = %panic_error_message(&panic_error),
        "recovered panic while serving request"
    );
    ApiError::Internal("application panic".to_string()).into_response()
}

fn panic_error_message(panic_error: &(dyn Any + Send + 'static)) -> String {
    if let Some(message) = panic_error.downcast_ref::<String>() {
        format!("application panic: {message}")
    } else if let Some(message) = panic_error.downcast_ref::<&str>() {
        format!("application panic: {message}")
    } else {
        "application panic".to_string()
    }
}
...
fn build_router_inner(
    ...
    let metrics_collector_for_panic = metrics_collector.clone();
    let auth_rate_limiter = RateLimiter::new(rate_config.auth_rpm, rate_config.auth_window);
    let auth_rate_limit_state = middleware::AuthRateLimitState {
        limiter: auth_rate_limiter,
        admin_key: state.admin_key.clone(),
    };
    let auth_rate_limited_routes =
        route_assembly::build_auth_rate_limited_routes(auth_rate_limit_state);
    ...
    router
        .with_state(state)
        // Keep panic recovery inside tracing/request-id layers so recovered responses stay correlated.
        .layer(CatchPanicLayer::custom(move |panic_error| {
            panic_response(metrics_collector_for_panic.as_ref(), panic_error)
        }))
        .layer(axum_middleware::from_fn(
            middleware::security_headers_middleware,
        ))
...
mod tests {
    ...
    fn panic_error_message_keeps_string_payload_for_logs() {
        let panic_payload: Box<dyn std::any::Any + Send + 'static> =
            Box::new(String::from("db password leaked"));

        assert_eq!(
            panic_error_message(panic_payload.as_ref()),
            "application panic: db password leaked"
        );
    }

    #[test]
    fn panic_error_message_falls_back_for_non_string_payloads() {
        let panic_payload: Box<dyn std::any::Any + Send + 'static> = Box::new(42usize);
