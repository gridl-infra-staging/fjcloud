...
test_check_prerequisites_surfaces_shared_source_provenance() {
    setup_orchestrator_workspace; install_passing_mocks
    install_prereq_command_mocks
    local explicit_dir="$TEST_WORKSPACE/source_flapjack"
    local cargo_log="$TEST_WORKSPACE/cargo.log"
    mkdir -p "$explicit_dir/engine/flapjack-server/src"
    printf '[workspace]\nmembers = ["flapjack-server"]\n' > "$explicit_dir/engine/Cargo.toml"
    printf 'fn main() {}\n' > "$explicit_dir/engine/flapjack-server/src/main.rs"
    write_mock_script "$TEST_WORKSPACE/bin/cargo" '
...
'
    ...
    run_orchestrator --args "--check-prerequisites" \
        "FLAPJACK_DEV_DIR=$explicit_dir" \
        "FLAPJACK_SOURCE_RECEIPT_DIR=$TEST_WORKSPACE/receipts" \
        "PATH=$TEST_WORKSPACE/bin:/usr/bin:/bin"

    assert_eq "$RUN_EXIT_CODE" "0" \
        "source-backed Flapjack binary should satisfy local signoff prerequisites"
    assert_contains "$RUN_OUTPUT" "prerequisite ok: flapjack_provenance=source-build:" \
        "local signoff should surface shared helper source-build provenance"
    assert_contains "$RUN_OUTPUT" "$TEST_WORKSPACE/receipts" \
        "local signoff should surface the helper-owned receipt path"
    assert_contains "$(cat "$cargo_log")" "cargo build -p flapjack-server" \
        "local signoff should delegate selected-source builds to the helper"
}

test_local_signoff_uses_shared_flapjack_helper_only() {
    local script_text
    script_text="$(cat "$REPO_ROOT/scripts/local-signoff.sh")"

    assert_contains "$script_text" "find_restart_ready_flapjack_binary" \
        "local signoff should resolve Flapjack through the shared helper"
    assert_contains "$script_text" "flapjack_source_provenance_summary" \
        "local signoff should print shared Flapjack provenance"
    assert_not_contains "$script_text" "cargo build -p flapjack-http" \
        "local signoff should not carry a caller-owned legacy Flapjack build path"
    assert_not_contains "$script_text" "cargo build -p flapjack-server" \
        "local signoff should not carry a caller-owned current Flapjack build path"
}
...
test_check_prerequisites_reports_shared_flapjack_provenance_failure() {
    setup_orchestrator_workspace; install_passing_mocks
    install_prereq_command_mocks
    local explicit_dir="$TEST_WORKSPACE/stale_flapjack"
    mkdir -p "$explicit_dir/engine/flapjack-server/src" "$explicit_dir/engine/target/debug"
    printf '[workspace]\nmembers = ["flapjack-server"]\n' > "$explicit_dir/engine/Cargo.toml"
    printf 'fn main() {}\n' > "$explicit_dir/engine/flapjack-server/src/main.rs"
    write_mock_script "$explicit_dir/engine/target/debug/flapjack" 'exit 0'
    install_path_flapjack_binary
    ...
    assert_eq "$RUN_EXIT_CODE" "1" \
        "stale source-backed Flapjack resolution should fail through the shared helper"
    assert_contains "$RUN_OUTPUT" "flapjack_binary" \
        "shared provenance rejection should be surfaced as the Flapjack prerequisite"
    assert_not_contains "$RUN_OUTPUT" "prerequisite ok: flapjack_provenance=legacy-release" \
        "signoff must not replace a failed selected source build with the PATH release"
}
# ============================================================================
