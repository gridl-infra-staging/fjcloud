...
vi.mock('$app/state', () => ({
	page: { url: new URL('http://localhost/console/onboarding') }
}));
...
describe('Onboarding wizard', () => {
	it('step 1 renders region options with index name input', () => {
		...
		expect(
			within(step1).getByText(/choose the region where the index will run/i)
		).toBeInTheDocument();
		expect(within(step1).getByText(/use a stable index name for your app/i)).toBeInTheDocument();
	...
	it('step 3 shows credentials with copy button', () => {
		...
		// Go to Console link
		expect(within(step3).getByRole('link', { name: /go to console/i })).toBeInTheDocument();
	});
	...
	it('completed status shows redirect message', () => {
		...
		expect(screen.getByRole('link', { name: /go to console/i })).toBeInTheDocument();
	});
});
...
describe('Plan-aware onboarding gates', () => {
	...
	it('Paid plan without payment method is blocked behind billing setup', () => {
		...
		expect(
			within(gate).getByText(
				/your paid plan needs a payment method before onboarding can continue/i
			)
		).toBeInTheDocument();
	...
	it('Paid plan with payment method can proceed to onboarding steps', () => {
