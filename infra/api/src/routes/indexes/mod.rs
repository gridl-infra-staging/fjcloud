...
pub(crate) struct AdmittedIndexDestination {
    pub customer_id: Uuid,
    pub index_name: String,
    pub region: String,
}

impl AdmittedIndexDestination {
    pub(crate) fn new(customer_id: Uuid, index_name: &str, region: &str) -> Self {
        Self {
            customer_id,
    ...
    pub(crate) fn flapjack_uid(&self) -> String {
        admitted_flapjack_index_uid(self.customer_id, &self.index_name)
    }
}
...
pub(crate) fn admitted_flapjack_index_uid(customer_id: Uuid, index_name: &str) -> String {
    flapjack_index_uid(customer_id, index_name)
}

pub(crate) fn catalog_identity_from_tenant(
...
pub use chat::chat;
pub use debug::{get_debug_events, post_preview_event, PreviewEventRequest};
...
pub use replicas::{create_replica, delete_replica, list_replicas};
pub use restore::{restore_index, restore_status};
