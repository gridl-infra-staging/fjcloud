...
assert_file_contains "$deploy_file" 'aws s3 cp.*fj-metering-agent\.service' "on-instance: downloads the metering systemd unit from S3"
assert_file_contains "$deploy_file" 'install -m 0644.*fj-metering-agent\.service.*\/etc\/systemd\/system\/fj-metering-agent\.service' "on-instance: installs the checked-in metering systemd unit onto the host"
assert_file_contains "$deploy_file" 'systemctl daemon-reload' "on-instance: reloads systemd after updating the metering unit"
...
assert_file_contains "$deploy_file" 'systemctl restart fj-metering-agent' "on-instance: restarts fj-metering-agent service"
...
assert_file_contains "$rollback_file" 'aws s3 cp.*fj-metering-agent\.service' "on-instance: downloads the metering systemd unit from S3 during rollback"
assert_file_contains "$rollback_file" 'install -m 0644.*fj-metering-agent\.service.*\/etc\/systemd\/system\/fj-metering-agent\.service' "on-instance: reinstalls the checked-in metering unit during rollback"
assert_file_contains "$rollback_file" 'systemctl daemon-reload' "on-instance: reloads systemd after updating the metering unit during rollback"
...
assert_file_contains "$rollback_file" 'systemctl restart fj-metering-agent' "on-instance: restarts fj-metering-agent service"
...
# Both deploy and rollback should handle the same set of binaries
assert_file_contains "$deploy_file" 'fjcloud-api fjcloud-aggregation-job fj-metering-agent' "deploy.sh manages all three binaries"
assert_file_contains "$rollback_file" 'fjcloud-api fjcloud-aggregation-job fj-metering-agent' "rollback.sh manages all three binaries"
