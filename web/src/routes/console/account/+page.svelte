<script lang="ts">
	...
	let lastToastedSharedSuccessMessage = '';
	let sharedSuccessCompletionVersion = $state(0);
	let lastToastedSharedSuccessCompletionVersion = $state(0);
	...
	const trackSharedSuccessResult: SubmitFunction = () => {
		return async ({ result, update }) => {
			if (result.type === 'success') {
				sharedSuccessCompletionVersion += 1;
			}
			await update();
		};
	};
	...
	$effect(() => {
		if (!sharedSuccessMessage) {
			lastToastedSharedSuccessMessage = '';
			return;
		}

		if (
			sharedSuccessMessage === lastToastedSharedSuccessMessage &&
			sharedSuccessCompletionVersion === lastToastedSharedSuccessCompletionVersion
		) {
			return;
		}

		toast.success(sharedSuccessMessage, { duration: TOAST_DURATION_MS });
		lastToastedSharedSuccessMessage = sharedSuccessMessage;
		lastToastedSharedSuccessCompletionVersion = sharedSuccessCompletionVersion;
	});
</script>
...
<div data-testid="account-page">
	...
	<div class="mb-6 rounded-lg bg-white p-6 shadow">
		...
		{#if profile}
			<form method="POST" action="?/updateProfile" use:enhance={trackSharedSuccessResult}>
				<div class="mb-4">
	...
	<div class="rounded-lg bg-white p-6 shadow">
		...
		<form method="POST" action="?/changePassword" use:enhance={trackSharedSuccessResult}>
			<div class="mb-4">
				<div class="max-w-md">
					<PasswordInput
						id="current-password"
	...
	>
		...
		<p class="mb-4 text-sm text-flapjack-ink/80">
			This deactivates your account and signs you out. Retained audit records may remain. Deleting
			the account does not cancel billing.
		</p>
		<p class="mb-4 text-sm text-flapjack-ink/80">
			Contact
			...
			to cancel billing before deleting your account.
		</p>
				...
				<div class="mb-4">
					<div class="max-w-md">
						<PasswordInput
							...
							inputClass="text-sm focus:border-flapjack-plum focus:outline-none focus:ring-flapjack-plum"
						/>
					</div>
				</div>
