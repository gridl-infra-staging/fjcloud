...
macro_rules! require_live_locked {
    ($condition:expr, $reason:expr) => {{
        let _env_guard = crate::common::integration_helpers::test_env_lock();
        crate::require_live!($condition, $reason);
    }};
}
...
fn mock_app_with_state(
    ...
    webhook_event_repo: std::sync::Arc<crate::common::MockWebhookEventRepo>,
...
fn seed_mock_draft_invoice(
    invoice_repo: &crate::common::MockInvoiceRepo,
...
fn stripe_test_key() -> Option<String> {
    crate::common::live_stripe_helpers::stripe_test_key()
}

fn stripe_webhook_secret() -> Option<String> {
    crate::common::live_stripe_helpers::stripe_webhook_secret()
}
...
fn stripe_api_available() -> bool {
    crate::common::live_stripe_helpers::stripe_api_available()
}
...
fn stripe_webhook_configured() -> bool {
    crate::common::live_stripe_helpers::stripe_webhook_configured()
}
...
) -> Result<crate::common::live_stripe_helpers::WebhookProbeResult, String> {
    crate::common::live_stripe_helpers::validate_stripe_webhook_delivery().await
}

/// Runtime probe for Stripe webhook forwarding (`stripe listen`).
async fn stripe_webhook_available() -> bool {
    crate::common::live_stripe_helpers::stripe_webhook_available().await
}
...
async fn validate_stripe_key_live() -> Result<(), String> {
    crate::common::live_stripe_helpers::validate_stripe_key_live().await
}

fn build_live_stripe_handles() -> crate::common::live_stripe_helpers::LiveStripeHandles {
    crate::common::live_stripe_helpers::build_live_stripe_handles()
}
...
async fn delete_stripe_customer(client: &stripe::Client, customer_id: &str) {
    crate::common::live_stripe_helpers::delete_stripe_customer(client, customer_id).await
}
...
async fn attach_test_payment_method(client: &stripe::Client, customer_id: &str) -> String {
    crate::common::live_stripe_helpers::attach_test_payment_method(client, customer_id).await
}
...
async fn attach_declining_payment_method(client: &stripe::Client, customer_id: &str) -> String {
    crate::common::live_stripe_helpers::attach_declining_payment_method(client, customer_id).await
}
...
async fn stripe_create_setup_intent_in_test_mode() {
    ...
    // The setup flow saves cards only. Without an explicit `payment_method_types`,
    // Stripe falls back to the account's automatic payment methods (Pix, Klarna,
    // Cash App Pay, ...), which turn the hosted Payment Element into a redirect
    // accordion and break the card confirmSetup return navigation. Retrieve the
    // created intent and assert the server constrained it to card-only.
    let intent_id: stripe::SetupIntentId = client_secret
        .split("_secret_")
        .next()
        .expect("client_secret splits on _secret_")
        .parse()
        .expect("setup intent id parses");
    let intent = stripe::SetupIntent::retrieve(&stripe.client, &intent_id, &[])
        .await
        .expect("retrieve setup intent should succeed");
    assert_eq!(
        intent.payment_method_types,
        vec!["card".to_string()],
        "setup intent must offer card only, got: {:?}",
        intent.payment_method_types
    );
...
async fn run_live_checkout_to_paid_invoice_end_to_end() {
    let client = crate::common::integration_helpers::http_client();
    let base = crate::common::integration_helpers::api_base();
    let db_url = crate::common::integration_helpers::db_url();
    ...
    let _jwt = crate::common::integration_helpers::register_and_login(&client, &base, &email).await;
...
mod helper_tests {
    use super::{
        stripe_api_available, stripe_test_key, stripe_webhook_configured, stripe_webhook_secret,
        validate_stripe_key_live, validate_stripe_webhook_delivery,
    };
