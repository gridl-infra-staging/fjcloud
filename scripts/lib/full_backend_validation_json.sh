...
emit_result_json() {
    ...
    VERDICT="$verdict" \
    FAIL_STATUSES="$BACKEND_GATE_FAIL_STATUSES" \
    MODE="$mode" \
...
fail_statuses = set(filter(None, os.environ.get("FAIL_STATUSES", "").split(",")))
for step in steps:
...
if data.get("verdict") == "pass":
    ...
    if isinstance(checks, list):
        failures = []
        for check in checks:
            if isinstance(check, dict) and check.get("status") in fail_statuses:
