<script lang="ts">
	...
	import { rootRelativeExperimentDetailHref } from './experiments_tab_helpers';
...
<div
	...
	data-testid={INDEX_DETAIL_TAB_PANEL_TEST_IDS.experiments}
>
			...
			<table class="w-full text-left text-sm">
				...
				<tbody class="divide-y">
					{#each experiments.abtests as experiment (experiment.abTestID)}
						<tr>
							<td class="px-4 py-2">
								<!-- eslint-disable svelte/no-navigation-without-resolve -- resolve() returns a relative href here; normalize it so detail links stay root-relative from every tab route. -->
								<a
									href={rootRelativeExperimentDetailHref(
										resolve('/console/indexes/[name]/experiments/[experimentId]', {
											name: index.name,
