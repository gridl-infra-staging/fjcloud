...
test.describe('Billing upgrade flow (mocked)', () => {
	...
	test('success state shows Paid banner and hides CTA', async ({ page }) => {
		await installUpgradeFixture(page, {
			billing_plan: 'free',
			has_payment_method: true,
			upgrade_outcome: {
	...
	test('already-Paid state shows banner and hides CTA', async ({ page }) => {
		await installUpgradeFixture(page, {
			billing_plan: 'free',
			has_payment_method: true,
			upgrade_outcome: {
		...
		await expect(page.getByTestId('already-shared-banner')).toContainText(
			'This account is already on Paid'
		);
		await expect(page.getByRole('heading', { name: 'Paid plan active' })).toBeVisible();
		await expect(page.getByTestId('current-plan-label')).toContainText('Paid');
		await expect(page.getByTestId('upgrade-to-shared-button')).toHaveCount(0);
