...
async fn invoice_create_idempotency_key_is_deterministic_for_same_inputs() {
    ...
    let first = invoice_create_idempotency_key(invoice_id, customer_id, start, end);
    let second = invoice_create_idempotency_key(invoice_id, customer_id, start, end);
...
async fn invoice_create_idempotency_key_changes_when_period_changes() {
    ...
    let first = invoice_create_idempotency_key(invoice_id, customer_id, start, end);
    let second = invoice_create_idempotency_key(invoice_id, customer_id, start, alternate);
...
async fn mock_stripe_create_customer_returns_id() {
    let svc = crate::common::mock_stripe_service();
    let id = svc
