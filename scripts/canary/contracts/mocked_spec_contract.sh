...
fail=0
forgot_auth_wire_skip_reason=""
reset_auth_wire_skip_reason=""
billing_wire_skip_reason=""
...
read_or_fetch_forgot_resend() {
  ...
  for attempt in $(seq 1 "$RATE_LIMIT_MAX_ATTEMPTS"); do
    ...
    if is_rate_limited_body "$response" && [[ "$attempt" -lt "$RATE_LIMIT_MAX_ATTEMPTS" ]]; then
      sleep "$RATE_LIMIT_RETRY_SECONDS"
      continue
    fi
    break
  done
  if is_rate_limited_body "$response"; then
    forgot_auth_wire_skip_reason="staging forgot-password action remained rate-limited after ${RATE_LIMIT_MAX_ATTEMPTS} attempts"
    printf '{}'
    return
  fi
  printf '%s' "$response"
...
read_or_fetch_reset_invalid() {
  ...
  if is_rate_limited_body "$response"; then
    reset_auth_wire_skip_reason="staging reset-password action remained rate-limited after ${RATE_LIMIT_MAX_ATTEMPTS} attempts"
    printf '{}'
    return
  fi
  printf '%s' "$response"
...
read_or_fetch_billing_load() {
  ...
  for attempt in $(seq 1 "$RATE_LIMIT_MAX_ATTEMPTS"); do
    ...
    if is_rate_limited_body "$register_response" && [[ "$attempt" -lt "$RATE_LIMIT_MAX_ATTEMPTS" ]]; then
      sleep "$RATE_LIMIT_RETRY_SECONDS"
      continue
    fi
    break
  ...
  if [[ -z "$token" ]]; then
    if is_rate_limited_body "$register_response"; then
      billing_wire_skip_reason="billing auth probe remained rate-limited after ${RATE_LIMIT_MAX_ATTEMPTS} attempts"
      printf '{}'
      return
    fi
...
if [[ -n "$forgot_auth_wire_skip_reason" ]]; then
  say_skip "$forgot_auth_wire_skip_reason"
elif [[ -z "$forgot_expected_keys" ]]; then
...
if [[ -n "$reset_auth_wire_skip_reason" ]]; then
  say_skip "$reset_auth_wire_skip_reason"
elif [[ -z "$reset_expected_keys" ]]; then
...
if [[ -n "$billing_wire_skip_reason" ]]; then
  say_skip "$billing_wire_skip_reason"
elif [[ -z "$billing_raw" ]]; then
