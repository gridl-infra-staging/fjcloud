...
#[test]
fn browser_error_report_accepts_valid_payload_and_logs_sanitized_fields() {
    ...
    tracing::subscriber::with_default(subscriber, || {
        ...
        runtime.block_on(async {
            let state = crate::common::TestStateBuilder::new().build();
            let app = api::router::build_router_with_auth_rate_config(
                state,
                100,
                Duration::from_secs(60),
            );

            // Other platform tests can touch request-log callsites before this subscriber is active.
    ...
    let completed_request_events: Vec<&Value> = events
        ...
        .collect();
    let route_path_events: Vec<&Value> = events
        .iter()
        .filter(|event| extract_event_field(event, "path") == Some(BROWSER_ERRORS_PATH))
        .collect();
    ...
    assert!(
        !route_path_events.is_empty(),
        "expected at least one structured request log event with route path"
    );

    let handler_event = browser_events[0];
    let completed_request_event = completed_request_events
        .first()
        .copied()
        .unwrap_or(route_path_events[0]);
