...
use api::middleware::metrics::metrics_middleware;
use api::middleware::{RequestSpan, ResponseLogger, UuidRequestId};
use api::router::panic_response;
use axum::body::Body;
...
fn find_panic_error_event_for_path<'a>(events: &'a [Value], path: &str) -> Option<&'a Value> {
    events.iter().rfind(|event| {
        event.get("level").and_then(Value::as_str) == Some("ERROR")
            && extract_event_field(event, "path") == Some(path)
            && event_message(event)
                .map(|message| message.contains("panic"))
                .unwrap_or(false)
    })
}
...
async fn request_id_appears_in_response() {
    let _guard = tracing_test_lock();
    let app1 = crate::common::test_app();
    ...
    let app2 = crate::common::test_app();
...
fn request_completed_log_includes_tenant_id_from_jwt() {
    ...
    let token = crate::common::create_test_jwt(customer_id);
    ...
    tracing::subscriber::with_default(subscriber, || {
        let mut request_span = RequestSpan::new(Arc::<str>::from(crate::common::TEST_JWT_SECRET));
...
/// Test 9: Panic responses keep the same public contract as internal errors
/// while recording the panic metric and a correlated error log.
#[test]
#[allow(clippy::await_holding_lock)]
fn panic_path_response_metric_and_log_contract_with_full_middleware_stack() {
    use axum::routing::get;
    let _guard = tracing_test_lock();
    let caller_request_id = "req-panic-contract-123";
    let buf = Arc::new(Mutex::new(Vec::new()));
    let writer = BufWriter(buf.clone());
    let subscriber = tracing_subscriber::registry().with(
        tracing_subscriber::fmt::layer()
    ...
    tracing::subscriber::with_default(subscriber, || {
        ...
        runtime.block_on(async {
            ...
            let collector_for_panic = collector.clone();
            ...
            let app = axum::Router::new()
                ...
                .with_state(state)
                .layer(CatchPanicLayer::custom(move |panic_error| {
                    panic_response(collector_for_panic.as_ref(), panic_error)
                }))
                ...
                .layer(
                    TraceLayer::new_for_http()
                        .make_span_with(RequestSpan::new(jwt_secret))
            ...
            let resp = app.oneshot(req).await.unwrap();
            observed_status = Some(resp.status());
            observed_request_id = resp.headers().get("x-request-id").map(|value| {
                value
                    .to_str()
            ...
            observed_body =
                Some(String::from_utf8(body.to_vec()).expect("panic body should be UTF-8"));
        });
    });

    let metrics_text = metrics_collector
        .expect("test should retain the shared metrics collector")
        .render();
    let output = buf.lock().unwrap().clone();
    let events = parse_json_lines(&output);
    let panic_error_event = find_panic_error_event_for_path(&events, "/panic");
    let mut failures = Vec::new();

    if observed_status != Some(StatusCode::INTERNAL_SERVER_ERROR) {
        failures.push(format!(
            "missing panic response contract: expected status 500, got {:?}",
            observed_status
        ));
    }
    if observed_body.as_deref() != Some(r#"{"error":"internal server error"}"#) {
        failures.push(format!(
            "missing panic response body contract: expected {{\"error\":\"internal server error\"}}, got {:?}",
            observed_body
        ));
    }
    if observed_request_id.as_deref() != Some(caller_request_id) {
        failures.push(format!(
            "missing panic request-id contract: expected x-request-id {caller_request_id}, got {:?}",
            observed_request_id
        ));
    }
    for expected_metric_line in [
        "# HELP panics_total",
        "# TYPE panics_total counter",
        "panics_total 1",
    ] {
        if !metrics_text.contains(expected_metric_line) {
            failures.push(format!(
                "missing panic metric contract: expected metrics render to contain `{expected_metric_line}`. Metrics output:\n{metrics_text}"
            ));
        }
    }

    match panic_error_event {
        Some(event) => {
            if extract_event_field(event, "request_id") != Some(caller_request_id) {
                failures.push(format!(
                    "missing panic log request-id contract: expected {caller_request_id} in correlated error event, got event {}",
                    serde_json::to_string_pretty(event).unwrap_or_else(|_| event.to_string())
                ));
            }
            if extract_event_field(event, "path") != Some("/panic") {
                failures.push(format!(
                    "missing panic log path contract: expected /panic in correlated error event, got event {}",
                    serde_json::to_string_pretty(event).unwrap_or_else(|_| event.to_string())
                ));
            }
            if extract_event_field(event, "route") != Some("/panic") {
                failures.push(format!(
                    "missing panic log route identity contract: expected route=/panic in correlated error event, got event {}",
                    serde_json::to_string_pretty(event).unwrap_or_else(|_| event.to_string())
                ));
            }
        }
        None => failures.push(format!(
            "missing panic log contract: expected panic-specific ERROR event correlated to request_id={caller_request_id} path=/panic; events={}",
    ...
    tracing::subscriber::with_default(subscriber, || {
        ...
        runtime.block_on(async {
            let app = axum::Router::new()
                ...
                .layer(
                    TraceLayer::new_for_http()
                        .make_span_with(RequestSpan::new(Arc::<str>::from(
                            crate::common::TEST_JWT_SECRET,
                        )))
                        .on_response(ResponseLogger),
                )
                .layer(PropagateRequestIdLayer::x_request_id())
