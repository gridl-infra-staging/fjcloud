...
is_allowed_public_ingress() {
  ...
  [[ "$resource_type" == "aws_security_group" && "$resource_name" == "alb" && "$protocol" == "tcp" \
...
run_sg_audit_for_file() {
  ...
  local security_group_ingress_protocol=""
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    if [[ "$in_ingress_rule" -eq 1 ]]; then
      ...
      if [[ "$active_line" =~ ^[[:space:]]*ip_protocol[[:space:]]*=[[:space:]]*\"([^\"]+)\" ]]; then
        ingress_rule_protocol="${BASH_REMATCH[1]}"
      fi
      ...
      if [[ "$ingress_rule_depth" -le 0 ]]; then
        if [[ "$ingress_rule_has_public" -eq 1 ]] && ! is_allowed_public_ingress "aws_vpc_security_group_ingress_rule" "$ingress_rule_name" "$ingress_rule_from_port" "$ingress_rule_to_port" "$ingress_rule_protocol"; then
          echo "ERROR: insecure public ingress in $tf_file (resource=${ingress_rule_name:-unknown}, from_port=${ingress_rule_from_port:-unset}, to_port=${ingress_rule_to_port:-unset}, protocol=${ingress_rule_protocol:-unset})" >&2
          failed=1
        fi
    ...
    if [[ "$in_security_group" -eq 1 ]]; then
      ...
      if [[ "$in_security_group_ingress" -eq 1 ]]; then
        ...
        if [[ "$active_line" =~ ^[[:space:]]*protocol[[:space:]]*=[[:space:]]*\"([^\"]+)\" ]]; then
          security_group_ingress_protocol="${BASH_REMATCH[1]}"
        fi
        ...
        if [[ "$security_group_ingress_depth" -le 0 ]]; then
          if [[ "$security_group_ingress_has_public" -eq 1 ]] && ! is_allowed_public_ingress "aws_security_group" "$security_group_name" "$security_group_ingress_from_port" "$security_group_ingress_to_port" "$security_group_ingress_protocol"; then
            echo "ERROR: insecure public ingress in $tf_file (resource=${security_group_name:-unknown}, ingress=inline, from_port=${security_group_ingress_from_port:-unset}, to_port=${security_group_ingress_to_port:-unset}, protocol=${security_group_ingress_protocol:-unset})" >&2
            failed=1
          fi
...
run_security_group_audit() {
  ...
  if [[ "$failed" -eq 0 ]]; then
    echo "Security-group audit passed (public 0.0.0.0/0 ingress limited to named TCP ALB 80/443 and Flapjack ACME 80 and HTTPS 443 rules)."
  fi
