<script lang="ts">
	...
	let filteredApiKeys = $derived.by(() => {
		...
		return apiKeys.filter(
			(key) => key.indexes.length === 0 || key.indexes.includes(selectedIndexFilter)
		);
	});
	...
	async function handleCopyKey(
		...
		await copyToClipboard(copyValueForKey(key), buttonElement);
	}
	...
	function updateIndexFilter(nextIndex: string): void {
		...
		if (nextIndex.trim().length > 0) {
			currentEntries.push(['index', nextIndex]);
		}
...
<div>
		...
		>
			<div class="flex flex-wrap items-start justify-between gap-3">
				...
				<button
					...
					onclick={(event) =>
						void copyToClipboard(createdKey, event.currentTarget as HTMLButtonElement)}
				>
					Copy
				</button>
			</div>
	...
	<div class="mb-4 flex flex-wrap items-center gap-3">
		...
		<select
			...
			onchange={(event) => updateIndexFilter((event.currentTarget as HTMLSelectElement).value)}
							...
							<td class="px-4 py-3 text-right">
								<div class="flex justify-end gap-2">
									<button
										...
										class="rounded border border-flapjack-ink/25 px-3 py-1 text-sm text-flapjack-ink/80 hover:bg-flapjack-cream/70"
										type="button"
										onclick={(event) =>
