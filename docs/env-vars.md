...
| `ENVIRONMENT`                       | No       | `unknown`                    | Environment name included in alert messages (e.g., `staging`, `prod`). Set to `local`, `dev`, or `development` together with `NODE_SECRET_BACKEND=memory` to enable local zero-dependency fallbacks. When set to `prod` or `production`, startup requires at least one non-blank alert webhook (`SLACK_WEBHOOK_URL` or `DISCORD_WEBHOOK_URL`). Aggregation success metrics publish only for exact `staging` or `prod`; missing, blank, or any other value skips publishing with an info log. |
...
| `APP_PUBLIC_BASE_URL`               | No       | —                            | Backward-compatible alias for `APP_BASE_URL`. When `APP_BASE_URL` is unset, startup reuses this value through the same `APP_BASE_URL` normalization path (single source of truth)                                                                                                                                                             |
| `FJCLOUD_ALGOLIA_MIGRATION_ENABLED` | No       | `false`                      | Runtime value generated from SSM `/fjcloud/{env}/algolia_migration_enabled`. Terraform provisions the parameter at `false`; `ops/scripts/set_algolia_migration_availability.sh` is the only post-provision mutation path. With the flag `true` and the current declared engine-support contract present, `/migration/algolia/availability` reports `available` and `POST /migration/algolia/list-indexes` is admitted locally; staging and production still serve the closed state by default because their runtime default remains `false`. |
| `FJCLOUD_ENGINE_DATA_PLANE_TLS_ENABLED` | No   | `false`                      | Runtime inert control-plane-to-engine TLS gate consumed by `engine_base_url`. Values must be `true` or `false`; invalid or blank values fail closed as disabled/cleartext. This repo does not enable it for local, staging, or production. |
| `FJCLOUD_VM_AUTOREPAIR_ENABLED`     | No       | `false`                      | Runtime kill switch for autonomous VM autorepair replacement decisions. The reconciler loop still runs when this is absent or `false`, records `detected_dead` and `replacement_refused`, and makes no provisioning, placement, retirement, or teardown mutations. Invalid or blank values fail closed as disabled. Local, staging, and production are absent=`false`; this repo does not enable the flag for staging or production. Values must be `true` or `false`. |
| `FJCLOUD_VM_AUTOREPAIR_CHECK_INTERVAL_SECONDS` | No | `60` | VM autorepair reconciliation interval in seconds. Must be a positive integer. |
| `FJCLOUD_VM_AUTOREPAIR_HOST_DEAD_AFTER_SECONDS` | No | `900` | Continuous provider-dead and engine-unreachable observation window before a host is classified `HostDead`. Must be a positive integer. |
| `FJCLOUD_VM_AUTOREPAIR_REPLACEMENT_COOLDOWN_SECONDS` | No | `1800` | Cooldown window after a replacement boots before another replacement may be admitted. Must be a positive integer. |
| `FJCLOUD_VM_AUTOREPAIR_REGION_DEATH_WINDOW_SECONDS` | No | `900` | Regional dampening lookback window for dead-host detections. Must be a positive integer. |
| `FJCLOUD_VM_AUTOREPAIR_REGION_DEATH_LIMIT` | No | `2` | Maximum regional dead-host detections allowed within the regional dampening window before replacement is refused. Must fit in `u32`. |
| `FJCLOUD_VM_AUTOREPAIR_CONCURRENT_REPLACEMENT_CAP` | No | `1` | Maximum unfinished VM replacements allowed at once. Must fit in `u32`; `0` fail-closes all enabled replacements through the concurrent-cap guardrail. |
| `FJCLOUD_VM_AUTOREPAIR_SPEND_WINDOW_SECONDS` | No | `86400` | Spend guardrail lookback window for admitted replacement attempts. Must be a positive integer. |
| `FJCLOUD_VM_AUTOREPAIR_REPLACEMENT_COST_CENTS` | No | `1000` | Planned spend charged to each admitted replacement attempt for spend-window guardrail accounting. Must be a non-negative integer. |
| `FJCLOUD_VM_AUTOREPAIR_SPEND_CEILING_CENTS` | No | `0` | Maximum projected replacement spend allowed inside the spend window. Default `0` fail-closes enabled replacement attempts unless operators explicitly configure a positive ceiling. Must be a non-negative integer. |
| `FJCLOUD_FLAPJACK_VERSION`          | No       | —                            | Expected Flapjack `/health` version used by API Algolia import admission and local-stack compatibility checks. When set, a selected shared VM with a different version is rejected before import persistence.                                                                                                                                    |
| `FJCLOUD_FLAPJACK_REQUIRED_REVISION` | No      | —                            | Expected Flapjack source revision used by API Algolia import admission and local-stack compatibility checks. When any exact identity variable is set, `/health` must also report non-dirty build evidence.                                                                                                                                      |
| `FJCLOUD_FLAPJACK_REQUIRED_BUILD_ID` | No       | —                            | Expected Flapjack build/workspace digest used by API Algolia import admission and local-stack compatibility checks. Accepts the same `/health` field aliases as `scripts/lib/local_stack_contract.sh`.                                                                                                                                         |
| `FJCLOUD_FLAPJACK_REQUIRED_SHA256`  | No       | —                            | Expected Flapjack binary SHA-256 verified by source/launch tooling before API startup. Runtime `/health` compatibility checks still reject a mismatched reported SHA, but real source-built engines are not required to self-report this artifact hash.                                                                                         |
| `FJCLOUD_FLAPJACK_REQUIRED_CAPABILITY` | No    | —                            | Optional Flapjack capability required by API Algolia import admission and local-stack compatibility checks. `/health` may expose capabilities as a list or a map with the required capability set to `true`.                                                                                                                                   |

**Flapjack identity note:** Algolia import admission fails closed unless `FJCLOUD_FLAPJACK_VERSION`, `FJCLOUD_FLAPJACK_REQUIRED_REVISION`, `FJCLOUD_FLAPJACK_REQUIRED_BUILD_ID`, and `FJCLOUD_FLAPJACK_REQUIRED_SHA256` are all non-blank. Runtime `/health` compatibility is anchored on version, revision, build/workspace digest, and non-dirty evidence; the binary SHA remains an artifact provenance check owned by the launcher/source-build tooling. Other API routes can run without this import-specific configuration.
...
| Variable                          | Required | Default                                        | Description                                                                                                                                                                                                                 |
| --------------------------------- | -------- | ---------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ALGOLIA_MIGRATION_PROBE_TOKEN`   | Execute  | Falls back to `ALGOLIA_INVALID_CREDENTIALS_TENANT_TOKEN` | Tenant JWT used only by `scripts/algolia_migration_safety_probe.sh` for authenticated proof of `GET /migration/algolia/availability`. Runtime toggle commands deliberately do not transport this token through SSM command history. |

## Retention Job Operator

`infra/retention-job` builds the `fjcloud-retention-job` batch binary. It reuses the existing API/database credentials: `DATABASE_URL` for candidate selection through `PgCustomerRepo` and `ADMIN_KEY` for the admin hard-erase route.

| Variable                      | Required | Default | Description                                                                                                                                                                                                                       |
| ----------------------------- | -------- | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `API_URL`                     | Yes      | —       | Base URL for the fjcloud API used by `fjcloud-retention-job` when calling `POST /admin/customers/:id/hard-erase`; trailing slashes are trimmed, and only `http`/`https` URLs with a host and no embedded credentials are accepted |
| `RETENTION_DAYS`              | No       | `30`    | Number of days to retain soft-deleted customers before they are eligible for hard erasure. Parsed as a signed integer by `infra/retention-job/src/config.rs`                                                                      |
| `RETENTION_MAX_ERASE_PER_RUN` | No       | `25`    | Maximum hard-erase attempts in one retention-job run. Parsed as an unsigned integer                                                                                                                                               |
| `RETENTION_DRY_RUN`           | No       | `true`  | Dry-run switch. Accepted values are `true`, `false`, `1`, and `0`; unset defaults to dry-run mode. The systemd service pins `RETENTION_DRY_RUN=0` for live purges                                                                 |
...
| `DUNNING_EMAILS_DISABLED` | No       | `false`          | Runtime boolean gate for Stripe dunning emails (`invoice.payment_failed` retry scheduled/exhausted and `invoice.payment_succeeded` recovery). Accepts `true`/`false` (case-insensitive). When `true`, webhook invoice/customer transitions and alerts still run; only customer dunning email delivery is skipped   |
...
| Variable                    | Required | Default        | Description                                                                                                                                                                                                                                                                  |
| --------------------------- | -------- | -------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AWS_AMI_ID`                | Yes\*    | —              | Flapjack AMI used for newly provisioned customer VMs. The API loads this value from `/fjcloud/<env>/aws_ami_id`; it is separate from the AMI running the API EC2 instance.                                                                                                    |
...
| `FJCLOUD_RUNTIME_SMOKE_ARTIFACT_DIR` | No | `ops/terraform/artifacts` | Override for Stage 7 runtime-smoke plan/evidence output. Unit tests set this to an isolated temporary directory; operators normally use the repository default.                                                                                                 |
...
| Variable                                               | Required           | Default | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ------------------------------------------------------ | ------------------ | ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `CLOUDFLARE_API_TOKEN`                                 | Staging/Prod infra | —       | Cloudflare API token. Two uses: (1) Terraform/staging DNS validation needs Zone:Read + DNS:Edit — keep that copy in the operator secret file, not the repo. (2) The mirror CI `deploy-web` step (`ci.yml` deploy-staging) needs Account → Cloudflare Pages → Edit and consumes it as a **staging-mirror GitHub Actions secret** (`gridl-infra-staging/fjcloud`) paired with `CLOUDFLARE_ACCOUNT_ID` for `wrangler pages deploy`. This is the least-privilege replacement for the legacy `CLOUDFLARE_GLOBAL_API_KEY` on the deploy path |
| `CLOUDFLARE_ACCOUNT_ID`                                | Staging mirror CI  | —       | Cloudflare account ID for the `wrangler pages deploy` in the mirror CI `deploy-web` step. Staging-mirror GitHub Actions secret only, paired with the Pages-scoped `CLOUDFLARE_API_TOKEN`                                                                                                                                                                                                                                                                                                                                               |
...
| Variable              | Required | Default | Description                                                                                                                                                                                                                          |
| --------------------- | -------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `SLACK_WEBHOOK_URL`   | Prod\*   | —       | Slack incoming webhook URL for API alert delivery                                                                                                                                                                                    |
| `DISCORD_WEBHOOK_URL` | Prod\*   | —       | Discord webhook URL for API alert delivery. Also consumed by the scheduled deploy-currency GitHub Actions canary when configured as the `DISCORD_WEBHOOK_URL` repository secret in `gridl-infra-staging/fjcloud`.                    |
| `DRIFT_MAX_AGE_HOURS` | No       | `24`    | Maximum allowed age, in hours, of the oldest undelivered mirror commit before `scripts/canary/deploy_currency_check.sh` pages. Owner: `scripts/canary/deploy_currency_check.sh`; unset workflow runs use the canary default of `24`. |
...
| `FLAPJACK_DEV_DIR`            | No       | —                                                       | `local-dev-up.sh`, `local-signoff.sh`, `integration-up.sh`, `restart-region.sh`             | Explicit local `flapjack_dev` checkout path (repo root or `engine/` subdirectory). When this selects a source checkout, `scripts/lib/flapjack_binary.sh` builds `flapjack-server` through Cargo, writes/validates the source-build receipt, and emits provenance; callers must not accept an existing same-version binary unless that shared receipt and exact engine identity are current. Release artifact and manifest inputs remain helper-function concerns, with no environment variable contract here |
| `FLAPJACK_DEV_DIR_CANDIDATES` | No       | Built-in nearby checkout list                           | `local-dev-up.sh`, restart helpers, test/dev overrides                                      | Optional space-separated directory candidates checked after `FLAPJACK_DEV_DIR` and before built-in nearby defaults. Candidate source checkouts use the same helper-owned Cargo build and source receipt semantics as `FLAPJACK_DEV_DIR`; restart-critical callers may continue through candidates only when the selected directory is absent or has no source-backed binary, not when selected source receipt validation fails                                                                 |
| `CATALOG_LIFECYCLE_SERVICE_WINDOW_API_BUILD_ROOT` | No | `infra/` | `catalog_lifecycle_service_window_live_probe.sh` | Test/dev override for the service-window live probe's API binary provenance check. The supplied `--api-binary` must resolve under this root's `target/debug/` tree. |
| `CATALOG_LIFECYCLE_SERVICE_WINDOW_ENGINE_BUILD_ROOT` | No | `FLAPJACK_DEV_DIR` or adjacent `flapjack_dev` | `catalog_lifecycle_service_window_live_probe.sh` | Test/dev override for the service-window live probe's engine binary provenance check. The supplied `--engine-binary` must resolve under this root's `target/debug/` tree; the probe also accepts `FLAPJACK_DEV_DIR/engine/target/debug/`. |
| `CATALOG_LIFECYCLE_SERVICE_WINDOW_HARD_ERASE_EMAIL` | No | `service-window-hard-erase@example.com` | `catalog_lifecycle_service_window_live_probe.sh` | Dedicated account email for the service-window hard-erasure tombstone matrix arm. |
| `CATALOG_LIFECYCLE_SERVICE_WINDOW_HARD_ERASE_PASSWORD` | No | `Integration-Hard-Erase-1!` | `catalog_lifecycle_service_window_live_probe.sh` | Dedicated account password for the service-window hard-erasure tombstone matrix arm. |
| `ALGOLIA_IMPORT_CATALOG_RUN_ID` | No | UTC timestamp plus process ID | `algolia_import_catalog_live_probe.sh` | Isolated run suffix for probe-owned indexes, users, and database names. |
| `ALGOLIA_IMPORT_CATALOG_PREFIX` | No | `fjcloud_import_catalog_probe` | `algolia_import_catalog_live_probe.sh` | Prefix for probe-owned Algolia indexes and local tenant identifiers. |
| `ALGOLIA_IMPORT_CATALOG_RUNTIME_PARENT` | No | `TMPDIR` or `/tmp` | `algolia_import_catalog_live_probe.sh` | Parent directory for secure temporary files and owned stack PID files. |
| `ALGOLIA_IMPORT_CATALOG_API_URL` | No | `http://127.0.0.1:${API_PORT:-3099}` | `algolia_import_catalog_live_probe.sh` | API base URL used by the probe after starting the isolated integration stack. |
| `ALGOLIA_IMPORT_CATALOG_ENGINE_URL` | No | `http://127.0.0.1:${FLAPJACK_PORT:-7799}` | `algolia_import_catalog_live_probe.sh` | Engine base URL used for health checks. |
| `ALGOLIA_IMPORT_CATALOG_INTEGRATION_UP` | No | `scripts/integration-up.sh` | `algolia_import_catalog_live_probe.sh` | Test/dev override for the integration stack startup command. |
| `ALGOLIA_IMPORT_CATALOG_INTEGRATION_DOWN` | No | `scripts/integration-down.sh` | `algolia_import_catalog_live_probe.sh` | Test/dev override for the integration stack teardown command. |
| `ALGOLIA_IMPORT_CATALOG_ENGINE_CONTRACT_CHECK` | No | `scripts/update_algolia_migration_engine_contract.sh` | `algolia_import_catalog_live_probe.sh` | Test/dev override for pinned source-built engine contract validation. |
| `ALGOLIA_IMPORT_CATALOG_CALLER_RUNNER` | No | `scripts/algolia_import_catalog_caller_runner.sh` | `algolia_import_catalog_live_probe.sh` | Test/dev override for the source-built production caller runner. The probe fails closed when the configured runner is unavailable. |
| `ALGOLIA_IMPORT_CATALOG_INVENTORY` | No | `scripts/tests/fixtures/catalog_lifecycle_writers.json` | `algolia_import_catalog_live_probe.sh` | Test/dev override for the writer inventory fixture. Production proof should use the default. |
| `ALGOLIA_IMPORT_CATALOG_ORACLE` | No | `scripts/tests/fixtures/catalog_lifecycle_acceptance_oracles.json` | `algolia_import_catalog_live_probe.sh` | Test/dev override for the acceptance oracle fixture. Production proof should use the default. |
...
### Internal Deployable-Currency Injection

These variables are paired internal runner inputs consumed by `scripts/lib/deployable_currency.sh::probe_staging_deployable_currency`. They are for the in-VPC SES coverage runner seam only; there is no public default or operator-facing staging setting.

| Variable                                  | Required | Default / Fallback | Consumed by                                          | Description                                                                                                                                                                                                                                                                              |
| ----------------------------------------- | -------- | ------------------ | ---------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `FJCLOUD_DEPLOYABLE_CURRENCY_JSON`        | Paired   | —                  | `deployable_currency.sh`                             | Path to the Stage 1 five-key deployable-currency verdict JSON: `schema_version`, `source_sha`, `dev_sha`, `deployable_drift`, and `doc_only_ahead`. Must be configured together with `FJCLOUD_DEPLOYABLE_CURRENCY_SOURCE_SHA`; partial configuration fails closed.                       |
| `FJCLOUD_DEPLOYABLE_CURRENCY_SOURCE_SHA`  | Paired   | —                  | `deployable_currency.sh`                             | Frozen runner SHA expected in the verdict file. Must match the verdict `source_sha`; a mismatch fails closed so the runner cannot reuse a deployable-currency verdict from a different source archive.                                                                                   |
...
| `SERVICE_STATUS`                | No       | `unknown` route fallback | Public `/status` input: publishable values are `operational`, `degraded`, or `outage`. Written by `scripts/set_status.sh`; read at runtime by `web/src/routes/status/+page.server.ts`. Missing or unrecognized values render the route fallback `unknown` / `Status Unavailable`. Parsing and label mapping are owned by `web/src/routes/status/status_contract.ts`. |
| `SERVICE_STATUS_UPDATED`        | No       | unset                   | UTC ISO 8601 timestamp of last status update. Written by `scripts/set_status.sh`; read at runtime by `+page.server.ts`. When unset or empty, `/status` renders no freshness claim.                                                               |
| `SERVICE_STATUS_MESSAGE`        | No       | unset                   | Optional public incident message shown on `/status`. Written by `scripts/set_status.sh --message`; read at runtime by `+page.server.ts`. Omitted or empty clears the message from the page.                                                    |
