...
test_missing_env_file_and_amis_returns_blocked_json() {
...
PY
2>/dev/null || true)"
    assert_eq "$RUN_EXIT_CODE" "0" "missing env-file and AMIs should exit 0 with blocked summary"
    ...
    assert_contains "$blocker_details" "--api-ami-id" "blocked prerequisites should include --api-ami-id remediation"
    assert_contains "$blocker_details" "--flapjack-ami-id" "blocked prerequisites should include --flapjack-ami-id remediation"
    assert_no_runtime_smoke_call "blocked prerequisites should not delegate to runtime-smoke owner"
}
test_env_file_present_but_amis_missing_blocks_only_on_amis() {
    require_live_e2e_script_for_contract "env-file present, AMIs missing blocked contract" || return 0
    setup_workspace
    _run_live_e2e_evidence --args "--env staging --domain api.flapjack.foo --artifact-dir $TEST_WORKSPACE/artifacts --env-file $TEST_WORKSPACE/inputs/live_e2e.explicit.env"
    local blocker_keys
    blocker_keys="$(python3 -c 'import json,sys; p=json.loads(sys.argv[1]); print("|".join(r.get("blocker","") for r in p.get("external_blockers",[]) if isinstance(r,dict)))' "$RUN_STDOUT" 2>/dev/null || true)"
    assert_eq "$RUN_EXIT_CODE" "0" "env-file present, AMIs missing should exit 0 with blocked summary"
    assert_valid_json "$RUN_STDOUT" "env-file present, AMIs missing should emit valid JSON"
    assert_eq "$(json_field "$RUN_STDOUT" "overall_verdict")" "blocked" "env-file present, AMIs missing should set overall_verdict=blocked"
    assert_eq "$blocker_keys" "missing_api_ami_id|missing_flapjack_ami_id" "env-file present, AMIs missing should report both distinct blockers"
    assert_no_runtime_smoke_call "env-file present, AMIs missing should not delegate to runtime-smoke owner"
}
...
test_runtime_smoke_pass_normalizes_to_pass_verdict() {
    ...
    setup_workspace
    _run_live_e2e_evidence --args "--env staging --domain api.flapjack.foo --artifact-dir $TEST_WORKSPACE/artifacts --env-file $TEST_WORKSPACE/inputs/live_e2e.explicit.env --api-ami-id ami-0123456789abcdef0 --flapjack-ami-id ami-0fedcba9876543210"
...
test_runtime_smoke_failure_normalizes_to_fail_verdict() {
    ...
    _run_live_e2e_evidence \
        --args "--env staging --domain $domain --artifact-dir $TEST_WORKSPACE/artifacts --env-file $env_file --api-ami-id ami-0123456789abcdef0 --flapjack-ami-id ami-0fedcba9876543210" \
        "RUNTIME_SMOKE_MOCK_EXIT_CODE=23" \
        "RUNTIME_SMOKE_MOCK_STDERR=runtime-smoke-mock-failure"
    expected_command="$(printf '%s %s %s %s %s %s %s %s %s %s %s' "$(shell_quote_for_script "$TEST_WORKSPACE/ops/terraform/tests_stage7_runtime_smoke.sh")" --env staging --domain "$(shell_quote_for_script "$domain")" --env-file "$(shell_quote_for_script "$env_file")" --api-ami-id ami-0123456789abcdef0 --flapjack-ami-id ami-0fedcba9876543210)"
    expected_command_json="$(python3 -c 'import json,sys; print(json.dumps(sys.argv[1]))' "$expected_command")"
    raw_command_json="$(python3 -c 'import json,sys; print(json.dumps(sys.argv[1]))' "$TEST_WORKSPACE/ops/terraform/tests_stage7_runtime_smoke.sh --env staging --domain $domain --env-file $env_file --api-ami-id ami-0123456789abcdef0 --flapjack-ami-id ami-0fedcba9876543210")"
...
test_artifact_dir_rejects_existing_file_without_partial_run_directory() {
    ...
    _run_live_e2e_evidence --args "--env staging --domain api.flapjack.foo --artifact-dir $artifact_file --env-file $TEST_WORKSPACE/inputs/live_e2e.explicit.env --api-ami-id ami-0123456789abcdef0 --flapjack-ami-id ami-0fedcba9876543210"
...
test_run_billing_rehearsal_without_env_file_is_blocked_without_owner_call() {
    ...
    assert_billing_opt_in_blocked_case \
        ...
        "--env staging --domain api.flapjack.foo --artifact-dir $TEST_WORKSPACE/artifacts --api-ami-id ami-0123456789abcdef0 --flapjack-ami-id ami-0fedcba9876543210 --run-billing-rehearsal --month 2026-03 --confirm-live-mutation" \
...
test_run_billing_rehearsal_without_month_is_blocked_without_owner_call() {
    ...
    assert_billing_opt_in_blocked_case \
        ...
        "--env staging --domain api.flapjack.foo --artifact-dir $TEST_WORKSPACE/artifacts --env-file $TEST_WORKSPACE/inputs/live_e2e.explicit.env --api-ami-id ami-0123456789abcdef0 --flapjack-ami-id ami-0fedcba9876543210 --run-billing-rehearsal --confirm-live-mutation" \
        "--month is required" \
        "yes"
}
test_run_billing_rehearsal_without_confirm_is_blocked_without_owner_call() {
    ...
    assert_billing_opt_in_blocked_case \
        ...
        "--env staging --domain api.flapjack.foo --artifact-dir $TEST_WORKSPACE/artifacts --env-file $TEST_WORKSPACE/inputs/live_e2e.explicit.env --api-ami-id ami-0123456789abcdef0 --flapjack-ami-id ami-0fedcba9876543210 --run-billing-rehearsal --month 2026-03" \
...
test_billing_delegation_boundary_forwards_owner_args_only() {
    ...
    _run_live_e2e_evidence --args "--env staging --domain api.flapjack.foo --artifact-dir $TEST_WORKSPACE/artifacts --env-file $env_file --api-ami-id ami-0123456789abcdef0 --flapjack-ami-id ami-0fedcba9876543210 --run-deploy --run-migrate --run-billing-rehearsal --month 2026-03 --confirm-live-mutation"
    ...
    assert_contains "$runtime_call" "--api-ami-id ami-0123456789abcdef0" "runtime smoke should receive the independent API AMI"
    assert_contains "$runtime_call" "--flapjack-ami-id ami-0fedcba9876543210" "runtime smoke should receive the independent Flapjack AMI"
    ...
    assert_not_contains "$billing_call" "--api-ami-id" "billing owner should not receive the API AMI flag"
    assert_not_contains "$billing_call" "--flapjack-ami-id" "billing owner should not receive the Flapjack AMI flag"
...
PYTHON3_WRAPPER
    ...
    _run_live_e2e_evidence \
        ...
        --args "--env staging --domain api.flapjack.foo --artifact-dir $TEST_WORKSPACE/artifacts --env-file $env_file --api-ami-id ami-0123456789abcdef0 --flapjack-ami-id ami-0fedcba9876543210 --run-billing-rehearsal --month 2026-03 --confirm-live-mutation" \
...
test_billing_owner_execution_normalizes_rows_and_logs() {
    ...
    setup_workspace
    _run_live_e2e_evidence \
        --args "--env staging --domain api.flapjack.foo --artifact-dir $TEST_WORKSPACE/artifacts --env-file $TEST_WORKSPACE/inputs/live_e2e.explicit.env --api-ami-id ami-0123456789abcdef0 --flapjack-ami-id ami-0fedcba9876543210 --run-billing-rehearsal --month 2026-03 --confirm-live-mutation" \
