...
_resolve_computed_api_url() {
    local workspace_path="${REPO_ROOT}/web"
...
check_service() {
    ...
    if web_runtime_service_is_ready "$url"; then
...
check_flapjack_binary_provenance() {
    ...
    if [ -n "$flapjack_bin" ] && [ -x "$flapjack_bin" ]; then
        echo "  OK: Flapjack binary provenance $(flapjack_source_provenance_summary)"
        return
    fi

    echo "FAIL: Flapjack binary provenance check failed — rebuild or clean the selected FLAPJACK_DEV_DIR checkout$(shared_remediation_suffix)" >&2
    errors=$((errors + 1))
}

check_api_compatibility() {
    local api_url="$1"
    if api_supports_capability "$api_url" "$FJCLOUD_API_PREVIEW_EVENTS_CAPABILITY"; then
        echo "  OK: API advertises ${FJCLOUD_API_PREVIEW_EVENTS_CAPABILITY}"
        return
    fi
    echo "FAIL: API at ${api_url} is live but does not advertise ${FJCLOUD_API_PREVIEW_EVENTS_CAPABILITY} — stop the exact listener on its API port, then restart scripts/api-dev.sh$(shared_remediation_suffix)" >&2
    errors=$((errors + 1))
    return 1
}
...
if check_service \
    ...
    "${api_url}/health"; then
    api_is_ready=1
    if ! check_api_compatibility "$api_url"; then
        api_is_ready=0
    fi
fi
