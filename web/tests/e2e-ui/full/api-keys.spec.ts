...
async function openCreateDialog(page: Page): Promise<void> {
	...
	await expectEditorDialogTokens(page, page.getByTestId('api-keys-create-dialog'));
}
...
test.describe('API Keys page', () => {
	...
	test('create key through UI does not discover seeded indexes in current runtime path', async ({
		...
		await seedCustomerIndex(activeCustomer, seededIndexName, testRegion);
	...
	test('index filter updates URL, preserves query params, and includes all-index keys', async ({
		...
		await seedCustomerIndex(activeCustomer, indexAName, testRegion);
		await seedCustomerIndex(activeCustomer, indexBName, testRegion);
	...
	test('copy button shows temporary copied feedback for a key row', async ({
		...
		const copiedToast = page.getByText('API key copied');
		await expect(copiedToast).toBeVisible({ timeout: 10_000 });
		await expect.poll(async () => await copyButton.textContent()).toBe('Copy');
		await page.mouse.move(0, 0);
		await expect(copiedToast).toBeHidden({ timeout: TOAST_DURATION_MS + 2_000 });
	});

	test('revoke key removes it from the table and shows success toast', async ({
		page,
		seedApiKey,
		listApiKeys
	}) => {
		...
		await expectClassTokens(page.getByTestId('confirm-dialog'), ['bg-flapjack-ink/55']);
		await expectClassTokens(page.getByTestId('confirm-input'), [
			'border-flapjack-ink/30',
			'focus:border-flapjack-plum'
		]);
		await expectClassTokens(page.getByTestId('confirm-confirm-btn'), ['bg-flapjack-plum']);
		await expectClassTokens(page.getByTestId('confirm-cancel-btn'), [
			'border-flapjack-ink/30',
			'text-flapjack-ink/80',
			'hover:bg-flapjack-cream/80'
		]);
		...
		// Assert: toast appears immediately after confirm (before row removal, since
		// TOAST_DURATION_MS=4s and row-disappearance timeout is 5s)
		const revokeToast = page.getByText(`API key '${name}' revoked.`);
		await expect(revokeToast).toBeVisible({ timeout: 10_000 });
