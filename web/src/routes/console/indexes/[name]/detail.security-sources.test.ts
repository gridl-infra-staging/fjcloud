...
describe('Index detail page — Security Sources tab', () => {
	...
	it('security source delete uses ConfirmDialog cancel/confirm before submitting delete action', async () => {
		...
		await fireEvent.click(
			screen.getByRole('button', { name: 'Delete security source 192.168.1.0/24' })
		);
	...
	it('routes append success from detail-page form state to a toast without inline copy', async () => {
		renderPage({}, {
		...
		expect(toastSuccessMock).toHaveBeenCalledWith('Security source added.', {
			duration: TOAST_DURATION_MS
		});
		expect(screen.queryByText(/security source added/i)).not.toBeInTheDocument();
	});

	it('routes delete success from detail-page form state to a toast without inline copy', async () => {
		renderPage({}, {
		...
		expect(toastSuccessMock).toHaveBeenCalledWith('Security source deleted.', {
			duration: TOAST_DURATION_MS
		});
		expect(screen.queryByText(/security source deleted/i)).not.toBeInTheDocument();
	});
