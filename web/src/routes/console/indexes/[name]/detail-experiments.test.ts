...
vi.mock('$app/paths', () => ({
	resolve: (path: string, params?: Record<string, string>) => {
		if (path === '/console/indexes/[name]/experiments/[experimentId]') {
			return `../../console/indexes/${params?.name}/experiments/${params?.experimentId}`;
		}
		if (path === '/console/indexes/[name]') {
			return `/console/indexes/${params?.name}`;
		}
		return path;
	}
}));
...
describe('Index detail page — Experiments', () => {
	...
	it('experiments tab empty state shows create button', async () => {
		...
		expect(screen.getByTestId('experiments-section')).toBeInTheDocument();
...
describe('Index detail page — API Activity Log', () => {
	...
	it('shows API Activity Log toggle button in the header', () => {
		render(IndexDetailPage, {
			data: createMockPageData({
				experiments: { abtests: [], count: 0, total: 0 },
		...
		expect(screen.getByRole('button', { name: 'API Activity Log' })).toBeInTheDocument();
		expect(screen.queryByRole('button', { name: 'Search Log' })).not.toBeInTheDocument();
	});

	it('toggling API Activity Log shows session-scoped log table headers', async () => {
		render(IndexDetailPage, {
			data: createMockPageData({
				experiments: { abtests: [], count: 0, total: 0 },
				debugEvents: { events: [], count: 0 }
		...
		expect(screen.getByRole('heading', { name: 'API Activity Log' })).toBeInTheDocument();
		expect(screen.getByText(/captured in this browser session/i)).toBeInTheDocument();
	...
	it('API Activity Log panel starts with empty state message', async () => {
		render(IndexDetailPage, {
			data: createMockPageData({
				experiments: { abtests: [], count: 0, total: 0 },
				debugEvents: { events: [], count: 0 }
	...
	it('records settings and suggestions form results in the shared API activity log', async () => {
	...
	it('records repeated identical form results as separate API activity log entries', async () => {
		const data = createMockPageData({
			experiments: { abtests: [], count: 0, total: 0 },
			experimentResults: {},
			debugEvents: { events: [], count: 0 }
