<script lang="ts">
	...
	function openCreateDialog(type: SynonymType, trigger: HTMLElement): void {
		editorMode = 'create';
	...
	function openEditDialog(synonym: Synonym, trigger: HTMLElement): void {
		editorMode = 'edit';
...
>
	<div class="mb-4 flex flex-wrap items-center justify-between gap-3">
		...
		<div class="flex flex-wrap items-center gap-2">
			{#if hasSynonyms}
				<form method="POST" action="?/clearSynonyms" use:enhance bind:this={clearAllFormRef}>
					<button
						type="button"
			...
			<button
				...
				onclick={(event) => openCreateDialog('synonym', event.currentTarget as HTMLElement)}
			>
				Add Synonym
			</button>
		</div>
			...
			{:else}
				...
				<div class="flex flex-wrap gap-2">
					...
					<button
						...
						class="rounded-md border border-flapjack-ink/30 px-3 py-2 text-sm text-flapjack-ink/80 hover:bg-flapjack-cream/80"
						onclick={(event) =>
							openCreateDialog('onewaysynonym', event.currentTarget as HTMLElement)}
					>
						Add One-way
					</button>
				</div>
					...
					<div class="mb-2 flex flex-wrap items-center justify-between gap-2">
						...
						<div class="flex items-center gap-2">
							<button
								...
								onclick={(event) => openEditDialog(synonym, event.currentTarget as HTMLElement)}
							>
								Edit
							</button>
	...
	<form method="POST" action="?/saveSynonym" use:enhance bind:this={saveSynonymFormRef}>
