...
- `indexes?: string[]` — index names stored on the key as management metadata (empty / absent = all indexes). Enforced only on `ApiKeyAuth` routes that call `ApiKeyAuth::require_index`, currently `GET /discover?index=...` via `infra/api/src/routes/discovery.rs::discover`.
- `restrict_sources?: string[]` — IP CIDR ranges. Optional. Enforced on `ApiKeyAuth` routes (`apikeyauth_route_consumers=1`, currently `infra/api/src/routes/discovery.rs::discover` at `infra/api/src/routes/discovery.rs:23`) by `infra/api/src/auth/api_key.rs::source_ip_allowed` (`infra/api/src/auth/api_key.rs:23`), invoked from `ApiKeyAuth::from_request_parts_with_stage1_decision` (`infra/api/src/auth/api_key.rs:181`). Only a `ConnectInfo`-derived socket peer satisfies the allowlist; an `x-forwarded-for` value never satisfies it, even when `TRUST_PROXY_HEADERS_FOR_RATE_LIMIT` is set. An unresolvable peer or malformed stored CIDR fails closed, and denial reuses the `InvalidToken` response shape. JWT search traffic (`AuthenticatedTenant`) and engine-issued `flapjack_proxy` search keys are outside this boundary.
- `expires_at?: string | null` — ISO-8601 timestamp. `null` / absent = no expiry. Expired management keys are rejected by `infra/api/src/auth/api_key.rs::ApiKeyAuth::from_request_parts_with_stage1_decision`.
- `max_hits_per_query?: number | null` — per-query hit cap. Optional. **STORED ONLY — no request-time owner yet.** No `ApiKeyAuth` route accepts a per-query hit count, so there is no request-time owner to clamp at. Enforcement becomes possible when a hit-count-bearing route authenticates through `ApiKeyAuth`. This is a recorded scope decision, not a deferral.
- `max_queries_per_ip_per_hour?: number | null` — per-IP rate cap. Optional. Enforced on `ApiKeyAuth` routes (`apikeyauth_route_consumers=1`, currently `infra/api/src/routes/discovery.rs::discover` at `infra/api/src/routes/discovery.rs:23`) by `infra/api/src/auth/api_key.rs::enforce_api_key_hourly_limit` (`infra/api/src/auth/api_key.rs:54`), invoked from `ApiKeyAuth::from_request_parts_with_stage1_decision` (`infra/api/src/auth/api_key.rs:204`) and backed by `infra/api/src/router.rs::RateLimiter::check_with_limit` (`infra/api/src/router.rs:131`). The bucket is `(api_key_id, client_ip)` across a sliding hour. Limit denials return 429 with a `Retry-After` header through `AuthError::RateLimited` (`infra/api/src/auth/error.rs:27`). Restarts reset the counter; each API instance grants its own budget; this is not distributed enforcement. JWT search traffic (`AuthenticatedTenant`) and engine-issued `flapjack_proxy` search keys are outside this boundary.
...
**Security framing — read before relying on advisory fields as a boundary.** Operators may tell customers that `restrict_sources` and `max_queries_per_ip_per_hour` are enforced only for `ApiKeyAuth` routes, currently just discovery; they must not describe those controls as covering JWT customer search traffic or engine-issued `flapjack_proxy` search keys. Operators may tell customers that `restrict_sources` fails closed when the socket peer is unavailable or the stored CIDR is malformed, and that forwarded headers do not satisfy the allowlist. Operators may tell customers that `max_queries_per_ip_per_hour` is an in-process sliding-hour limiter with `Retry-After`; restarts reset the counter; each API instance grants its own budget; this is not distributed enforcement. Operators MUST NOT tell customers that `max_hits_per_query` clamps or rejects oversized queries until a hit-count-bearing route authenticates through `ApiKeyAuth`. Expiry is enforced during `ApiKeyAuth` extraction, while index membership is enforced only by `ApiKeyAuth` consumers that call `require_index`, currently discovery.
...
The shipped page now renders the Wave B 3A self-service UI: per-index filter, lifecycle fields, copy-with-feedback, `EditorDialog` create flow, and typed `ConfirmDialog` revoke flow. The populated, empty, create-dialog, and one-time key-reveal states have structural axe proof in `web/src/routes/console/api-keys/api-keys.test.ts`. Remaining enforcement gap: `max_hits_per_query` is stored/displayed metadata because no hit-count-bearing route authenticates through `ApiKeyAuth`. The enforced `restrict_sources` and `max_queries_per_ip_per_hour` boundaries are documented above; engine-issued search keys and non-`ApiKeyAuth` search traffic remain outside them.

## Enforcement Boundaries (runtime owner anchors)

- `infra/api/src/auth/api_key.rs` (`ApiKeyAuth::from_request_parts_with_stage1_decision`) rejects expired management keys during authentication.
- `infra/api/src/auth/api_key.rs` (`ApiKeyAuth::require_index`) is the single index-allowlist policy owner; an empty allowlist remains unrestricted.
- `infra/api/src/routes/discovery.rs` (`discovery::discover`) is currently the only index-bearing `ApiKeyAuth` route consumer and calls `require_index` before discovery lookup.
