...
#[tokio::test]
async fn get_index_settings_mirrors_engine_attribute_arrays_to_dashboard_keys() {
    let (http, _ssm, proxy) = setup().await;

    http.push_json_response(
        200,
        json!({
            "attributesForFaceting": ["category", "filterOnly(brand)"],
    ...
    assert_eq!(
        ...
        json!(["category", "filterOnly(brand)"])
    );
    assert_eq!(
