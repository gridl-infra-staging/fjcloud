...
async fn migration_pressure_allows_after_slot_freed() {
    let repo = crate::common::mock_index_migration_repo();
...
async fn cutover_abort_on_dest_failure_restores_source_routing() {
    ...
    enqueue_source_ops(&f.http_client, &f.physical_uid, 100);
    ...
    f.http_client.enqueue_response(Ok(MigrationHttpResponse {
        status: 200,
        body: crate::common::oplog_metric(&f.physical_uid, 100),
    }));
    f.http_client.enqueue_response(Ok(MigrationHttpResponse {
        status: 200,
        body: crate::common::oplog_metric(&f.physical_uid, 95),
    }));
    f.http_client.enqueue_response(Ok(MigrationHttpResponse {
        status: 200,
        body: "{}".to_string(),
    ...
    assert!(
        requests.iter().any(|r| {
            r.url
                == format!(
                    "{}/internal/resume/{}",
                    f.source_vm.flapjack_url, f.physical_uid
                )
        }),
