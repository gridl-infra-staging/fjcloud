<!-- [scrai:start] -->
## tests

| File | Summary |
| --- | --- |
| customer_broadcast_smoke.sh | Focused smoke coverage for scripts/customer_broadcast.sh wrapper contract. |
| probe_ses_bounce_complaint_e2e_smoke.sh | Smoke tests for scripts/probe_ses_bounce_complaint_e2e.sh. |

| Directory | Summary |
| --- | --- |
| fixtures | — |
| lib | A collection of shell script test helpers and fixtures providing shared utilities for integration tests, chaos testing, mock management, and staging billing rehearsal scenarios. |
<!-- [scrai:end] -->
