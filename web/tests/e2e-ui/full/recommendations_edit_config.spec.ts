...
const RECOMMENDATION_FIELD_LABELS = {
	...
	'recommendations-facet-name': 'Facet Name',
	'recommendations-facet-value': 'Facet Value',
	'recommendations-threshold': 'Threshold',
	'recommendations-max-recommendations': 'Max Recommendations'
} as const;
...
test.describe('Recommendations edit configuration dialog', () => {
	...
	test('edit configuration updates inline state and waits for explicit submit', async ({
		...
		await expect(recommendationField(section, 'recommendations-facet-name')).toHaveValue(
			seeded.facetName
		);
		await expect(recommendationField(section, 'recommendations-facet-value')).toHaveValue(
			seeded.facetValue
		);
		await expect(recommendationField(section, 'recommendations-threshold')).toHaveValue('12');
		await expect(recommendationField(section, 'recommendations-max-recommendations')).toHaveValue(
			'9'
		);
		await expect(recommendationField(section, 'recommendations-object-id')).toHaveCount(0);
