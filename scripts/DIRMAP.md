...
| Directory | Summary |
| --- | --- |
| canary | The canary directory contains synthetic monitoring and health check scripts that validate fjcloud's end-to-end customer flows (signup through billing setup), external service availability, email deliverability, and contract tests for system boundaries and third-party integrations like OAuth and Stripe webhooks. |
| chaos | This directory contains chaos engineering and failure-injection test scripts that validate system resilience, including HA failover detection, region health monitoring, metering breaker alerting, and recovery procedures. |
| dev | This directory contains a Python script for migrating and consolidating integration tests. |
| launch | The `launch/` directory contains bash helper scripts for verifying and testing fjcloud's staging deployment, including staging API validation, billing rehearsal captures, browser automation against staging, synthetic traffic generation, and AWS SSM execution utilities. |
| lib | This is a shared library of bash script utilities providing reusable helpers for HTTP requests, database operations, Stripe integration, alert dispatch, and validation checks (security, metering, health) across fjcloud's testing, integration, and ops scripts. |
| load | The load directory contains load testing validation utilities, with `lib/load_checks.sh` as the primary tool for detecting performance regressions by comparing offline simulated harness results against live load test outcomes. |
| reliability | The reliability directory contains shell scripts and utilities for capacity profiling across three data tiers, security automation checks (cargo audit, secret scanning, unsafe code patterns), and backend reliability gates that validate system performance and security. |
| stripe | This directory contains shell scripts for configuring Stripe integration, including tools to set up the Customer Portal configuration and create the product catalog for specific Stripe accounts. |
| tests | This directory contains shell script smoke tests for customer broadcast and SES event handling, along with shared utilities for test infrastructure including assertions, mocks, and harnesses for chaos, integration, budget validation, and staging billing rehearsal tests. |
| vlm | The vlm directory contains verdict-bundle aggregation logic and environment variable handling utilities extracted from deployment scripts, supporting the VLM judge's environment value reading contract through a self-contained closure of helper functions. |
| w3_triage | w3_triage is a triage orchestration system that bootstraps by probing live state and discovering audit summaries, then processes those discoveries through rule application, lane authoring, recommendation parsing, and manifest emission for downstream dispatch. |
| canary | The canary directory owns synthetic end-to-end monitoring scripts that continuously verify fjcloud's customer-facing operations, including signup-through-billing flows, external service health, email deliverability, and API contract compliance. |
| chaos | The chaos directory contains failure injection and resilience test scripts that validate Flapjack's handling of region outages, VM failures, and service degradation. |
| dev | The dev directory contains a single Python script, migrate_integration_tests_consolidation.py, which appears to be a placeholder stub for consolidating integration tests. |
| launch | The `launch/` directory contains shell scripts that orchestrate post-deployment validation and integration testing for fjcloud's staging environment, including SSM-backed environment hydration, synthetic traffic seeding, browser-based Playwright test execution, and verification probes for tenant isolation and API configuration. |
| lib | A collection of shared bash utility libraries providing reusable helpers for operational tasks across the fjcloud infrastructure: alert dispatch, billing workflows, API requests, database operations, Stripe integration, environment loading, validation checks, and infrastructure configuration. |
| load | The load directory contains regression testing utilities that compare offline and live load harness results to validate performance consistency across different test environments. |
| reliability | The reliability directory contains automation scripts for capacity profiling across three document tiers, security validation (cargo audit, secret scanning, unsafe code detection), and infrastructure consistency checks. |
| stripe | This directory contains scripts for configuring Stripe integration, including setting up the Stripe Customer Portal and creating the Flapjack product catalog. |
| tests | The tests directory contains shell script-based smoke tests for wrapper scripts and operational probes, alongside shared testing utilities and helpers in lib/ that support integration, e2e, staging, and chaos testing across the codebase. |
| vlm | The vlm directory contains a verdict aggregation script and environment helper utilities extracted from deployment common scripts to support the VLM judge's environment value reading contract. |
| w3_triage | The w3_triage directory contains tools for a W3 audit triage system that bootstraps preflight state by probing live systems and discovering audit summaries, then parses audit recommendations to apply rules and author lane files for dispatch. |
<!-- [scrai:end] -->
