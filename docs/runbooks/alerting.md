...
The scheduled deploy-currency alarm runs from `.github/workflows/deploy_currency.yml` in the public staging mirror `gridl-infra-staging/fjcloud`. It runs hourly and also supports manual `workflow_dispatch` probes. Scheduled runs are guarded to the staging mirror; manual dispatch is allowed in any mirror for operator probes.

The breach rule is owned by `scripts/canary/deploy_currency_check.sh`: page only when GitHub compare reports `ahead_by > 0` and the oldest undelivered mirror commit age is strictly greater than `DRIFT_MAX_AGE_HOURS` (default `24`). Do not replace this with deployed-commit-age or mirror-HEAD-age shortcuts. Deployed-commit-age false-pages after quiet periods or in-flight deploys, while HEAD-age misses continuous-push dead-pipeline drift.

The workflow passes `DISCORD_WEBHOOK_URL` from the staging repository secret and `GITHUB_TOKEN` from `github.token`, then lets `scripts/lib/alert_dispatch.sh::send_critical_alert` and `post_webhook_payload` own Discord transport. Do not duplicate webhook posting or GitHub API logic in the workflow.

For a planned deploy freeze expected to exceed `DRIFT_MAX_AGE_HOURS`, use `scripts/tests/scheduled_alarm_acknowledgements.txt` as the single recorded home for the UTC freeze window, its incident or change owner, and every deliberate scheduled red/skip state. Then silence the schedule explicitly in `gridl-infra-staging/fjcloud` with `gh workflow disable deploy_currency.yml -R gridl-infra-staging/fjcloud`. The liveness guard is `scripts/probe_scheduled_alarm_liveness.sh`: it must continue to report stale or missing scheduled runs, and an acknowledgement never suppresses a deploy-currency breach. Re-enable with `gh workflow enable deploy_currency.yml -R gridl-infra-staging/fjcloud`, run a manual dispatch after the freeze, and retain the dated registry comment as the freeze record. Do not silence this alarm by deleting `DISCORD_WEBHOOK_URL` or weakening the breach rule in `scripts/canary/deploy_currency_check.sh`.

On 2026-08-05, staging `deploy_currency.yml` was `disabled_manually` with zero lifetime runs, while the prod copy skipped by design because of its repository guard. Both existing tests stayed green, and the silence was found only by external audit.

This alarm is anchored to the June 2026 deploy-staleness incident pattern: mirror commits existed while the live `/version` `mirror_sha` stayed behind long enough that a scheduled page should have fired. It has two known blind spots. First, it measures mirror-to-live drift only; it will not page if debbie stops syncing dev-to-mirror. Second, GitHub may auto-disable scheduled workflows after a long quiet period unless the operator confirms the schedule is still armed.

### API panic alarm

The authored CloudWatch alarm is `fjcloud-<env>-api-panics-high`. Its Terraform owner is `ops/terraform/monitoring/main.tf::aws_cloudwatch_metric_alarm.api_panics_high`; its API-side publisher owner is `infra/api/src/services/panics.rs::PanicsPublisher::publish_once`.

The operator-facing metric contract is namespace `fjcloud/api`, metric `PanicsPerPeriod`, dimension `Env`, statistic `Sum`, period `60` seconds, and threshold `> 0`. The CloudWatch datum is the per-period delta published from `MetricsCollector::panic_total()`, not the raw monotonic `panics_total` Prometheus counter.

The alarm uses `treat_missing_data = "notBreaching"`. Interpret missing data only as "no panic datapoint observed"; it is not proof the API is healthy. Keep `api_heartbeat_missing` in `ops/terraform/monitoring/main.tf` as the separate missing-process, lost-egress, and IAM-regression alarm owner.
...
- [ ] Decide which Slack/Discord channels are the production targets (or "log-only is fine for the current public beta volume" — but explicitly choose).
