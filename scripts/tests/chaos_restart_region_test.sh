...
test_restart_region_falls_back_to_path_flapjack_binary() {
    ...
    # Wait for the backgrounded pinned legacy artifact to start.
    sleep 0.5
    ...
    assert_contains "$output" "legacy-release:$tmp_dir/bin/flapjack" \
        "should report the pinned legacy PATH artifact provenance"
}
