...
| repos | This directory implements the repository pattern for fjcloud's domain entities, providing abstract trait definitions paired with PostgreSQL and in-memory concrete implementations for managing customers, billing, storage, deployments, and infrastructure resources. |
...
| routes | The routes directory contains HTTP API endpoints for fjcloud, organized by domain including authentication, billing, invoicing, account management, search indexes, and S3-compatible storage operations. |
...
| services | The services directory contains the core business logic modules for the fjcloud API, including infrastructure provisioning and scheduling, data replication and migration, admin audit and alerting, email and webhook integration, S3-compatible storage proxying via Garage, and health monitoring. |
