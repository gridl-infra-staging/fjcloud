...
async function expectDeploymentPanelHasNoDeployments(page: Page): Promise<void> {
	await expect(page.getByText('No deployments found for this customer.')).toBeVisible();
	await expect(page.getByText('Deployment data unavailable.')).toBeHidden();
}
...
test.describe('Admin customer detail workflows', () => {
	...
	test('terminate deployment action removes the seeded active deployment @p0_coverage', async ({
		page,
		createUser,
		seedAdminDeployment
	}) => {
		const { customer, customerName } = await createAdminCustomer(
			createUser,
			'Admin Terminate Target',
			'admin-terminate'
		);
		const deployment = await seedAdminDeployment(customer);

		await gotoCustomerDetail(page, customer.customerId, customerName);
		await page.getByRole('button', { name: 'Deployments' }).click();
		const deploymentRow = page.getByRole('row').filter({ hasText: deployment.region });
		await expect(deploymentRow).toHaveCount(1);
		await expect(deploymentRow.getByText(/^running$/i)).toBeVisible();

		await deploymentRow.getByTestId('terminate-deployment-button').click();
		await expect(page.getByText('Deployment terminated')).toBeVisible({ timeout: 30_000 });
		await expectDeploymentPanelHasNoDeployments(page);
