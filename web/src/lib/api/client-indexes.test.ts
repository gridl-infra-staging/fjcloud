...
describe('ApiClient - index endpoints', () => {
	...
	it('POST /indexes/:name/restore starts a customer-owned cold restore', async () => {
		const expected = {
		...
		const result = await client.restoreIndex('products');

		expect(fetch).toHaveBeenCalledWith(`${BASE_URL}/indexes/products/restore`, {
			method: 'POST',
			headers: { 'Content-Type': 'application/json', Authorization: 'Bearer my-jwt-token' }
		});
		expect(result).toEqual(expected);
