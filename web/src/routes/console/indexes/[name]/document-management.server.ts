...
type BrowseExpectation = {
	presentObjectIDs?: string[];
	absentObjectID?: string;
};

const POST_WRITE_BROWSE_ATTEMPTS = 5;
const POST_WRITE_BROWSE_DELAY_MS = 200;
...
function objectIDsFromBatchRequest(batchRequest: AddObjectsRequest): string[] {
	return batchRequest.requests
		.map((requestEntry) => requestEntry.body?.objectID)
		.filter((objectID): objectID is string => typeof objectID === 'string' && objectID.length > 0);
}

function browseMatchesExpectation(
	...
	const allExpectedPresent = (expectation.presentObjectIDs ?? []).every((objectID) =>
		objectIDs.has(objectID)
	);
	const expectedAbsent = !expectation.absentObjectID || !objectIDs.has(expectation.absentObjectID);
	return allExpectedPresent && expectedAbsent;
}
...
function objectIDBrowseRequest(objectID: string, hitsPerPage: number): BrowseObjectsRequest {
	return {
		query: objectID,
		hitsPerPage: hitsPerPage > 0 ? hitsPerPage : DEFAULT_DOCUMENT_HITS_PER_PAGE
	};
}
...
async function browseAfterDocumentWrite(
	...
	indexName: string,
	refreshRequest: BrowseObjectsRequest,
	expectation: BrowseExpectation
): Promise<BrowseObjectsResponse> {
	let latestDocuments = emptyDocumentsBrowse(refreshRequest.query ?? '');
	for (let attempt = 0; attempt < POST_WRITE_BROWSE_ATTEMPTS; attempt += 1) {
		latestDocuments = normalizeDocumentsBrowseResponse(
			(await api.browseObjects(indexName, refreshRequest)) as Record<string, unknown>,
			refreshRequest.query ?? ''
		);
		if (browseMatchesExpectation(latestDocuments, expectation)) {
			return latestDocuments;
		}
		await wait(POST_WRITE_BROWSE_DELAY_MS);
	}
	return latestDocuments;
}
...
export async function uploadDocumentsAction({ request, indexName, token }: DocumentActionArgs) {
	...
	try {
		...
		const documents = await browseAfterDocumentWrite(api, indexName, refreshRequest, {
			presentObjectIDs: objectIDsFromBatchRequest(batchRequest)
		});
...
export async function addDocumentAction({ request, indexName, token }: DocumentActionArgs) {
	...
	try {
		...
		const expectedObjectID =
			typeof documentBody.objectID === 'string' && documentBody.objectID.length > 0
				? [documentBody.objectID]
				: [];
		const postWriteBrowseRequest =
			expectedObjectID.length > 0
				? objectIDBrowseRequest(expectedObjectID[0], refreshRequest.hitsPerPage ?? 0)
				: refreshRequest;
		const documents = await browseAfterDocumentWrite(api, indexName, postWriteBrowseRequest, {
			presentObjectIDs: expectedObjectID
		});
