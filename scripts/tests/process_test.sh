...
test_kills_matching_wrapper_command_line() {
    local tmp_dir
    tmp_dir=$(mktemp -d)
    trap "rm -rf '$tmp_dir'" RETURN

    # start-metering.sh records the `cargo run` wrapper PID before Cargo has
    # exec'd or spawned the final fj-metering-agent binary. Teardown must still
    # accept that PID, but only when the full command line proves it is the
    # metering-agent wrapper and not some unrelated cargo process.
    bash -c 'while true; do sleep 1; done' \
        cargo run --manifest-path /tmp/fjcloud/infra/Cargo.toml -p metering-agent &
    local pid=$!
    echo "$pid" > "$tmp_dir/metering.pid"

    LOG_OUTPUT=""
    kill_pid_file "$tmp_dir/metering.pid" "metering-agent" "metering-agent" "*cargo run --manifest-path */infra/Cargo.toml -p metering-agent*"

    if kill -0 "$pid" 2>/dev/null; then
        kill "$pid" 2>/dev/null || true
        fail "kill_pid_file should kill the metering-agent cargo wrapper"
    else
        pass "kill_pid_file kills matching wrapper command line"
    fi

    assert_contains "$LOG_OUTPUT" "Stopping metering-agent" \
        "log should mention stopping the named wrapper process"
}

test_skips_unrelated_wrapper_command_line() {
    local tmp_dir
    tmp_dir=$(mktemp -d)
    trap "rm -rf '$tmp_dir'" RETURN

    bash -c 'while true; do sleep 1; done' \
        cargo run --manifest-path /tmp/fjcloud/infra/Cargo.toml -p api &
    local pid=$!
    echo "$pid" > "$tmp_dir/metering.pid"
    ...
    if kill -0 "$pid" 2>/dev/null; then
        pass "kill_pid_file skips unrelated wrapper command line"
        kill "$pid" 2>/dev/null || true
    else
        fail "kill_pid_file should not kill an unrelated cargo wrapper"
    fi

    assert_contains "$LOG_OUTPUT" "stale PID file" \
        "log should mention stale PID file when wrapper command line does not match"
}
