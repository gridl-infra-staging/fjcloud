...
assert_contains_active "$dns_main_file" 'domain_name\s*=\s*local\.deployment_domain' \
  "ACM certificate domain_name uses the normalized deployment domain (not hardcoded)"

assert_contains_active "$dns_main_file" 'subject_alternative_names.*\*\.\$\{local\.deployment_domain\}' \
  "ACM certificate wildcard SAN uses the normalized deployment domain"
...
assert_contains_active "$dns_main_file" 'acm_validation_record_keys\s*=\s*toset\(\["apex"\]\)' \
  "ACM validation records use a plan-known key set"

assert_resource_block_contains "$dns_main_file" "cloudflare_dns_record" "cert_validation" 'for_each\s*=\s*local\.acm_validation_record_keys' \
  "ACM validation Cloudflare records iterate over plan-known keys"

assert_not_contains_active "$dns_main_file" 'for_each\s*=\s*local\.acm_validation_records' \
  "ACM validation Cloudflare records do not iterate over apply-time record maps"

assert_not_contains_active "$dns_main_file" 'dvo\.resource_record_name[^[:cntrl:]]*=>' \
  "ACM validation map keys are not derived from ACM record names"

assert_not_contains_active "$dns_main_file" 'dvo\.resource_record_value[^[:cntrl:]]*=>' \
  "ACM validation map keys are not derived from ACM record values"
...
assert_contains_active "$dns_main_file" 'email_identity\s*=\s*local\.deployment_domain' \
  "SES identity uses the normalized deployment domain"
...
assert_contains_active "$dns_main_file" '_domainkey\.\$\{local\.deployment_domain\}' \
  "SES DKIM record names use the normalized deployment domain"
...
assert_contains_active "$runtime_params_main_file" 'resource "aws_ssm_parameter" "runtime_algolia_migration_enabled"' \
  "runtime_params owns Algolia migration availability parameter"

assert_resource_block_contains "$runtime_params_main_file" "aws_ssm_parameter" "runtime_algolia_migration_enabled" 'name\s*=\s*"/fjcloud/\$\{var\.env\}/algolia_migration_enabled"' \
  "runtime_params publishes canonical Algolia migration availability path"

assert_resource_block_contains "$runtime_params_main_file" "aws_ssm_parameter" "runtime_algolia_migration_enabled" 'value\s*=\s*"false"' \
  "runtime_params initializes Algolia migration availability fail-closed"

assert_resource_block_contains "$runtime_params_main_file" "aws_ssm_parameter" "runtime_algolia_migration_enabled" 'ignore_changes\s*=\s*\[value\]' \
  "runtime_params leaves post-provision Algolia migration value changes to the operator script"

assert_not_contains_active "$runtime_params_vars_file" 'algolia_migration_enabled' \
  "runtime_params does not expose Algolia migration availability as a module input"

assert_not_contains_active "$shared_main_file" 'algolia_migration_enabled\s*=' \
  "root module does not thread Algolia migration availability into runtime_params inputs"

assert_contains_active "$shared_main_file" 'domain\s*=\s*local\.deployment_domain' \
  "root main.tf passes the normalized deployment domain into monitoring module"
