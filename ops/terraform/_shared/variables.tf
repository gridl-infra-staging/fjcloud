...
variable "api_ami_id" {
  description = "AMI ID for the fjcloud API control-plane instance"
  type        = string

  validation {
    condition     = trimspace(var.api_ami_id) != ""
    error_message = "api_ami_id must be non-empty."
  }
}

variable "flapjack_ami_id" {
  description = "Packer-built AMI ID published for customer Flapjack VMs"
  type        = string

  validation {
    condition     = trimspace(var.flapjack_ami_id) != ""
    error_message = "flapjack_ami_id must be non-empty."
  }
}
