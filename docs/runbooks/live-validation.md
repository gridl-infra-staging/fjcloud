...
# Source these independently: API_AMI_ID from the running API instance ImageId,
# and FLAPJACK_AMI_ID from /fjcloud/staging/aws_ami_id.
terraform plan -var="env=staging" -var="api_ami_id=<API_AMI_ID>" -var="flapjack_ami_id=<FLAPJACK_AMI_ID>"
```

Required variables: `env` (staging or prod), `api_ami_id` (control-plane instance AMI),
`flapjack_ami_id` (customer-VM runtime pointer seed).
