...
export function customerFacingErrorMessage(error: unknown, fallback: string): string {
	if (error instanceof ApiRequestError) {
		const bodyMessage = customerFacingApiErrorBodyMessage(error.body);
...
function customerFacingApiErrorBodyMessage(body: unknown): string | null {
	const detail = apiErrorBodyDetail(body);
	if (!detail) return null;
	return isCustomerSafeMessage(detail) ? detail.trim() : null;
}

function apiErrorBodyDetail(body: unknown): string | null {
	if (!body || typeof body !== 'object') return null;
	const record = body as Record<string, unknown>;
	if (typeof record.detail === 'string') return record.detail;
	if (record.error && typeof record.error === 'object') {
		const nestedError = record.error as Record<string, unknown>;
		if (typeof nestedError.detail === 'string') return nestedError.detail;
	}
	return null;
}
