...
use crate::repos::{
    CatalogLifecycleTargetIdentity, IndexReplicaRepo, RepoError, TenantRepo, VmInventoryRepo,
};
...
impl ReplicaService {
    ...
    /// held, just before the guarded routing mutation runs. Test-only seam used
    /// to prove the open service window excludes competing catalog admission.
    ...
    pub async fn remove_replica(
        ...
        let tenant = self
            .tenant_repo
            .find_raw(customer_id, &replica.tenant_id)
            .await
        ...
        self.guarded_target_mutation(
            ...
            || async {
                self.replica_repo.set_status(replica_id, "removing").await?;
                self.replica_repo.delete(replica_id).await?;
                Ok(())
            },
        )
        .await?;
...
fn map_repo_error(error: RepoError) -> ReplicaError {
    match error {
        RepoError::Conflict(message) if message == "destination_conflict" => {
            ReplicaError::DestinationConflict
        }
        RepoError::Conflict(message) if message == "destination_changed" => {
            ReplicaError::DestinationChanged
        }
        RepoError::NotFound => ReplicaError::ReplicaNotFound,
        other => ReplicaError::Repo(other.to_string()),
    }
}
...
mod tests {
    ...
    fn replica_error_display_variants() {
        ...
        assert_eq!(
            ReplicaError::DestinationConflict.to_string(),
            "destination_conflict"
        );
        assert_eq!(
            ReplicaError::DestinationChanged.to_string(),
            "destination_changed"
        );
    }
}
