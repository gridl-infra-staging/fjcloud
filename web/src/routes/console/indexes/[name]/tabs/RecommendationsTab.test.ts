...
async function expectTooltipCopy(trigger: HTMLElement, expectedCopy: string): Promise<void> {
	...
	expect(tooltip).toHaveTextContent(expectedCopy);
...
describe('RecommendationsTab', () => {
	it('renders advanced-control tooltip triggers with single-sourced copy for recommendation inputs', async () => {
		render(RecommendationsTab, defaultProps());

		const modelTooltipTrigger = screen.getByRole('button', {
			name: 'What this recommendation model does'
		});
		const thresholdTooltipTrigger = screen.getByRole('button', {
			name: 'What the recommendation threshold does'
		});
		const objectIdTooltipTrigger = screen.getByRole('button', {
			name: 'What the recommendation object ID is for'
		});
		const maxRecommendationsTooltipTrigger = screen.getByRole('button', {
			name: 'What max recommendations controls'
		});
		...
		await fireEvent.change(modelSelect, { target: { value: 'looking-similar' } });
		await expectTooltipCopy(
			screen.getByRole('button', { name: 'What the recommendation object ID is for' }),
			'Enter the objectID that should act as the starting point for this recommendation preview.'
		);

		await fireEvent.change(modelSelect, { target: { value: 'trending-facets' } });
		expect(
			screen.queryByRole('button', { name: 'What the recommendation object ID is for' })
		).not.toBeInTheDocument();
		await expectTooltipCopy(
			screen.getByRole('button', { name: 'What the recommendation facet name is for' }),
			'Choose the facet attribute whose values you want to trend, such as category or brand.'
		);
		await expectTooltipCopy(
			screen.getByRole('button', { name: 'What the recommendation facet value is for' }),
