...
test_gate_reliability_api_checks_use_integration_binary_targets() {
    ...
    assert_contains "$cargo_invocations" "args=test -p api --test platform scheduler_test::" \
        "scheduler reliability check should use platform grouped binary selector"
    assert_contains "$cargo_invocations" "args=test -p api --test indexes reliability_replication_test::" \
        "replication reliability check should use indexes grouped binary selector"
    assert_contains "$cargo_invocations" "args=test -p api --test platform reliability_api_crash_test::" \
        "api crash reliability check should use platform grouped binary selector"
    assert_contains "$cargo_invocations" "args=test -p api --test platform reliability_cold_tier_test::" \
        "cold tier reliability check should use platform grouped binary selector"
    assert_contains "$cargo_invocations" "args=test -p api --test auth_admin reliability_security_test::" \
        "sql guard reliability check should use auth_admin grouped binary selector"
