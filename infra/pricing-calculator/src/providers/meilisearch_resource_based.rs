...
pub fn metadata() -> ProviderMetadata {
    super::provider_metadata(
        ...
        DISPLAY_NAME,
        Some(NaiveDate::from_ymd_opt(2026, 7, 6).expect("valid verification date")),
        SOURCE_URLS,
    )
}
...
/// Prices are the US East (N. Virginia) region — the pricing page default and
/// the region fjcloud itself runs in; other regions differ by a few percent.
...
pub const INSTANCE_TIERS: &[InstanceTier] = &[
    ...
    InstanceTier {
        ...
        vcpus: dec!(2),
        ram_gib: 8,
        monthly_cents: 11_568,
    },
    InstanceTier {
        name: "XL",
        vcpus: dec!(4),
        ram_gib: 16,
        monthly_cents: 25_936,
    },
];
...
mod tests {
    ...
    fn estimate_small_workload_within_base_storage() {
        ...
        // RAM: 0.19 × 2.5 = 0.48 GiB → XS tier (1 GiB, 1488 cents/mo)
        ...
        assert_eq!(est.line_items[0].amount_cents, 1488); // XS instance
    ...
    fn instance_tiers_match_official_platform_page() {
        ...
        assert_eq!(INSTANCE_TIERS[0].monthly_cents, 1_488);
        assert_eq!(INSTANCE_TIERS[4].name, "XL");
        assert_eq!(INSTANCE_TIERS[4].ram_gib, 16);
        assert_eq!(INSTANCE_TIERS[4].monthly_cents, 25_936);
    }
