...
impl FlapjackHttpClient for StaticSuccessFlapjackHttpClient {
    async fn send(
        ...
        Ok(FlapjackHttpResponse {
            status: 200,
            body: "{}".to_string(),
            request_api_key: "static-success-api-key".to_string(),
        })
    }
}
...
async fn build_index_capable_auth_app(
    ...
    let provisioning_service = Arc::new(ProvisioningService::new(
        crate::common::mock_vm_provisioner(),
        crate::common::mock_dns_manager(),
        node_secret_manager,
...
async fn verify_email_already_verified_returns_400() {
    ...
    let app3 = crate::common::test_app_with_repo(repo);
    let req2 = json_post(
        "/auth/verify-email",
        serde_json::json!({ "token": &verify_token }),
    );
...
async fn forgot_password_sends_reset_email() {
    ...
    // Forgot password
    let app2 = crate::common::build_test_app_with_email(repo.clone(), email_svc.clone());
    let req = json_post(
    ...
    assert!(sent_emails[0]
        ...
        .contains(&format!("reset-password/{reset_token}")));
    assert!(
        !sent_emails[0].text_body.contains("reset-password?token="),
        "reset email text body must not use the obsolete query-param route"
    );
}
...
async fn register_retries_verification_email_before_succeeding() {
    ...
    let app = crate::common::build_test_app_with_email(repo, email_svc.clone());
...
async fn reset_password_with_valid_token_succeeds() {
    ...
    // Reset password
    let app3 = crate::common::test_app_with_repo(repo.clone());
    let req = json_post(
        "/auth/reset-password",
        serde_json::json!({
            "token": reset_token,
    ...
    let app4 = crate::common::test_app_with_repo(repo);
    let login_req = json_post(
        "/auth/login",
        serde_json::json!({
            "email": "eve@example.com",
...
async fn signup_sends_verification_email() {
    let repo = crate::common::mock_repo();
    let email_svc = Arc::new(MockEmailService::new());
    let app = crate::common::build_test_app_with_email(repo.clone(), email_svc.clone());
    ...
    assert!(emails[0]
        ...
        .contains(&format!("verify-email/{verify_token}")));
    assert!(
        !emails[0].text_body.contains("verify-email?token="),
        "verification email text body must not use the obsolete query-param route"
    );
}
...
async fn resend_verification_email_for_unverified_customer() {
    ...
    assert!(emails[1]
        ...
        .contains(&format!("verify-email/{new_token}")));
    assert!(
        !emails[1].text_body.contains("verify-email?token="),
        "resend email text body must not use the obsolete query-param route"
    );
    assert!(
        !emails[1]
            .text_body
            .contains(&format!("verify-email/{original_token}")),
        "resend email text body must not include the old token"
    );
...
async fn resend_verification_invalid_auth_returns_401() {
    ...
    let app = crate::common::test_app_with_repo(repo);
    let jwt = crate::common::create_jwt_with_secret(customer.id, "wrong-secret");
...
async fn resend_verification_returns_503_when_email_delivery_fails() {
    let repo = crate::common::mock_repo();
    let registration_app = crate::common::test_app_with_repo(repo.clone());
    let resend_app =
        crate::common::build_test_app_with_email(repo, Arc::new(AlwaysFailEmailService));
...
async fn resend_verification_503_keeps_last_deliverable_token_and_allows_immediate_retry() {
    let repo = crate::common::mock_repo();
    let app = crate::common::build_test_app_with_email(
        repo.clone(),
...
async fn resend_password_reset_rotates_token_and_suppresses_cooldown_enumeration_signal() {
    ...
    assert!(
        !emails[2].html_body.contains("reset-password?token="),
        "resend-password-reset email must not use the obsolete query-param route"
    );
    assert!(
        emails[2]
            .text_body
            .contains(&format!("reset-password/{rotated_token}")),
        "resend-password-reset email text body should include rotated token path"
    );
    assert!(
        !emails[2].text_body.contains("reset-password?token="),
        "resend-password-reset email text body must not use the obsolete query-param route"
    );
}
...
async fn resend_password_reset_delivery_failure_restores_state_and_returns_200() {
    ...
    let reg_app = crate::common::test_app_with_repo(repo.clone());
    let reg_req = json_post(
        "/auth/register",
        serde_json::json!({
    ...
    let failing_app = crate::common::build_test_app_with_email(
        repo.clone(),
...
async fn resend_password_reset_rollback_failure_returns_500() {
    let repo = crate::common::mock_repo();
    let setup_app = crate::common::test_app_with_repo(repo.clone());
...
async fn register_returns_503_and_rolls_back_customer_when_email_delivery_fails() {
    let repo = crate::common::mock_repo();
    let app =
        crate::common::build_test_app_with_email(repo.clone(), Arc::new(AlwaysFailEmailService));
    ...
    let recovered_app = crate::common::test_app_with_repo(repo);
...
async fn forgot_password_first_delivery_failure_restores_empty_reset_state() {
    ...
    let failing_app = crate::common::build_test_app_with_email(repo.clone(), email_svc.clone());
