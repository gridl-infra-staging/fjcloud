...
async fn seed_index_with_flapjack_url_creates_vm_and_sets_vm_id() {
    ...
    assert_eq!(
        ...
        json!({
            "cpu_weight": 4.0,
            "mem_rss_bytes": 8_000_000_000u64,
            "disk_bytes": 100_000_000_000u64,
            "query_rps": 500.0,
            "indexing_rps": 200.0
        }),
        "seeded local flapjack VMs should use the ResourceVector capacity shape consumed by placement"
    );
...
#[tokio::test]
async fn seed_index_with_flapjack_url_relinks_existing_index_to_requested_vm() {
    let customer_repo = Arc::new(MockCustomerRepo::new());
    let customer = customer_repo.seed_verified_free_customer("Acme", "acme@test.com");
    ...
    let stale_vm = vm_inventory_repo
        .create(api::models::vm_inventory::NewVmInventory {
            region: "us-east-1".into(),
            provider: BARE_METAL_VM_PROVIDER.into(),
            hostname: "e2e-seed-stale".into(),
            flapjack_url: "https://api.staging.flapjack.foo".into(),
            capacity: json!({"cpu_cores": 8, "memory_gb": 32, "disk_gb": 500}),
        })
        .await
        .unwrap();
    let target_vm = vm_inventory_repo
        .create(api::models::vm_inventory::NewVmInventory {
            region: "us-east-1".into(),
            provider: BARE_METAL_VM_PROVIDER.into(),
            hostname: "vm-shared-f2b9c8a6".into(),
            flapjack_url: "http://vm-shared-f2b9c8a6.flapjack.foo:7700".into(),
            capacity: json!({"cpu_cores": 8, "memory_gb": 32, "disk_gb": 500}),
        })
        .await
        .unwrap();
    tenant_repo
        .set_vm_id(customer.id, &tenant.tenant_id, stale_vm.id)
        .await
        .unwrap();
    ...
    let body = axum::body::to_bytes(response.into_body(), usize::MAX)
        ...
        .unwrap();
    let resp: serde_json::Value = serde_json::from_slice(&body).unwrap();
    assert_eq!(resp["name"], "demo-shared-free");
    assert_eq!(
        resp["endpoint"],
        "http://vm-shared-f2b9c8a6.flapjack.foo:7700"
    );

    let relinked_tenant = tenant_repo
        .find_raw(customer.id, "demo-shared-free")
        .await
        .unwrap()
        .expect("tenant should still exist");
    assert_eq!(relinked_tenant.vm_id, Some(target_vm.id));
    assert_ne!(
        relinked_tenant.vm_id,
        Some(stale_vm.id),
        "seed reruns with a direct flapjack_url must repair stale VM linkage"
    );
}
...
#[tokio::test]
async fn seed_index_with_flapjack_url_relinks_existing_tenant_to_requested_vm() {
    let customer_repo = Arc::new(MockCustomerRepo::new());
    let customer = customer_repo.seed_verified_free_customer("Acme", "acme@test.com");
    let deployment_repo = Arc::new(MockDeploymentRepo::new());
    let tenant_repo = crate::common::mock_tenant_repo();
    ...
    let stale_deployment = deployment_repo.seed(
        customer.id,
        "control-plane-node",
        "us-east-1",
        "t4g.small",
    ...
    tenant_repo
        .create(customer.id, "tenant-a", stale_deployment.id)
        .await
        .expect("preseed existing tenant");
    let stale_vm = vm_inventory_repo
        .create(api::models::vm_inventory::NewVmInventory {
            region: "us-east-1".into(),
            provider: BARE_METAL_VM_PROVIDER.into(),
            hostname: "control-plane".into(),
            flapjack_url: "https://api.staging.flapjack.foo".into(),
            capacity: json!({"cpu_cores": 8, "memory_gb": 32, "disk_gb": 500}),
        })
        .await
        .expect("preseed stale VM");
    tenant_repo
        .set_vm_id(customer.id, "tenant-a", stale_vm.id)
        .await
        .expect("preseed stale tenant VM linkage");
    ...
    let vms = vm_inventory_repo
        ...
        .await
        .unwrap();
    let provider_aligned_requested_vms = vms
        .iter()
        .filter(|vm| {
            vm.provider == BARE_METAL_VM_PROVIDER
                && vm.flapjack_url == "http://vm-shared-f2b9c8a6.flapjack.foo:7700"
        })
        .collect::<Vec<_>>();
    assert_eq!(
        provider_aligned_requested_vms.len(),
        1,
        "rerun should create or reuse exactly one provider-aligned VM for the requested direct node"
    );

    let tenant = tenant_repo
        .find_raw(customer.id, "tenant-a")
        .await
        .unwrap()
        .expect("tenant should exist");
    assert_eq!(tenant.vm_id, Some(provider_aligned_requested_vms[0].id));
}
